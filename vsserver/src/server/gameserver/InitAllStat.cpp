...
#include	"skill/EffectEaglesEye1.h"
#include	"skill/EffectEaglesEye2.h"
#include	"skill/EffectMagicShield1.h"
#include	"skill/EffectMagicShield2.h"
#include "skill/EffectGladiator.h"
#include "skill/EffectAllysterWind.h"
#include "skill/EffectBloodyShout1.h"
#include "skill/EffectBloodyShout2.h"
...
void Slayer::initAllStat(int numPartyMember) throw()
{
	...
	if (isFlag(Effect::EFFECT_CLASS_BLESS))
	{
		...
		if (pBless != NULL)
		{
			...
			m_INT[ATTR_CURRENT] += pBless->getDEXBonus();
		}
	}
	if (isFlag(Effect::EFFECT_CLASS_BLESS2))
	{
		EffectBless2* pBless = dynamic_cast<EffectBless2*>(findEffect(Effect::EFFECT_CLASS_BLESS2));
		if (pBless != NULL)
		{
			// STR, DEX¸¦ ¿Ã¸°´Ù.
			m_STR[ATTR_CURRENT] += pBless->getSTRBonus();
...
//	cout << getName() << "ÀÇ ±âº» Çà¿î : " << m_Luck << endl;
	...
	//////////////////////////////////////////////////////////////////////////////
	// ´É·ÂÄ¡¿¡ ÀÇÇÑ Damage ¸¦ ÀúÀåÇØ µÐ´Ù. BERSER ±â¼úÀÇ º¸³Ê½º °è»êÀ» À§ÇØ¼­.
	//////////////////////////////////////////////////////////////////////////////
...
//		if ( level >= 125 ) HPBonusPercent = 50;
	...
	////	// ÀüÀï º¸³Ê½º
	// Áö±ÝÀº ÀüÀï ½ÂÆÐ¿¡ °ü°è¾øÀÌ ¾î´ÀÂÊÀÌµç º¸³Ê½º°¡ Àû¿ëµÉ ¼ö ÀÖ´Ù. by sigi
	...
	//////////////////////////////////////////////////////////////////////////
	// ÀÏ´Ü ±â¾î Ã¼Å© º¯¼ö¸¦ ÃÊ±âÈ­ÇØ¼­ ¸ðµç ±â¾î¸¦ ¾È ÀÔÀº °ÍÀ¸·Î °£ÁÖÇÏ°í ½ÃÀÛÇÑ´Ù.
	//////////////////////////////////////////////////////////////////////////////
	...
	//////////////////////////////////////////////////////////////////////////////
	// ¼ºÀ» ¼ÒÀ¯ÇÑ Á¾Á·Àº º¸³Ê½º ¿É¼ÇÀ» ¹Þ°Ô µÈ´Ù
	//////////////////////////////////////////////////////////////////////////////
	// Blood Bible °¢°¢ÀÇ º¸³Ê½º ¿É¼ÇÀ» ¹Þ´Â °É·Î °íÃÆ´Ù.
/*	if (m_pZone->isHolyLand() )
	...
	//////////////////////////////////////////////////////////////////////////////
	// Blood Bible °¢°¢ÀÇ º¸³Ê½º ¿É¼ÇÀ» ¹Þ´Â´Ù.
	//////////////////////////////////////////////////////////////////////////////
/*	if ( m_pZone->isHolyLand() && !g_pWarSystem->hasActiveRaceWar() )
	...
	if ( m_pPetInfo != NULL )
	{
		...
		if ( m_pPetInfo->getPetVersionOption() != 0 ) computeOptionStat( m_pPetInfo->getPetVersionOption() );
		computePetItemOptionTypeStat( m_pPetInfo );
	}
	...
	// ÀÎº¥Åä¸®³»¿¡ ¼ÒÀ¯ÇÏ°íÀÖ´Â Âü È®ÀÎÇÏ±â.
	// ÀÎº¥Åä¸® ÁÂÇ¥ x8 y0 ÀÌ»óÀÇ À§Ä¡¿¡ ÀÖ´ÂºÎºÐ È¿°ú Àû¿ë.
	// ¸¶¿ì½º¿¡ µé°íÀÖ´Â°Ç Á¦¿Ü ÇÏµµ·Ï ÇÑ´Ù.
	for(int x=8; x<=9; x++){
		for(int y=0; y<6; y++){

			Item* pItem = getInventory()->SerchStoneItem(x,y);
			if(pItem != NULL && pItem != getExtraInventorySlotItem()){
					computeItemStat(pItem);
			}
		}
	}
	...
	{
		Item* pItem = m_pWearItem[i];
		if ( pItem != NULL && m_pRealWearingCheck[i] == true && pItem->getItemClass() == Item::ITEM_CLASS_CORE_ZAP && pItem->getItemType()<4 )
		{
	...
	{
		computeOptionStat(182); //¸ðÀú 9
		computeOptionStat(185); //¸ð´É 3
	}
	...
	// ÀÏ´Ü À§¿¡¼­ ´Ù ÀÔ¾ú´Âµ¥..
	// ´É·ÂÄ¡¿¡ µû¶ó¼­ º¹Àå??Àû¿ëÀÌ ¾ÈµÇ´Â ¾ÆÀÌÅÛÀº º¹Àå Á¤º¸¸¦ ¾ø¾Ø´Ù.
	// ÀÌÀü¿¡´Â ¸ø ÀÔ¾ú´Âµ¥ ÀÌÁ¦´Â ÀÔÀ» ¼ö ÀÖ´Ù¸é ÀÔÈ÷´Â ÆÐÅ¶À» º¸³½´Ù.
	// by sigi. 2002.10.30
	for (int i=0; i<WEAR_MAX; i++)
	{
	...
	//////////////////////////////////////////////////////////////////////////////
	// ºÎ°¡ÀûÀÎ ´É·ÂÄ¡¸¦ Á÷Á¢ ¼öÁ¤?Ï´?ÀÌÆåÆ®¸¦ °Ë»çÇÑ´Ù.
	//////////////////////////////////////////////////////////////////////////////
	if (isFlag(Effect::EFFECT_CLASS_STRIKING))
	{
		...
		if (pStriking != NULL)
		{
			...
			if (pWeapon != NULL && pStriking->isTargetItem( pWeapon ) )
			{
				...
				// ItemOID°¡ ÀÏÄ¡ÇÑ´Ù´Â ¸»Àº ¹æ±Ý ½ºÆ®¶óÀÌÅ·ÀÌ °É·È°Å³ª,
	...
	if (isFlag(Effect::EFFECT_CLASS_STRIKING2))
	{
		EffectStriking2* pStriking = dynamic_cast<EffectStriking2*>(findEffect(Effect::EFFECT_CLASS_STRIKING2));
		if (pStriking != NULL)
		{
			Damage_t   DamageBonus  = pStriking->getDamageBonus();
			...
			if (pWeapon != NULL && pStriking->isTargetItem( pWeapon ) )
			{
				...
				// ItemOID°¡ ÀÏÄ¡ÇÑ´Ù´Â ¸»Àº ¹æ±Ý ½ºÆ®¶óÀÌÅ·ÀÌ °É·È°Å³ª,
				// ´Ù¸¥ ¹«±â¸¦ ?é¾ú´Ù°? ´Ù½Ã ½ºÆ®¶óÀÌÅ·ÀÌ °É¸° ¹«±â¸¦
				// µé¾ú´Ù´Â ¸»ÀÌ´Ù. ±×·¯¹Ç·Î ÀÌÆåÆ®¸¦ ºÙÀÌ¶ó°í ³¯·ÁÁà¾ß ÇÑ´Ù.
				GCAddEffect gcAddEffect;
				gcAddEffect.setObjectID(m_ObjectID);
				gcAddEffect.setEffectID(Effect::EFFECT_CLASS_STRIKING2);
				gcAddEffect.setDuration(pStriking->getRemainDuration());
				m_pZone->broadcastPacket(m_X, m_Y, &gcAddEffect);
			}
			else
...
/*	if (isFlag(Effect::EFFECT_CLASS_PARALYZE))
	...
	//////////////////////////////////////////////////////////////////////
	// ¸ÅÁ÷½¯µå ½ÃÀÛ
	// ¸ÅÁ÷ ½¯µå »ç¿ë½Ã¿¡´Â "¸¶¹ý ¹æ¾î·Â"ÀÌ ´ëÆø Áõ°¡ÇÑ´Ù.
	/////////////////////////////////////////////////////////////////////
	if( isFlag(Effect::EFFECT_CLASS_MAGIC_SHIELD1) ){
		EffectMagicShield1* pDancingSword = dynamic_cast<EffectMagicShield1*>(findEffect(Effect::EFFECT_CLASS_MAGIC_SHIELD1));
		if (pDancingSword!= NULL)
		{
			m_MagicDamageReduce += pDancingSword->getLevel();
		}
	}
	if( isFlag(Effect::EFFECT_CLASS_MAGIC_SHIELD2) ){
		EffectMagicShield2* pDancingSword = dynamic_cast<EffectMagicShield2*>(findEffect(Effect::EFFECT_CLASS_MAGIC_SHIELD2));
		if (pDancingSword!= NULL)
		{
			m_MagicDamageReduce += pDancingSword->getLevel();
		}
	...
	//////////////////////////////////////////////////////////////////////
	// ±Û·¹µð¿¡ÀÌÅÍ ½ÃÀÛ
	// ±Û·¹µð¿¡ÀÌÅÍ »ç¿ë½Ã ¹æ¾î·ÂÀÌ Áõ°¡ÇÑ´Ù.
	/////////////////////////////////////////////////////////////////////
	if( isFlag(Effect::EFFECT_CLASS_GLADIATOR) ){
		EffectGladiator* pDancingSword = dynamic_cast<EffectGladiator*>(findEffect(Effect::EFFECT_CLASS_GLADIATOR));
		if (pDancingSword!= NULL)
		{

			m_Damage[ATTR_CURRENT]  += pDancingSword->getDamageBonus();
			m_Damage[ATTR_MAX]     += pDancingSword->getDamageBonus();
			m_Protection[ATTR_CURRENT] += pDancingSword->getProtectionBonus();
		}
	}
	//////////////////////////////////////////////////////////////////////
	// ±Û·¹µð¿¡ÀÌÅÍ ³¡
	...
	if (isFlag(Effect::EFFECT_CLASS_BERSERKER))
	{
		...
		if (pBerserker != NULL)
		{
			...
			// µ¥¹ÌÁö ¹× ÅõÈý º¸³Ê½º, µðÆæ½º ¹× ÇÁ·ÎÅØ¼Ç Æä³ÎÆ¼´Â ÆÛ¼¾Æ® °ªÀÌ´Ù.
			// µ¥¹ÌÁö´Â ´É·ÂÄ¡¿¡ ÀÇÇÑ µ¥¹ÌÁö¿Í ¹«±â(µµ) µ¥¹ÌÁöÀÇ ÇÕ¿¡ ´ëÇÑ ºñÀ²ÀÌ´Ù.
			// ´Ù¸¥ ÀÌÆåÆ®¿¡ ÀÇÇÑ Ãß°¡ µ¥¹ÌÁö??ÀÌ °è»ê¿¡¼­ Á¦¿ÜÇÑ´Ù.
...
//				if ( m_Resist[i] < 0 ) m_Resist[i] = 0;
	...
	if (isFlag(Effect::EFFECT_CLASS_HOLY_ARMOR))
	{
		...
		if ( pHolyArmor != NULL )
		{
			m_Defense[ATTR_CURRENT] += pHolyArmor->getDefBonus();
		}
	}

	if (isFlag(Effect::EFFECT_CLASS_HOLY_ARMOR2))
	{
		EffectHolyArmor2* pHolyArmor = dynamic_cast<EffectHolyArmor2*>(findEffect(Effect::EFFECT_CLASS_HOLY_ARMOR2));
...
//			m_MagicDamageReduce += pWhitsuntide->getBonus();
	...
	if( isFlag(Effect::EFFECT_CLASS_THE_ROAR_OF_AWAKEN_WARRIOR) ){
			m_Resist[MAGIC_DOMAIN_POISON] += 15;
			m_Resist[MAGIC_DOMAIN_ACID] += 15;
			m_Resist[MAGIC_DOMAIN_CURSE] += 15;
			m_Resist[MAGIC_DOMAIN_BLOOD] += 15;
			m_Protection[ATTR_CURRENT] += 100;
			m_HP[ATTR_MAX] += getPercentValue(m_HP[ATTR_MAX], 10);
			m_STR[ATTR_CURRENT] += 30;
			m_DEX[ATTR_CURRENT] += 30;
			m_INT[ATTR_CURRENT] += 30;
	}
	...
	if (isFlag(Effect::EFFECT_CLASS_INTIMATE_GRAIL2))
	{
		EffectIntimateGrail2* pIntimateGrail = dynamic_cast<EffectIntimateGrail2*>(findEffect(Effect::EFFECT_CLASS_INTIMATE_GRAIL2));

		if ( pIntimateGrail != NULL )
		{
			// ½½·¹ÀÌ¾î´Â Ãàº¹
			int hpratio = 25 + (int)(pIntimateGrail->getSkillLevel() / 3.3);
			m_HP[ATTR_MAX] += getPercentValue(m_HP[ATTR_MAX], hpratio);
			m_MP[ATTR_MAX] += getPercentValue(m_MP[ATTR_MAX], hpratio);

			int defratio = 20 + (pIntimateGrail->getSkillLevel() / 5);
			m_Defense[ATTR_CURRENT] += getPercentValue(m_Defense[ATTR_CURRENT] , defratio);
			m_Protection[ATTR_CURRENT] += getPercentValue(m_Protection[ATTR_CURRENT], defratio);
		}
	}

	if (isFlag(Effect::EFFECT_CLASS_BEHMOTH_FORCE_SCROLL))
	{
		EffectSevenFoldStone* pEffect = dynamic_cast<EffectSevenFoldStone*>(findEffect(Effect::EFFECT_CLASS_BEHMOTH_FORCE_SCROLL));
		m_Defense[ATTR_CURRENT] = m_Defense[ATTR_CURRENT] + 30;
	}

	if (isFlag(Effect::EFFECT_CLASS_SAFE_FORCE_SCROLL))
	{
		EffectSevenFoldStone* pEffect = dynamic_cast<EffectSevenFoldStone*>(findEffect(Effect::EFFECT_CLASS_SAFE_FORCE_SCROLL));
		m_Protection[ATTR_CURRENT] = m_Protection[ATTR_CURRENT] + 30;
	}

	if (isFlag(Effect::EFFECT_CLASS_CANELIAN_FORCE_SCROLL))
	{
		int Resist = 0;
		EffectSevenFoldStone* pEffect = dynamic_cast<EffectSevenFoldStone*>(findEffect(Effect::EFFECT_CLASS_CANELIAN_FORCE_SCROLL));
		Resist = 18;
			for (int i=0; i<MAGIC_DOMAIN_MAX; i++)
			{
				m_Resist[i] += Resist;
	...
	{
		int Resist = 0;
		EffectSevenFoldStone* pEffect = dynamic_cast<EffectSevenFoldStone*>(findEffect(Effect::EFFECT_CLASS_FURIOUS_INCENSE));
			Resist = 30;
		m_Damage[ATTR_CURRENT] += Resist;
		m_Damage[ATTR_MAX] += Resist;
		m_CriticalRatio[ATTR_CURRENT] += Resist;
		m_CriticalRatio[ATTR_MAX]     += Resist;
	}

	if (isFlag(Effect::EFFECT_CLASS_STRENGTHEN_INCENSE))
	{
		int Resist = 0;
		EffectSevenFoldStone* pEffect = dynamic_cast<EffectSevenFoldStone*>(findEffect(Effect::EFFECT_CLASS_STRENGTHEN_INCENSE));

			Resist = 100;

		m_HP[ATTR_MAX] += 500 + Resist;
		m_Protection[ATTR_CURRENT] += Resist;
	}

	if (isFlag(Effect::EFFECT_CLASS_GREYFELL_FORCE_SCROLL))
	{
		int Resist = 0;
		EffectSevenFoldStone* pEffect = dynamic_cast<EffectSevenFoldStone*>(findEffect(Effect::EFFECT_CLASS_GREYFELL_FORCE_SCROLL));

			Resist = 30;

			m_STR[ATTR_CURRENT] += Resist;
	}

	if (isFlag(Effect::EFFECT_CLASS_SOVIUS_FORCE_SCROLL))
	{
		int Resist = 0;
		EffectSevenFoldStone* pEffect = dynamic_cast<EffectSevenFoldStone*>(findEffect(Effect::EFFECT_CLASS_SOVIUS_FORCE_SCROLL));

			Resist = 30;

			m_DEX[ATTR_CURRENT] += Resist;
	}
	if (isFlag(Effect::EFFECT_CLASS_MIR_FORCE_SCROLL))
	{
		int Resist = 0;
		EffectSevenFoldStone* pEffect = dynamic_cast<EffectSevenFoldStone*>(findEffect(Effect::EFFECT_CLASS_MIR_FORCE_SCROLL));

			Resist = 30;

			m_INT[ATTR_CURRENT] += Resist;
	}
	if (isFlag(Effect::EFFECT_CLASS_ODIN_FORCE_SCROLL))
	{
		int Resist = 0;
		EffectSevenFoldStone* pEffect = dynamic_cast<EffectSevenFoldStone*>(findEffect(Effect::EFFECT_CLASS_ODIN_FORCE_SCROLL));
			Resist = 20;

			m_MagicBonusDamage += Resist;
	}
	if (isFlag(Effect::EFFECT_CLASS_TITUOS_FORCE_SCROLL))
	{
		int Resist = 0;
		EffectSevenFoldStone* pEffect = dynamic_cast<EffectSevenFoldStone*>(findEffect(Effect::EFFECT_CLASS_TITUOS_FORCE_SCROLL));
			Resist = 25;

			m_PhysicBonusDamage += Resist;
	}
	if (isFlag(Effect::EFFECT_CLASS_EXTREAME_FORCE_SCROLL))
	{
		int Resist = 0;
		EffectSevenFoldStone* pEffect = dynamic_cast<EffectSevenFoldStone*>(findEffect(Effect::EFFECT_CLASS_EXTREAME_FORCE_SCROLL));
			Resist = 18;
	...
	{
		int Resist = 0;
		EffectSevenFoldStone* pEffect = dynamic_cast<EffectSevenFoldStone*>(findEffect(Effect::EFFECT_CLASS_ACCURA_FORCE_SCROLL));
			Resist = 30;

		m_ToHit[ATTR_CURRENT] += Resist;
		m_ToHit[ATTR_MAX] += Resist;
	}
	if (isFlag(Effect::EFFECT_CLASS_LIFE_FORCE_SCROLL))
	{
		int Resist = 0;
		EffectSevenFoldStone* pEffect = dynamic_cast<EffectSevenFoldStone*>(findEffect(Effect::EFFECT_CLASS_LIFE_FORCE_SCROLL));
			Resist = 500;


		m_HP[ATTR_MAX] += Resist;
	}


	// »õ·Î Ãß°¡µÈ ÆÐ½Ãºê ±â¼ú
		if ( hasSkill( SKILL_BLAST_OF_VITALITY )){
		m_HP[ATTR_MAX] += 800;
		}
		if ( hasSkill( SKILL_BLAST_OF_MANA)){
		m_MagicBonusDamage += 40;
		}

		if ( hasSkill( SKILL_BLAST_OF_ASSAULT)){
		m_PhysicBonusDamage += 40;
		}

		if ( hasSkill( SKILL_BLAST_OF_FALCON)){
		m_ToHit[ATTR_CURRENT]         += 100;
		m_ToHit[ATTR_MAX]             += 100;
		}

		// ¸ÅÁ÷ ¾îº¸ÀÌµå Ãß°¡
		if( hasSkill(SKILL_MAGIC_AVOID1)){
		m_MagicDefense += 3;
		}
		if( hasSkill(SKILL_MAGIC_AVOID2)){
		m_MagicDefense += 4;
		}
		if( hasSkill(SKILL_MAGIC_AVOID3)){
		m_MagicDefense += 110;
		}
		if ( hasSkill( SKILL_BLAST_OF_EVASION) ){
		m_Defense[ATTR_CURRENT]       += 100;
		m_Defense[ATTR_MAX]           += 100;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH1 )){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH2 )){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH3 )){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH4 )){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH5 )){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH6 )){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH7 )){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH8 )){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH9 )){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH10 )){
		m_HP[ATTR_MAX] += 500;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH11 )){
		m_HP[ATTR_MAX] += 700;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH12 )){
		m_HP[ATTR_MAX] += 900;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH13 )){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH14 )){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH15 )){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_BIND_OF_BLOOD )){
		m_HP[ATTR_MAX] += 1500;
		}
		if ( hasSkill( SKILL_PRAYER_OF_WAR ) && pWeapon != NULL && pWeapon->getItemClass() == Item::ITEM_CLASS_BLADE){
		m_HP[ATTR_MAX] += 1000;
		m_PhysicBonusDamage	+= 30;
		}
		if ( hasSkill( SKILL_PRAYER_OF_GUARD ) && pWeapon != NULL && pWeapon->getItemClass() == Item::ITEM_CLASS_SWORD){
		m_PhysicBonusDamage	+= 30;
		m_MagicDamageReduce += 50;
		}
		if ( hasSkill( SKILL_PRAYER_OF_COMBAT ) && pWeapon != NULL && pWeapon->isGun()){
		m_CriticalRatio[ATTR_CURRENT] += 50;
		m_CriticalRatio[ATTR_MAX]     += 50;
		m_ToHit[ATTR_CURRENT]         += 50;
		m_ToHit[ATTR_MAX]             += 50;
		}
		if ( hasSkill( SKILL_PRAYER_OF_GUARD ) && pWeapon != NULL && pWeapon->getItemClass() == Item::ITEM_CLASS_CROSS){
		m_MagicBonusDamage	+= 50;
		m_MagicDamageReduce += 50;
		}
		if ( hasSkill( SKILL_PRAYER_OF_GUARD ) && pWeapon != NULL && pWeapon->getItemClass() == Item::ITEM_CLASS_MACE){
		m_MagicBonusDamage	+= 30;
		m_PhysicDamageReduce += 30;
		}


	// ÆÐ½Ãºê ±â¼ú¿¡ ÀÇÇØ ¿Ã¶ó°¡´Â ´É·ÂÄ¡¸¦ °è»êÇÑ´Ù.
	if (pWeapon != NULL)
	{
...
//				m_CriticalRatio[ATTR_MAX]     = m_CriticalRatio[ATTR_MAX] + CriticalRatioBonus;
		...
		if(isFlag(Effect::EFFECT_CLASS_EAGLES_EYE1)){
				EffectEaglesEye1* pEffect = dynamic_cast<EffectEaglesEye1*>(findEffect(Effect::EFFECT_CLASS_EAGLES_EYE1));
				if(pEffect != NULL){
					ToHitBonus += pEffect->getLevel();
				}
		}

		// ÀÌ±Û¾ÆÀÌ 2
		if(isFlag(Effect::EFFECT_CLASS_EAGLES_EYE2)){
				EffectEaglesEye2* pEffect = dynamic_cast<EffectEaglesEye2*>(findEffect(Effect::EFFECT_CLASS_EAGLES_EYE2));
				if(pEffect != NULL){
					ToHitBonus += pEffect->getLevel();
				}
		}
...
/*	cout << "¹°¸®°ø°Ý·Â " << m_PhysicBonusDamage << endl;
	cout << "¹°¸®¹æ¾î·Â " << m_PhysicDamageReduce << endl;
	cout << "¸¶¹ý°ø°Ý·Â " << m_MagicBonusDamage << endl;
	cout << "¸¶¹ý¹æ¾î·Â " << m_MagicDamageReduce << endl;*/

	cout << "³¡±îÁö´Â µ¹¾Ò´Âµ¥.." << endl;
...
void Slayer::computeItemStat(Item* pItem) throw()
{
	...
	if ( pItem->getHeroOption() > 0 && pItem->getHeroOptionAttr() > 0 )
	{
			computeOptionClassStat( (OptionClass)(pItem->getHeroOption()-1), (int)pItem->getHeroOptionAttr() );
	}
	...
	// ¼¼Æ® ¿É¼Ç ¹ßµ¿.
	const list<OptionType_t>& optionType2 = pItem->getOptionTypeList2();
	list<OptionType_t>::const_iterator itr2;
	for (itr2=optionType2.begin(); itr2!=optionType2.end(); itr2++)
	{
...
void Slayer::computeSetItemStat(Item* pItem) throw()
{
	...
	// ¼¼Æ® ¿É¼Ç ¹ßµ¿.
	const list<OptionType_t>& optionType = pItem->getOptionTypeList2();
	list<OptionType_t>::const_iterator itr;
	for (itr=optionType.begin(); itr!=optionType.end(); itr++)
	{
		computeOptionStat( *itr );
...
void Slayer::computePetItemOptionTypeStat(PetInfo* pPetInfo) throw()
{
	...
	// ¼¼Æ® ¿É¼Ç ¹ßµ¿.
	const list<OptionType_t>& optionType = pPetInfo->getOptionType();
	list<OptionType_t>::const_iterator itr;
	for (itr=optionType.begin(); itr!=optionType.end(); itr++)
	{
		computeOptionStat( *itr );
	...
	const list<OptionType_t>& optionType2 = pPetInfo->getMixOptionType();
	list<OptionType_t>::const_iterator itr2;
	for (itr2=optionType2.begin(); itr2!=optionType2.end(); itr2++)
	{
		computeOptionStat( *itr2 );
...
void Slayer::computeOptionClassStat( OptionClass OClass, int PlusPoint )
{
	switch (OClass) 
	{
		...
		case OPTION_CRITICAL_HIT_DAMGE:
		{
			m_CriticalAddDamage += PlusPoint;
			break;
		}
		case OPTION_HIT_ADD_GOLD:
		{
			m_HitAddGold += PlusPoint;
			break;
		}
		case OPTION_HIT_ADD_EXP:
		{
			m_HitAddExp += PlusPoint;
			break;
		}
		case OPTION_HIT_SLEEP:
		{
			m_HitSleep += PlusPoint;
			break;
		}
		case OPTION_HIT_ICE:
		{
		...
		case OPTION_ADD_ATTACK:
		{
			m_HitAddAttack += PlusPoint;
			break;
		}
		case OPTION_ADD_MAGIC:
		{
			m_HitAddMagic += PlusPoint;
			break;
		}
		case OPTION_YOU_RES:
		{
		...
		{
			m_AddPlayerDamage += PlusPoint;
			break;
		}
		case OPTION_MAGIC_DEFENSE:
		{
			m_MagicDefense += PlusPoint;
			break;
		}
		case OPTION_DROP_MAGIC:
		{
			m_DropMagic += PlusPoint;
			break;
		}
		default:
			break;
	...
	throw()
{
	...
	BEGIN_DB
	{
		...
		pResult = pStmt->executeQuery("SELECT OwnerID FROM CharterStatus WHERE OwnerID = '%s'", m_Name.c_str());
		if(pResult->getRowCount() == 1){
		pStmt->executeQuery("UPDATE CharterStatus SET STR = %d, DEX = %d, `INT` = %d, HP = %d, MP = %d, HPSteal = %d, MPSteal = %d, HPRegeneration = %d,  MPRegeneration = %d,  ToHit = %d,  Defense = %d,  Damage = %d,  Protection = %d,  PoisonResistance = %d,  AcidResistance = %d,  CurseResistance = %d,  BloodResistance = %d, AttackSpeed = %d,  CriticalHit = %d,  Luck = %d,  MagicDamage = %d,  PhysicalDamage = %d,  MagicProtection = %d,  PhysicalProtection = %d, CriticalHitPlusDamage = %d, HitToGold = %d, HitToExp = %d, HitToSleep = %d, HitToIce = %d, AddAttackPercent = %d,  AddMagicPercent = %d,  TargetResitanceDown = %d,  HitToPlayerAddDamage = %d, MagicDefense = %d, DropItemMagic = %d WHERE OwnerID ='%s'", 
		m_STR[ATTR_CURRENT], m_DEX[ATTR_CURRENT], m_INT[ATTR_CURRENT], m_HP[ATTR_MAX], m_MP[ATTR_MAX], m_HPStealAmount, m_MPStealAmount, m_HPRegen, m_MPRegen, m_ToHit[ATTR_CURRENT], m_Defense[ATTR_CURRENT], 
		m_Damage[ATTR_CURRENT], m_Protection[ATTR_CURRENT], m_Resist[MAGIC_DOMAIN_POISON], m_Resist[MAGIC_DOMAIN_ACID], m_Resist[MAGIC_DOMAIN_CURSE], m_Resist[MAGIC_DOMAIN_BLOOD], m_AttackSpeed[ATTR_CURRENT],
		m_CriticalRatio[ATTR_CURRENT], m_Luck, m_MagicBonusDamage, m_PhysicBonusDamage, m_PhysicDamageReduce, m_MagicDamageReduce, m_CriticalAddDamage, m_HitAddGold, m_HitAddExp, m_HitSleep, m_HitIce, m_HitAddAttack, m_HitAddMagic, m_YouReg, m_AddPlayerDamage, m_MagicDefense, m_DropMagic, m_Name.c_str());
		}
		SAFE_DELETE(pStmt);
	} 
	END_DB(pStmt)
	...
	throw()
{
	...
	//////////////////////////////////////////////////////////////////////////////
	// ´É·ÂÄ¡ °è»ê??À§ÇÑ ÆÄ¶ó¹ÌÅÍµéÀ» ÃÊ±âÈ­ÇÑ´Ù.
	//////////////////////////////////////////////////////////////////////////////
	attr.nSTR    = m_STR[ATTR_CURRENT];
	attr.nDEX    = m_DEX[ATTR_CURRENT];
	attr.nINT    = m_INT[ATTR_CURRENT];
...
*/
	//////////////////////////////////////////////////////////////////////////////
	// Blood Bilbe °¢°¢ÀÇ º¸³Ê½º ¿É¼ÇÀ» ¹Þ´Â´Ù.
	//////////////////////////////////////////////////////////////////////////////
/*	if ( m_pZone->isHolyLand() && !g_pWarSystem->hasActiveRaceWar() )
	{
		const BloodBibleBonusHashMap& bloodBibleBonus = g_pBloodBibleBonusManager->getBloodBibleBonuses();
...
//				if ( m_Resist[i] < 0 ) m_Resist[i] = 0;
	...
	// by sigi. 2002.6.19
	// isEffect¸¦ isFlag·Î ¹Ù²Þ. 2003.3.27 by Sequoia
	...
	if (isFlag(Effect::EFFECT_CLASS_INTIMATE_GRAIL2))
	{
		EffectIntimateGrail2* pIntimateGrail2 = dynamic_cast<EffectIntimateGrail2*>(findEffect(Effect::EFFECT_CLASS_INTIMATE_GRAIL2));
	...
	if(isFlag(Effect::EFFECT_CLASS_RAGE_OF_BLOOD))
	{
		m_Protection[ATTR_CURRENT] += getPercentValue(m_Protection[ATTR_CURRENT], 15);
		m_ToHit[ATTR_CURRENT] += getPercentValue(m_ToHit[ATTR_CURRENT], 15);
		m_ToHit[ATTR_MAX] += getPercentValue(m_ToHit[ATTR_MAX], 15);
		m_HPRegen += 60;
	}
	//////////////////////////////////////////////////////////////////////
	// ¸ÅÁ÷½¯µå ½ÃÀÛ
	// ¸ÅÁ÷ ½¯µå »ç¿ë½Ã¿¡´Â "¸¶¹ý ¹æ¾î·Â"ÀÌ ´ëÆø Áõ°¡ÇÑ´Ù.
	/////////////////////////////////////////////////////////////////////
	if( isFlag(Effect::EFFECT_CLASS_BLOODY_SHOUT1) ){
		m_HP[ATTR_MAX] += getPercentValue(m_HP[ATTR_MAX], 10);
	}
	if( isFlag(Effect::EFFECT_CLASS_BLOODY_SHOUT2) ){
		m_HP[ATTR_MAX] += getPercentValue(m_HP[ATTR_MAX], 20);
	}

	if( isFlag(Effect::EFFECT_CLASS_TRUE_COLOR_OF_VAMP_LOAD) ){
			m_Resist[MAGIC_DOMAIN_POISON] += 15;
			m_Resist[MAGIC_DOMAIN_ACID] += 15;
			m_Resist[MAGIC_DOMAIN_CURSE] += 15;
			m_Resist[MAGIC_DOMAIN_BLOOD] += 15;
	...
	// »õ·Î Ãß°¡µÈ ÆÐ½Ãºê ±â¼ú
		if ( hasSkill( SKILL_BLAST_OF_VITALITY ) != NULL ){
		m_HP[ATTR_MAX] += 800;
		}
		if ( hasSkill( SKILL_BLAST_OF_MANA) != NULL ){
		m_MagicBonusDamage += 40;
		}
		if ( hasSkill( SKILL_BLAST_OF_ASSAULT) != NULL ){
		m_PhysicBonusDamage += 40;
		}
		if ( hasSkill( SKILL_BLAST_OF_FALCON) != NULL ){
		m_ToHit[ATTR_CURRENT]         += 100;
		m_ToHit[ATTR_MAX]             += 100;
		}
		if ( hasSkill( SKILL_BLAST_OF_EVASION) != NULL ){
		m_Defense[ATTR_CURRENT]       += 100;
		m_Defense[ATTR_MAX]           += 100;
		}
		// ¸ÅÁ÷ ¾îº¸ÀÌµå Ãß°¡
		...
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH1 ) != NULL ){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH2 ) != NULL ){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH3 ) != NULL ){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH4 ) != NULL ){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH5 ) != NULL ){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH6 ) != NULL ){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH7 ) != NULL ){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH8 ) != NULL ){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH9 ) != NULL ){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH10 ) != NULL ){
		m_HP[ATTR_MAX] += 500;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH11 ) != NULL ){
		m_HP[ATTR_MAX] += 700;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH12 ) != NULL ){
		m_HP[ATTR_MAX] += 900;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH13 ) != NULL ){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH14 ) != NULL ){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_INCREASE_OF_HEALTH15 ) != NULL ){
		m_HP[ATTR_MAX] += 200;
		}
		if ( hasSkill( SKILL_BIND_OF_BLOOD ) != NULL ){
		m_HP[ATTR_MAX] += 1500;
		}
		if ( hasSkill( SKILL_GRIM_EXTINCTION ) != NULL ){
		m_PhysicBonusDamage	+= 30;
		m_ToHit[ATTR_CURRENT]         += 30;
		m_ToHit[ATTR_MAX]             += 30;
		}
		if ( hasSkill( SKILL_GRIM_REAPER) != NULL ){
		m_MagicBonusDamage	+= 30;
		m_MagicDamageReduce += 30;
		}
	...
	throw()
{
	...
	// ?¼·Î¿öÁ?STR, DEX, INT·Î »õ·Î °è»êÀ» ÇÑ ´ÙÀ½
	// ¾ÆÀÌÅÛ ¶Ç´Â ¸¶¹ý ¼öÄ¡¸¦ ´õÇÑ´Ù. 
	m_HP[ATTR_MAX] = computeHP(CClass, &cur_attr);
	m_HP[ATTR_MAX] += m_HP[ATTR_BASIC];
...
/*	OptionClass   OClass        = pOptionInfo->getClass();
	...
	switch (OClass)
	{
		...
		case OPTION_MP:
			...
			m_HP[ATTR_BASIC] += pOptionInfo->getPlusPoint();
			break;
		// ¹ìÆÄÀÌ¾î´Â MPÈí¼ö?É¼ÇÀ?ºÙÀº ¾ÆÀÌÅÛµµ HPÈí¼ö·Î Ã³¸®?ØÁØ´?
		// 2003. 1. 17. Sequoia
		case OPTION_HP_STEAL:
		case OPTION_MP_STEAL:
			m_HPStealAmount += pOptionInfo->getPlusPoint();
	...
	throw()
{
	...
	BEGIN_DB
	{
		...
		if(pResult->getRowCount() == 1){
		pStmt->executeQuery("UPDATE CharterStatus SET STR = %d, DEX = %d, `INT` = %d, HP = %d, MP = %d, HPSteal = %d, MPSteal = %d, HPRegeneration = %d,  MPRegeneration = %d,  ToHit = %d,  Defense = %d,  Damage = %d,  Protection = %d,  PoisonResistance = %d,  AcidResistance = %d,  CurseResistance = %d,  BloodResistance = %d, AttackSpeed = %d,  CriticalHit = %d,  Luck = %d,  MagicDamage = %d,  PhysicalDamage = %d,  MagicProtection = %d,  PhysicalProtection = %d, CriticalHitPlusDamage = %d, HitToGold = %d, HitToExp = %d, HitToSleep = %d, HitToIce = %d, AddAttackPercent = %d,  AddMagicPercent = %d,  TargetResitanceDown = %d,  HitToPlayerAddDamage = %d, MagicDefense = %d, DropItemMagic = %d WHERE OwnerID ='%s'", 
		m_STR[ATTR_CURRENT], m_DEX[ATTR_CURRENT], m_INT[ATTR_CURRENT], m_HP[ATTR_MAX], m_HP[ATTR_MAX], m_HPStealAmount, m_HPStealAmount, m_HPRegen, m_HPRegen, m_ToHit[ATTR_CURRENT], m_Defense[ATTR_CURRENT], 
		m_Damage[ATTR_CURRENT], m_Protection[ATTR_CURRENT], m_Resist[MAGIC_DOMAIN_POISON], m_Resist[MAGIC_DOMAIN_ACID], m_Resist[MAGIC_DOMAIN_CURSE], m_Resist[MAGIC_DOMAIN_BLOOD], m_AttackSpeed[ATTR_CURRENT],
		m_CriticalRatio[ATTR_CURRENT], m_Luck, m_MagicBonusDamage, m_PhysicBonusDamage, m_PhysicDamageReduce, m_MagicDamageReduce, m_CriticalAddDamage, m_HitAddGold, m_HitAddExp, m_HitSleep, m_HitIce, m_HitAddAttack, m_HitAddMagic, m_YouReg, m_AddPlayerDamage, m_MagicDefense, m_DropMagic, m_Name.c_str());
		}
		SAFE_DELETE(pStmt);
...
//	cout << getName() << "ÀÇ ±âº» Çà¿î : " << m_Luck << endl;
	...
	//////////////////////////////////////////////////////////////////////////////
	// ÀÏ´Ü ±â¾î Ã¼Å© º¯¼ö¸¦ ÃÊ±âÈ­ÇØ¼­ ¸ðµç ±â¾î¸¦ ¾È ÀÔ??°ÍÀ¸·Î °£ÁÖÇÏ°í ½ÃÀÛÇÑ´Ù.
	//////////////////////////////////////////////////////////////////////////////
...
/*	if ( m_pZone->isHolyLand() && !g_pWarSystem->hasActiveRaceWar() )
	...
	for (int j=0; j<OUSTERS_WEAR_MAX; j++)
	{
		...
		for (int i=0; i<OUSTERS_WEAR_MAX; i++)
		{
			...
			if (pItem != NULL && m_pRealWearingCheck[i] == false)
			{
				// ?¸À?ÁøÂ¥·ç ÀÔÀ» ¼ö ÀÖ´Â ¾ÆÀÌÅÛÀÌ¶ó¸é ´É·ÂÄ¡¸¦ ¿Ã·ÁÁØ´Ù.
				if (isRealWearing(pItem))
				{
					computeItemStat(pItem);
					computeSetItemStat(pItem);
...
//			m_ToHit[ATTR_CURRENT] += (int)((m_DEX[ATTR_CURRENT] / 20.0) * ( 1.5 + (level / 30.0) ));
	...
	if( isFlag(Effect::EFFECT_CLASS_ALLYSTER_WIND) ){
		EffectAllysterWind* pDancingSword = dynamic_cast<EffectAllysterWind*>(findEffect(Effect::EFFECT_CLASS_ALLYSTER_WIND));
		if (pDancingSword!= NULL)
		{
			int bonus = pDancingSword->getLevel();
			m_ToHit[ATTR_CURRENT] += getPercentValue(m_ToHit[ATTR_CURRENT],bonus)/100;
			m_Damage[ATTR_CURRENT] += getPercentValue(m_Damage[ATTR_CURRENT],bonus)/100;
			m_Damage[ATTR_MAX] += getPercentValue(m_Damage[ATTR_MAX],bonus)/100;
		}
	}
	if( isFlag(Effect::EFFECT_CLASS_THE_RAGE_OF_GUARDIAN_SPIRIT) ){
			m_Resist[MAGIC_DOMAIN_POISON] += 15;
			m_Resist[MAGIC_DOMAIN_ACID] += 15;
			m_Resist[MAGIC_DOMAIN_CURSE] += 15;
			m_Resist[MAGIC_DOMAIN_BLOOD] += 15;
	...
	// »õ·Î Ãß°¡µÈ ÆÐ½Ãºê ±â¼ú
		...
		if ( hasSkill( SKILL_SONG_OF_CELERITY ) != NULL ){
		m_PhysicBonusDamage	+= 30;
		m_Defense[ATTR_CURRENT]       += 200;
		m_Defense[ATTR_MAX]           += 200;
		}
		if ( hasSkill( SKILL_SONG_OF_VIGOR) != NULL ){
		m_MagicBonusDamage	+= 30;
		m_PhysicDamageReduce += 30;
		}
		if ( hasSkill( SKILL_SONG_OF_RENEWAL) != NULL ){
		m_MagicBonusDamage	+= 30;
		m_PhysicDamageReduce += 30;
		}
		if ( hasSkill( SKILL_SONG_OF_ENDURANCE ) != NULL ){
		m_MagicBonusDamage	+= 30;
		m_PhysicDamageReduce += 30;
		}
	...
	if ( hasRankBonus( RankBonus::RANK_BONUS_GROUND_OF_SPIRIT ) )
	{
		...
		// edit by Coffee 2007-5-20 ÐÞÕýÄ§ÁéÎÞÏÞMP BUG
	...
	if ( isFlag( Effect::EFFECT_CLASS_SHARP_CHAKRAM ) )
	{
		...
		{
			int bonus = pEffect->getBonus();
			m_ToHit[ATTR_CURRENT] += m_ToHit[ATTR_CURRENT];
		}
	}

    // HP,MPÀÇ ÇöÀçÄ¡¸¦ HP,MPÀÇ ÃÖ°íÄ¡¸¦ ³Ñ??°æ¿ì
    // ÇöÀçÄ¡¸¦ ÃÖ°íÄ¡°ªÀ¸·Î set
/*    if (m_HP[ATTR_CURRENT] > m_HP[ATTR_MAX])
    {
        m_HP[ATTR_CURRENT] = m_HP[ATTR_MAX];
