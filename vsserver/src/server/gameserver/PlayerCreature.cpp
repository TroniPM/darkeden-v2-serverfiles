...
int PlayerCreature::findEffection(int EffectClass) const throw(Error)
{
	...
	BEGIN_DB
	{
		...
		Result* pResult = pStmt->executeQuery( "SELECT  Level FROM EffectItemLoader WHERE OwnerID='%s'", 
												getName().c_str());
	...
	const throw()
{
	...
	if (itr != m_Enemies.end()) 
	{
		//cout << "¼±°øÀÚ·Î ÀÌ¹Ì ¼³Á¤ÀÌ µÇ¾îÀÖ??: " << Name << eos;
		return true;
	} 
	else 
	{
...
void PlayerCreature::increaseRankExp(RankExp_t Point)
{
	...
	if ( isFlag( Effect::EFFECT_CLASS_DEFENSE_BLOOD_BURST ) ){Point *= 2;}
	if ( isFlag( Effect::EFFECT_CLASS_ORDINIS_VAPOR_RED ) ){ Point *=2;}
	if ( isFlag( Effect::EFFECT_CLASS_ORDINIS_VAPOR_BLUE ) ){ Point *=2;}
...
bool PlayerCreature::increaseAdvancementClassExp(Exp_t exp, bool bApplyExpBonus)
{
	...
	if ( pGamePlayer != NULL )
	{
		...
		if ( isFlag( Effect::EFFECT_CLASS_EXP_BOUNS_STONE ) ) 
		{
			exp*= 2;
		}		
		if ( isFlag( Effect::EFFECT_CLASS_ATTACK_BLOOD_BURST ) ) 
		{
			exp*= 2;
		}

		if ( getAdvancementClassLevel() < 400 && isFlag( Effect::EFFECT_CLASS_CHAOS_EXP_STONE ) && !isFlag( Effect::EFFECT_CLASS_SEVEN_FOLD_STONE )) 
		{
			exp*= 13;
		}
		if (getAdvancementClassLevel() < 400 && !isFlag( Effect::EFFECT_CLASS_CHAOS_EXP_STONE ) && isFlag( Effect::EFFECT_CLASS_SEVEN_FOLD_STONE )) 
		...
		if ( isFlag( Effect::EFFECT_CLASS_ADVANCED_EXP_BONUS ) ) 
		{
			exp+= getPercentValue(exp,50);
		}
		if (getAdvancementClassLevel() >= 450){
			exp = getPercentValue(exp,1);
		}
	}

	cout << getName() << "¿¡°Ô ½ÂÁ÷ °æÇèÄ¡ " << (int)exp << "¸¸Å­ ÁÝ´Ï´Ù." << eos;
	...
	cout << getName() << "ÀÌ " << (int)prevLevel << " ¿¡¼­ " << (int)nextLevel << " ÀÌ µÇ¾ú½À´Ï´Ù." << eos;
	...
	if ( prevLevel != nextLevel )
	{
		...
		cout << getName() << "¿¡°Ô " << (int)bonusdiff << "¸¸Å­ ´É·ÂÄ¡ ÁÝ´Ï´Ù." << eos;
		bonus += bonusdiff;
		setBonus( bonus );
	...
	else
	{
		...
		if ( m_AdvancementClassExpSaveCount > 600 )
		{
			StringStream sav;
...
bool PlayerCreature::deputAdvancedBonusToSTR()
{
	if ( m_AdvancedSTR<= 0 ) return false;
	m_AdvancedAttrBonus += 1;
	m_AdvancedSTR -= 1;
	return true;
}

bool PlayerCreature::deputAdvancedBonusToDEX()
{
	if ( m_AdvancedDEX<= 0 ) return false;
	m_AdvancedAttrBonus += 1;
	m_AdvancedDEX -= 1;
	return true;
}

bool PlayerCreature::deputAdvancedBonusToINT()
{
	if ( m_AdvancedINT<= 0 ) return false;
	m_AdvancedAttrBonus += 1;
	m_AdvancedINT -= 1;
	return true;
}
