	...
	throw(Error)
{
	...
	g_pEtheRealInfoManager= new EtheRealInfoManager();
	g_pEtheRealInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_ETHEREAL] = g_pEtheRealInfoManager;
	EtheReal::initItemIDRegistry();

	g_pSkillBookInfoManager= new SkillBookInfoManager();
	g_pSkillBookInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_SKILLBOOK] = g_pSkillBookInfoManager;
	SkillBook::initItemIDRegistry();

	g_pStoneAbilityInfoManager= new StoneAbilityInfoManager();
	g_pStoneAbilityInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_STONE_ABILITY] = g_pStoneAbilityInfoManager;
	StoneAbility::initItemIDRegistry();

	g_pNecklaceZapInfoManager= new NecklaceZapInfoManager();
	g_pNecklaceZapInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_NECKLACEZAP] = g_pNecklaceZapInfoManager;
	NecklaceZap::initItemIDRegistry();

	g_pCoatZapInfoManager= new CoatZapInfoManager();
	g_pCoatZapInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_COATZAP] = g_pCoatZapInfoManager;
	CoatZap::initItemIDRegistry();

	g_pBootZapInfoManager= new BootZapInfoManager();
	g_pBootZapInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_BOOTZAP] = g_pBootZapInfoManager;
	BootZap::initItemIDRegistry();

	g_pWeaponZapInfoManager= new WeaponZapInfoManager();
	g_pWeaponZapInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_WEAPONZAP] = g_pWeaponZapInfoManager;
	WeaponZap::initItemIDRegistry();

	g_pHelmZapInfoManager= new HelmZapInfoManager();
	g_pHelmZapInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_HELMZAP] = g_pHelmZapInfoManager;
	HelmZap::initItemIDRegistry();

	g_pBeltZapInfoManager= new BeltZapInfoManager();
	g_pBeltZapInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_BELTZAP] = g_pBeltZapInfoManager;
	BeltZap::initItemIDRegistry();

	g_pGloveZapInfoManager= new GloveZapInfoManager();
	g_pGloveZapInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_GLOVEZAP] = g_pGloveZapInfoManager;
	GloveZap::initItemIDRegistry();

	g_pShoesZapInfoManager= new ShoesZapInfoManager();
	g_pShoesZapInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_SHOESZAP] = g_pShoesZapInfoManager;
	ShoesZap::initItemIDRegistry();

	g_pCommonItemInfoManager= new CommonItemInfoManager();
	g_pCommonItemInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_COMMON_ITEM] = g_pCommonItemInfoManager;
	CommonItem::initItemIDRegistry();

	g_pSlayerTuningItemInfoManager= new SlayerTuningItemInfoManager();
	g_pSlayerTuningItemInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_SLAYER_TUNING_ITEM] = g_pSlayerTuningItemInfoManager;
	SlayerTuningItem::initItemIDRegistry();

	g_pVampireWingItemInfoManager= new VampireWingItemInfoManager();
	g_pVampireWingItemInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_VAMPIREWING_ITEM] = g_pVampireWingItemInfoManager;
	VampireWingItem::initItemIDRegistry();

	g_pOustersWingItemInfoManager= new OustersWingItemInfoManager();
	g_pOustersWingItemInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_OUSTERSWING_ITEM] = g_pOustersWingItemInfoManager;
	OustersWingItem::initItemIDRegistry();

	g_pVampireTuningItemInfoManager= new VampireTuningItemInfoManager();
	g_pVampireTuningItemInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_VAMPIRE_TUNING_ITEM] = g_pVampireTuningItemInfoManager;
	VampireTuningItem::initItemIDRegistry();

	g_pOustersTuningItemInfoManager= new OustersTuningItemInfoManager();
	g_pOustersTuningItemInfoManager->init();
	m_InfoClassManagers[Item::ITEM_CLASS_OUSTERS_TUNING_ITEM] = g_pOustersTuningItemInfoManager;
	OustersTuningItem::initItemIDRegistry();
	...
	throw()
{
	...
	if (IType < m_InfoClassManagers[IClass]->getInfoCount())
	{
		if (!OType.empty())
		{
			try
			{
				//cout << "IM::getOptionInfo(" << (int)OType << ")" << eos;
				//cout << "IM::getOptionInfo(" << getOptionTypeToString(OType).c_str() << ")" << eos;
