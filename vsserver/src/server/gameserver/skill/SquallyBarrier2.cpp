...
// 슬레이어 셀프 핸들러
//////////////////////////////////////////////////////////////////////////////
void SquallyBarrier2::execute(Ousters* pSlayer, OustersSkillSlot* pSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected && !bEffected2)
		{
			...
			// 이팩트 클래스를 만들어 붙인다.
			EffectSquallyBarrier2* pEffect = new EffectSquallyBarrier2(pSlayer);
			pEffect->setDeadline(output.Duration);
			pEffect->setLevel(70);
			pSlayer->addEffect(pEffect);
			pSlayer->setFlag(Effect::EFFECT_CLASS_SQUALLY_BARRIER2);
