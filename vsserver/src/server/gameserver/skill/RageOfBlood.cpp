...
// 슬레이어 셀프 핸들러
//////////////////////////////////////////////////////////////////////////////
void RageOfBlood::execute(Vampire* pSlayer, VampireSkillSlot* pSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
...
//		SkillLevel_t      SkillLevel = pSkillSlot->getExpLevel();
		...
		bool bEffected   = pSlayer->isFlag(Effect::EFFECT_CLASS_RAGE_OF_BLOOD);
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected)
		{
			...
			// 이팩트 클래스를 ?링榕?붙인다.
			EffectRageOfBlood* pEffect = new EffectRageOfBlood(pSlayer);
			pEffect->setDeadline(output.Duration);
//			pEffect->setLevel(SkillLevel);
			pSlayer->addEffect(pEffect);
			pSlayer->setFlag(Effect::EFFECT_CLASS_RAGE_OF_BLOOD);
