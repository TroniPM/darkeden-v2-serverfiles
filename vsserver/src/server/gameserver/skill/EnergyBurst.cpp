...
// 슬레이어 셀프 핸들러
//////////////////////////////////////////////////////////////////////////////
void EnergyBurst::execute(Ousters* pSlayer, OustersSkillSlot* pSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
...
//		SkillLevel_t      SkillLevel = pSkillSlot->getExpLevel();
		...
		bool bEffected   = pSlayer->isFlag(Effect::EFFECT_CLASS_ENERGY_BURST);
		...
		if (bManaCheck && bTimeCheck && !bEffected)
		{
			...
			// 이팩트 클래스를 만들어 붙인다.
			EffectEnergyBurst* pEffect = new EffectEnergyBurst(pSlayer);
			pEffect->setDeadline(output.Duration);
			pSlayer->addEffect(pEffect);
			pSlayer->setFlag(Effect::EFFECT_CLASS_ENERGY_BURST);
