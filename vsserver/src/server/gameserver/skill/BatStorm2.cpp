...
// 뱀파이어 오브젝트 핸들러
//////////////////////////////////////////////////////////////////////////////
void BatStorm2::execute(Vampire* pVampire, ObjectID_t TargetObjectID, VampireSkillSlot* pVampireSkillSlot, CEffectID_t CEffectID)
    throw(Error)
{
    __BEGIN_TRY
...
// 뱀파이어 타일 핸들러
//////////////////////////////////////////////////////////////////////////////
void BatStorm2::execute(Vampire* pVampire, ZoneCoord_t X, ZoneCoord_t Y, VampireSkillSlot* pVampireSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try
	{
		...
		// Knowledge of Bat 가 있다면 hit bonus 10
		int HitBonus = 0;
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bTileCheck)
		{
			...
			Tile& tile = pZone->getTile(X, Y);
			Effect* pOldEffect = tile.getEffect(Effect::EFFECT_CLASS_BAT_STORM);
			if(pOldEffect != NULL)
			{
			...
			EffectBatStorm* pEffect = new EffectBatStorm(pZone, X, Y);
			...
			for(oY = -edge; oY <= edge; oY++)
			{
				...
				if(pTargetCreature != NULL)
				{
					...
					if(pTargetCreature->isSlayer()||pTargetCreature->isOusters())
					{
						...
						{
							//cout << "BatStorm to Slayer Success" << endl;
						...
						{
							//cout << "BatStorm to creature fail" << endl;
						}
					}
					else if(pTargetCreature->isMonster())
					{
						if(pEffect->affectCreature(pTargetCreature, false) == true)
						{
							//cout << "BatStorm to Monster Success" << endl;
...
// 몬스터 타일 핸들러
//////////////////////////////////////////////////////////////////////////////
void BatStorm2::execute(Monster* pMonster, ZoneCoord_t X, ZoneCoord_t Y)
	throw(Error)
{
	__BEGIN_TRY
	...
	try
	{
		...
		SkillType_t SkillType = SKILL_BAT_STORM;
		SkillInfo*  pSkillInfo =  g_pSkillInfoManager->getSkillInfo(SkillType);
