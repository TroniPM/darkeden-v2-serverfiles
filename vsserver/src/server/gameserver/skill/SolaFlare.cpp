...
// 아우스터즈 오브젝트 핸들러
//////////////////////////////////////////////////////////////////////////////
void SolaFlare::execute(Slayer* pSlayer, ObjectID_t TargetObjectID, SkillSlot* pSkillSlot, CEffectID_t CEffectID)
    throw(Error)
{
    __BEGIN_TRY
...
// 아우스터즈 타일 핸들러
//////////////////////////////////////////////////////////////////////////////
void SolaFlare::execute(Slayer* pSlayer, ZoneCoord_t X, ZoneCoord_t Y, SkillSlot* pSkillSlot, CEffectID_t CEffectID)
	throw(Error)
{
	__BEGIN_TRY
	...
	try 
	{
		...
		if (pWeapon == NULL || pWeapon->getItemClass() != Item::ITEM_CLASS_MACE || !pSlayer->isRealWearingEx(Slayer::WEAR_RIGHTHAND))
		{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bTileCheck )
		{
			...
			Effect* pOldEffect = tile.getEffect(Effect::EFFECT_CLASS_SOLA_FLARE);
			if (pOldEffect != NULL)
			{
			...
			// 이펙트 오브젝트를 생성한다.
			EffectSolaFlare* pEffect = new EffectSolaFlare(pZone, X, Y);
