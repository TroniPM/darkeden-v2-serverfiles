	...
	throw(Error)
{
	...
	try 
	{
		...
		bool bEffected   = pTargetSlayer->isFlag(Effect::EFFECT_CLASS_SPIRIT_GUARD_1);
		bool bEffected2   = pTargetSlayer->isFlag(Effect::EFFECT_CLASS_Bramble_Halo);
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected && !bEffected2)
		{
			...
			// 이팩트 클래스를 만들어 붙인다.
			EffectBrambleHalo* pEffect = new EffectBrambleHalo(pTargetSlayer);
			...
			pTargetSlayer->setFlag(Effect::EFFECT_CLASS_Bramble_Halo);
	...
	throw(Error)
{
	...
	try 
	{
		...
		bool bHitRoll    = HitRoll::isSuccessMagic(pSlayer, pSkillInfo, pSkillSlot);
		bool bEffected2   = pSlayer->isFlag(Effect::EFFECT_CLASS_SPIRIT_GUARD_1);
