	...
	throw(Error)
{
	...
	try 
	{
		...
		bool bEffected2 = pTargetCreature->isFlag(Effect::EFFECT_CLASS_INTIMATE_GRAIL2);
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected && !bEffected2)
		{
			...
			// 이펙트를 만들어 붙인다.
			EffectIntimateGrail2* pEffect = new EffectIntimateGrail2(pTargetCreature);
			pEffect->setSkillLevel(input.SkillLevel);
			pEffect->setDeadline(output.Duration);
			pTargetCreature->setFlag(Effect::EFFECT_CLASS_INTIMATE_GRAIL2);
...
/*			SLAYER_RECORD prev;
			...
			gcAddEffect.setEffectID(Effect::EFFECT_CLASS_INTIMATE_GRAIL2);
	...
	throw(Error)
{
	...
	try 
	{
		...
		bool bEffected2   = pSlayer->isFlag(Effect::EFFECT_CLASS_INTIMATE_GRAIL2);
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected && !bEffected2)
		{
			...
			// 이펙트를 만들어 붙인다.
			EffectIntimateGrail2* pEffect = new EffectIntimateGrail2(pSlayer);
			pEffect->setSkillLevel(input.SkillLevel);
			pEffect->setDeadline(output.Duration);
			pSlayer->setFlag(Effect::EFFECT_CLASS_INTIMATE_GRAIL2);
...
IntimateGrail2 g_IntimateGrail2;
