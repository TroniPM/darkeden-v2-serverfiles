	...
	throw(Error)
{
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bCanResurrect)
		{
			...
			// 부활 아르?牡絹躍?방지하기 위해서 Aftermath 이펙트를 붙인다.
			// 2002.11.19 장홍창 
			...
			// 대상의 체력을 10%만 채운다.
			HP_t CurrentHP = getPercentValue(pTargetSlayer->getHP(ATTR_MAX), 100);
...
//			increaseAlignment(pSlayer, pTargetCreature, _GCSkillToObjectOK1);
			...
			_GCSkillToObjectOK2.addShortData(MODIFY_CURRENT_HP, pTargetSlayer->getHP(ATTR_CURRENT));
