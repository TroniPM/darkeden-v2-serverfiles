	...
	throw (InvalidProtocolException, Error)
{
	...
	// 처음 접속한 사람일 경우 초보자용 아이템세트를 일단 준 다음..s
	...
	BEGIN_DB
	{
		...
		*/
		...
		pResult = pStmt->executeQuery(
			...
			ZoneID, XCoord, YCoord, Sight, Alignment,
			StashGold, StashNum, Competence, CompetenceShape, ResurrectZone, SilverDamage, Reward, SMSCharge,
			Rank, RankGoalExp,KillPoint, AdvBatColor, ShopColor, AttackBloodBurstPoint, DefenseBloodBurstPoint, ADBloodBurstPoint FROM Vampire WHERE Name = '%s' AND Active = 'ACTIVE'",
			m_Name.c_str()
		);
		// end by Sonic 
...
//			setHotKey(j, pResult->getInt(++i));
		...
		setAdvanceBatColor( pResult->getInt(++i) );
	...
	throw()
{
	...
	// 옷이라면 옷에 따른 색깔을 정해준다.
	// 나중에라도 옷 타입이 여러 가지가 될 수 있으리???珝▤求쨉?
	...
	if (pItem->getItemClass() == Item::ITEM_CLASS_VAMPIRE_COAT) 
	{
		...
		// item type을 설정해준다. 
		m_VampireInfo.setCoatType( getVampireCoatType(pItem->getItemType()) );
	}
	...
	throw()
{
	...
	// ?프?적용되는 아이템만 복장을 바꾼다. by sigi. 2002.10.30
	...
	throw ()
{
	...
	pInfo->setSex(m_Sex);
	pInfo->SetBatType(m_BatType);
	pInfo->setWingItemType(m_WingItemType);
	pInfo->SetWingColor1(m_WingColor);
	pInfo->SetWingColor2(m_WingEffectColor);
	pInfo->setBatColor(m_BatColor);
	pInfo->setAdvanceBatColor(m_AdvanceBatColor);
	...
	throw ()
{
	...
	*/
	...
	{
		// 신규 이동수단이 0일경우 일반 뱃 또는 승직 전용 뱃을 표현한다.
		//  이동 수단이 0 이상일 경우보다 먼저 출력하기 때문에 출력 순서는 기존 뱃을 선출한다.
		if(m_BatType == 1){
			if(getAdvancementClassLevel() > 0)
			{
				m_VampireInfo.setShape(SHAPE_VAMPIRE_GHOST);
			}else{
				m_VampireInfo.setShape(SHAPE_BAT);	
			}
		}
	
		// 뱃 타입이 0이상일 경우 나오는 부분.
		if(m_BatType == 2){
			m_VampireInfo.setShape(SHAPE_FLITTERMOUSE);
		}
	...
	// 엽색약용
	m_VampireInfo.setBatColor(m_BatColor);
	m_VampireInfo.setWingItemType(m_WingItemType);
	m_VampireInfo.setAdvanceBatColor(m_AdvanceBatColor);
	m_VampireInfo.SetBatType(m_BatType);
	m_VampireInfo.setSkinColor(m_SkinColor);
	m_VampireInfo.setMasterEffectColor(m_MasterEffectColor);
...
// get Inventory Info
//////////////////////////////////////////////////////////////////////////////
InventoryInfo* Vampire::getInventoryInfo2() const
    throw()
{
	__BEGIN_TRY
	__BEGIN_DEBUG
		...
		if (diffTime.tv_sec > 0)
		{
			...
			)
			{
				...
				else
				{
					...
					else if(isFlag(Effect::EFFECT_CLASS_RAGE_OF_BLOOD))
					{
						NewHP = ( 60 + m_HPRegenBonus ) * diffTime.tv_sec;
					}else{
    ...
    throw (Error)
{
	...
	*/
	...
	{
		pStmt = g_pDatabaseManager->getConnection("DARKEDEN")->createStatement();
		pStmt->executeQuery( "UPDATE Vampire SET Alignment=%d, Fame=%d, GoalExp=%lu%s, Rank=%d, RankGoalExp=%lu, AdvancementClass=%u, AdvancementGoalExp=%lu, KillPoint= %d, AttackBloodBurstPoint = %d, DefenseBloodBurstPoint = %d, ADBloodBurstPoint = %d WHERE Name='%s'",
							m_Alignment, m_Fame, m_GoalExp, silverDam, getRank(), getRankGoalExp(), getAdvancementClassLevel(), getAdvancementClassGoalExp(), m_KillPoint, m_AttackBloodBurstPoint, m_DefenseBloodBurstPoint, m_PartyBloodBurstPoint, m_Name.c_str() );
...
//	throw ()
{
	...
	{
		ItemType_t IType = getVampireCoatType(pItem->getItemType());
...
Vampire::addShape(Item::ItemClass IClass, ItemType_t IType, Color_t color)
{
	...
	switch (IClass)
	{
		case Item::ITEM_CLASS_VAMPIRE_COAT:
		{
			...
			m_VampireInfo.setCoatColor( color );
			m_VampireInfo.setCoatType( getVampireCoatType(IType) );
		}
		break;
