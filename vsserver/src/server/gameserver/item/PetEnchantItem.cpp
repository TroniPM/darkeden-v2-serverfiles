	...
	throw()
{
	...
	if (!g_pItemInfoManager->isPossibleItem(getItemClass(), m_ItemType, getOptionTypeList()))
	{
	...
	throw(Error)
{
	...
	BEGIN_DB 
	{
		...
		sql << "INSERT INTO PetEnchantItemObject "
			<< "(ItemID,  ObjectID, ItemType, OwnerID, Storage, StorageID, X, Y, Num, ItemFlag, OptionType) VALUES(" 
			<< m_ItemID << ", "
			<< m_ObjectID << ", " << m_ItemType << ", '" << ownerID << "', " 
			<<(int)storage << ", " << storageID << ", " <<(int)x << ", " <<(int)y << ", " 
			<<(int)m_Num << ", " << (int)m_CreateType << ",'" << optionField.c_str() << "')";
	...
	throw(Error)
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		pStmt->executeQuery( "UPDATE PetEnchantItemObject SET ObjectID=%ld, ItemType=%d, OwnerID='%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, Num=%d, OptionType='%s' WHERE ItemID=%ld",
								m_ObjectID, m_ItemType, ownerID.c_str(), (int)storage, storageID, (int)x, (int)y, (int)m_Num, optionField.c_str(), m_ItemID );
	...
	throw(Error)
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, Num, ItemFlag, OptionType FROM PetEnchantItemObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)",
		...
		while (pResult->next())
		{
			try {
				...
				pPetEnchantItem->setOptionType(optionTypes);
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelts)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pPetEnchantItem))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pPetEnchantItem);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pPetEnchantItem);
										}
