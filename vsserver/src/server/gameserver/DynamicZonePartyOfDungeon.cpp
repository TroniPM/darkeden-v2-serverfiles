...
DynamicZonePartyOfDungeon::DynamicZonePartyOfDungeon()
{
	// 최종 남은 몬스터 수를 위한 부분
	m_CountMonster = 1;
	m_PersentMonster = 1;
...
void DynamicZonePartyOfDungeon::init()
{
	Assert( m_pZone != NULL );
...
void DynamicZonePartyOfDungeon::heartbeat()
{
	switch ( m_OMOAStatus )
	{
	case WAIT_FOR_PC:
...
bool DynamicZonePartyOfDungeon::checkPC()
{
	Assert( m_pZone != NULL );
...
bool DynamicZonePartyOfDungeon::addMono()
{
			cout << " 실행되나.. " << endl;
	Assert( m_pZone != NULL );
...
bool DynamicZonePartyOfDungeon::clearMono()
{
	Assert( m_pZone != NULL );
...
bool DynamicZonePartyOfDungeon::openGateToOut()
{
	Assert( m_pZone != NULL );
...
bool DynamicZonePartyOfDungeon::checkComplete()
{
	Timeval current;
	getCurrentTime( current );
...
void DynamicZonePartyOfDungeon::CountMonster()
{
...
void DynamicZonePartyOfDungeon::addItem()
{
...
void DynamicZonePartyOfDungeon::processEntering()
{
	Assert( m_pZone != NULL );
...
bool DynamicZonePartyOfDungeon::checkMono()
{
	Assert( m_pZone != NULL );
...
void DynamicZonePartyOfDungeon::killPC()
{
	Assert( m_pZone != NULL );
...
DEFINE_DYNAMIC_ZONE_FACTORY( DynamicZonePartyOfDungeon, DYNAMIC_ZONE_PARTY_OF_DUNGEON )
