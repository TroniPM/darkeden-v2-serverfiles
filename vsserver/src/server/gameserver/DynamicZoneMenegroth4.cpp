...
DynamicZoneMenegroth4::DynamicZoneMenegroth4()
{
	m_MonoObjectID = 0;
	m_CabracamHP = 0;
}
...
void DynamicZoneMenegroth4::init()
{
	Assert( m_pZone != NULL );
...
void DynamicZoneMenegroth4::heartbeat()
{
	switch ( m_OMOAStatus )
	{
	case WAIT_FOR_PC:
...
bool DynamicZoneMenegroth4::checkPC()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth4::addMonster()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth4::addMonster_boss_regen()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth4::addMono()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth4::clearMono()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth4::checkMonoHPcount()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth4::checkMonoEffect()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth4::openGateToOut()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth4::openGateToOutNextStage()
{
	Assert( m_pZone != NULL );

	int targetDynamicZoneType = g_pDynamicZoneInfoManager->getDynamicZoneTypeByZoneID( 1705 );
	...
	DynamicZoneMenegroth5* pAlterOfBlood = dynamic_cast<DynamicZoneMenegroth5*>(pDynamicZone);
	...
	m_pZone->getPCManager()->transportAllCreatures( pDynamicZone->getZoneID(), 34, 34 );
        }
    }
...
void DynamicZoneMenegroth4::timecheck()
{
...
bool DynamicZoneMenegroth4::checkComplete()
{
	Timeval current;
	getCurrentTime( current );
...
void DynamicZoneMenegroth4::processEntering()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth4::clearMonster()
{
...
bool DynamicZoneMenegroth4::checkMonster()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth4::checkbossmonster()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth4::checkMono()
{
	Assert( m_pZone != NULL );
...
void DynamicZoneMenegroth4::TreasureCheck()
{
...
void DynamicZoneMenegroth4::killPC()
{
	Assert( m_pZone != NULL );
...
DEFINE_DYNAMIC_ZONE_FACTORY( DynamicZoneMenegroth4, DYNAMIC_ZONE_MENEGROTH4 )
