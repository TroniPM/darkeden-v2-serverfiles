	...
	throw (Error)
{
	...
	do 
	{
		...
		{
			// ¸ó½ºÅÍÄ«¿îÅÍ °´Ã¼¸¦ »ý¼º, hash_map ¿¡ µî·ÏÇÑ´Ù.
		...
		if (!bReload) // reload°¡ ¾Æ´Ï¸é..
		{
			for (uint m = 0 ; m < maxMonsters ; m ++) 
			{
				...
				*/
				...
				{
					//cerr << "MonsterManager::load() : ÀÚ¸®°¡ ¾ø³×?" << eos;
	...
	throw (DuplicatedException , Error)
{
		...
		&& pMonster->isEventMonster())
	{
		...
		//	<< "] = " << info.monsterType << ", delay = " << info.regenDelay << eos;
		return;
	}
	...
	throw ()//NoSuchElementException , Error)
{
	...
	{
		cerr << "MonsterManager::deleteCreature() : NoSuchElementException" << eos;
	...
	throw(Error)
{
	...
	////cout << "MonsterManager::addPotentialEnemy()" << eos;
	...
	for (; itr!=m_Creatures.end(); itr++) 
	{
			...
			&& pMonsterCreature!=pAttackedMonster)
		{
			...
			////cout << "addPotentialEnemy: " << pMonster->getName().c_str() << eos;
	...
	throw(Error)
{
	...
	////cout << "MonsterManager::addEnemy()" << eos;
	...
	for (; itr!=m_Creatures.end(); itr++) 
	{
		...
		{
			Monster* pMonster = dynamic_cast<Monster*>(pMonsterCreature);
			////cout << "addEnemy: " << pMonster->getName().c_str() << eos;
		...
		while (current != m_Creatures.end()) 
		{
			...
			if (pCreature->isAlive()) 
			{
				...
				if (pMonster->isEnemyLimit())
				{
					...
					if (itr == m_Monsters.end()) 
					{
						//cerr << "MonsterManager::processCreatures() : NoSuchElementException" << eos;
						//throw NoSuchElementException("±×·± SpriteTypeÀ» °¡Áø ¸ó½ºÅÍ´Â Á¸ÀçÇÏÁö ¾Ê½À´Ï´Ù.");
					}
					else
					{
	...
	for (; itr != m_Monsters.end() ; itr ++) 
	{
		...
		while (pCounter->getCurrentMonsters() < pCounter->getMaxMonsters()) 
		{
			...
			*/
			...
			{
				//cerr << "MonsterManager::processCreatures() : ÀÚ¸®°¡ ¾ø³×?" << eos;
				SAFE_DELETE(pMonster);
			}
		}
	}
...
/*	if ( monsterType == 722 )
	...
	while (true)
	{
		...
		if (++count >= 300)
		{
			cerr << "MonsterManager::findPosition() : Max Count Exceeded" << eos;
	...
	throw (Error)
{
	...
	// ¸ó½ºÅÍ µå¶ø ·Îµå ÄÚµå¸¦ ºÒ·¯¿Â´Ù '-'
	MonsterDropSetup* pMonsterItemDrop = g_pMonsterDropSetupManager->getMonsterDropSetup(pDeadMonster->getMonsterType());
	if(pMonsterItemDrop != NULL && pMonsterItemDrop->IsInventory()){
	//cout << " ÀÎº¥Åä¸® ¾ÆÀÌÅÛÀ» Áö±Þ " << eos;
	MonsterItemDrop(pDeadMonster);
	}
...
MonsterManager::addMonsters(ZoneCoord_t x, ZoneCoord_t y, MonsterType_t monsterType, int num, const SUMMON_INFO& summonInfo, list<Monster*>* pSummonedMonsters)
{
	...
	for (int i=0; i<num; i++)
	{
		...
		try
		{
			...
			////cout << "clanType=" << (int)pMonster->getClanType() << eos;
	...
	throw (Error)
{
	...
	if (g_pVariableManager->getHarvestFestivalItemRatio() > 0 &&  rand() % g_pVariableManager->getHarvestFestivalItemRatio() == 0) 
	{
		...
		////cout << "ÀÌº¥Æ® ¾ÆÀÌÅÛ »ý¼º"  << "[" << i >> "," << EventSelector << "]" << m_SumOfCakeRatio << eos 
			 //<< "(" << ricecake_template.ItemClass << " " << ricecake_template.ItemType << ")" << eos;
	...
	if ( pTreasureList != NULL )
	{
		...
		for (; itr != treasures.end(); itr++)
		{
			...
			if ( pZone->isPayPlay() 
				|| pZone->isPremiumZone())
			{
				////cout << "»ý¼ºÈ®·ü!!!! : " << g_pVariableManager->getPremiumItemProbePercent() << eos;
	...
	//					//cout << "ºê·ÎµåÄ³½ºÆÃ : " << pDeadMonster->getX() << ", " << pDeadMonster->getY() << eos;
	...
	//				//cout << "·çÆÃ ¾ÆÀÌÅÛ µî±Þ : " << pItem->getGrade() << eos;
	...
	//							//cout << "ºê·ÎµåÄ³?ºÆ?: " << pDeadMonster->getX() << ", " << pDeadMonster->getY() << eos;
	...
	// ¾ÆÀÌÅÛ Å¬·¡½º Å¸ÀÔ ÄÚµå°¡ ÀÖÀ¸¸é ¸ó½ºÅÍ ½ÃÃ¼¿¡ ¾ÆÀÌÅÛÀ» ³Ö¾îÁà¾ß ÇÑ´Ù.
	// ¸ó½ºÅÍ µå¶ø ÆÑÅä¸®¾ó¿¡¼­ ¾Ë¾Æ¼­ ²ø¾î¿ÍÁØ´Ù.
	bool bChiefMonsterBonus = pDeadMonster->isChief() && g_pVariableManager->isActiveChiefMonster();

	MonsterDropSetup* pMonsterItemDrop = g_pMonsterDropSetupManager->getMonsterDropSetup(pMonsterInfo->getMonsterClass());
	...
	if ( pTreasureList != NULL )
	{
		for(int dropsystem = 0; dropsystem < pMonsterItemDrop->getRandomDropNum(); dropsystem++){
		const list<Treasure*>& treasures = pTreasureList->getTreasures();
		list<Treasure*>::const_iterator itr = treasures.begin();
		for (; itr != treasures.end(); itr++)
		{
			...
			// ¾ÆÀÌÅÛ Ãß°¡ º¸³Ê½º µå¶øÀ²
			itemRatioBonus += pMonsterItemDrop->getItemDropBounsRatio();
			...
			if (pTreasure->getRandomItem(&it, g_pVariableManager->getItemProbRatio()+itemRatioBonus))
			{
	...
	//cout << " ¹®Á¦Á¡ " << it.ItemClass << it.ItemType << eos;
		...
		if(pMonsterItemDrop->getItemSetHeroOptionSetup() && pMonsterItemDrop->getItemSetHeroOptionRatio() > rand()%100000){
			...
			pItem->setHeroOption( HeroOption+1 );
			pItem->setHeroOptionAttr( HeroOptionAttrSetting );
			}
		}
			pMonsterCorpse->addTreasure(pItem);
				}
	...
	*/
	...
	//   1/1500 ÀÇ È®·ü·Î º° ¾ÆÀÌÅÛÀ» Ãß°¡·Î »ý¼ºÇÑ´Ù.(°É¸®´Â ?ðÀ?Àç¼ö´Ù)
	//////////////////////////////////////////////////////////////////////
	////cout << "Monster Manager: star -> " << g_pVariable->getStar() << eos;
	...
	if(rand()%500 == 0) {
		...
		//cout << "ÀÌº¥Æ® ¾ÆÀÌÅÛ »ý¼º" << star_template.ItemType << eos;
...
//	//cout << "ratio : " << ratio << eos;
//	//cout << "value : " << value << eos;
...
//	//cout << "before : " << it.ItemClass << "/" << (int)it.ItemType << eos;
	if ( ratio > 0 && value < ratio )
	{
		it.ItemType = getUpgradeItemType(it.ItemClass, it.ItemType, 1);
//		//cout << "after : " << it.ItemClass << "/" << (int)it.ItemType << eos;
		return 1;
	}
...
int MonsterManager::upgradeOptionByLuck(int luckLevel, Creature::CreatureClass ownerCreatureClass, ITEM_TEMPLATE& it) throw (Error)
{
	...
	luckLevel = luckLevel + (rand()%20) - 10;
	luckLevel = min(MAX_LUCK_LEVEL, luckLevel);
//	//cout << "Apply luck to option : " << luckLevel << eos;

	int grade = pOptionInfo->getGrade() + 1;
//	//cout << "Option Grade : " << grade << eos;
...
//	//cout << "before : " << pOptionInfo->getHName() << eos;
...
//		//cout << "after : " << g_pOptionInfoManager->getOptionInfo( it.OptionType.front() )->getHName() << eos;
		return 1;
	}
...
//	MonsterType_t MonsterType = pMonster->getMonsterType();
	MonsterDropSetup* pMonsterItemDrop = g_pMonsterDropSetupManager->getMonsterDropSetup(pMonster->getMonsterType());
	Creature::CreatureClass ownerCreatureClass;
	Item* pItem = NULL;
	TreasureList* pTreasureList = NULL;

	const PCManager* pPCManager = pMonster->getZone()->getPCManager();
	...
	for (i=0, itr=creatures.begin(); itr!=creatures.end(); i++, itr++)
	{
		...
		if (pCreature->isPC())
		{
			...
			if(pPC->getDistance(pMonster->getX(), pMonster->getY()) <= pMonsterItemDrop->getInventoryXY()){
			//------------------------------------------------------------
			// º¸»ó ¾ÆÀÌÅÛÀ» »ý¼ºÇÑ´Ù.
			//------------------------------------------------------------
			// 100% Áö±Þ µÇ´Â ¹æ½ÄÀ¸·Î º¯°æ.			
				ownerCreatureClass = pCreature->getCreatureClass();
				if (ownerCreatureClass == Creature::CREATURE_CLASS_SLAYER)
				{
					pTreasureList = g_pMonsterItemDropManager->getMonsterItemDrop(pMonsterItemDrop->getInventorySetup1())->getSlayerTreasureList();
				}
				else if (ownerCreatureClass == Creature::CREATURE_CLASS_VAMPIRE)
				{
					pTreasureList = g_pMonsterItemDropManager->getMonsterItemDrop(pMonsterItemDrop->getInventorySetup1())->getVampireTreasureList();
				}
				else if (ownerCreatureClass == Creature::CREATURE_CLASS_OUSTERS)
				{
					pTreasureList = g_pMonsterItemDropManager->getMonsterItemDrop(pMonsterItemDrop->getInventorySetup1())->getOustersTreasureList();
				}
				if ( pTreasureList != NULL )
				{
					const list<Treasure*>& treasures = pTreasureList->getTreasures();
					...
					for (; itr != treasures.end(); itr++)
					{
						...
						pTreasure->setRndItemOptionMax(g_pMonsterItemDropManager->getMonsterItemDrop(pMonsterItemDrop->getInventorySetup1())->getOptionCount());

						if (pTreasure->getRandomItem(&it, g_pVariableManager->getItemProbRatio()))
						{
			...
			if(pItem != NULL){
			(pMonster->getZone()->getObjectRegistry()).registerObject(pItem);
			...
			if (pInventory->getEmptySlot(pItem, p))
			{
				...
				if ( pItem != NULL && pItem->isTraceItem() )
				{
					...
					remainTraceLogNew( pItem, pCreature->getName(), ITL_GET, ITLD_EVENTNPC, pMonster->getZone()->getZoneID() );
				}
			...
			else
			{
				...
				TPOINT p = pMonster->getZone()->addItem(pItem, pCreature->getX(), pCreature->getY());
				if (p.x != -1)
				{
					pItem->create("", STORAGE_ZONE, pMonster->getZone()->getZoneID(), p.x, p.y );

					// ItemTrace ¿¡ Log ¸¦ ³²±ä´Ù
					if ( pItem != NULL && pItem->isTraceItem() )
					{
						char zoneName[15];
						sprintf( zoneName , "%4d%3d%3d", pMonster->getZone()->getZoneID(), p.x, p.y);
						remainTraceLog( pItem, "LairMaster", zoneName, ITEM_LOG_CREATE, DETAIL_EVENTNPC);
						remainTraceLogNew( pItem, zoneName, ITL_GET , ITLD_EVENTNPC, pMonster->getZone()->getZoneID(), p.x, p.y);
					}
				}
				else
				{
