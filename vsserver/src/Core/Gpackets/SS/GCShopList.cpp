	 ...
	 throw (ProtocolException , Error)
{
	...
	for (i=0; i<nTotal; i++)
	{
		...
		for (int j2=0; j2<optionSize2; j2++)
		{
			OptionType_t optionType2;
			iStream.read(optionType2);
			item.optionType.push_back( optionType2 );
		}
     ...
     throw (ProtocolException , Error)
{
	...
	for (i=0; i<SHOP_RACK_INDEX_MAX; i++)
	{
		...
		if (item.bExist)
		{
			...
			for (itr2=item.optionType2.begin(); itr2!=item.optionType2.end(); itr2++)
			{
...
{ 
	...
	for (int i=0; i<SHOP_RACK_INDEX_MAX; i++)
	{
		if (m_pBuffer[i].bExist) 
		{
			...
			size += szBYTE + m_pBuffer[i].optionType2.size();   // item option type
	...
	for (int i=0; i<SHOP_RACK_INDEX_MAX; i++)
	{
		...
		if (m_pBuffer[i].bExist)
		{
			msg << "ObjectID:"     << (int)(m_pBuffer[i].objectID)
			    ...
			    << "OptionTypeSize2:"   << (int)(m_pBuffer[i].optionType2.size())
				...
				<< "HeroOption:" << (int)(m_pBuffer[i].heroOption);
			msg << ")";
		}
	}
	...
	throw()
{
	...
	m_pBuffer[index].heroOption = pItem->getHeroOption();
}
