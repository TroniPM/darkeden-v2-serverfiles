...
//		pPacketIDSet->addPacketID( Packet::PACKET_CL_REGISTER_PLAYER );
		...
		pPacketIDSet->addPacketID( Packet::PACKET_CG_LOADING_READY );
	...
	throw ( NoSuchElementException , IgnorePacketException , Error )
{
	...
	bool ok = m_PacketIDSets[playerStatus]->hasPacketID(packetID);
	cerr << "DBG isValidPacketID: status=" << (int)playerStatus << " packetID=" << packetID << " -> " << (ok ? "OK" : "REJEITADO") << " set=" << m_PacketIDSets[playerStatus]->toString() << endl;
	return ok;
