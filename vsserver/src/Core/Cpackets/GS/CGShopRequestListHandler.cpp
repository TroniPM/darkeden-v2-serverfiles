	...
	*/
	...
	if (type == SHOP_RACK_SPECIAL)
	{
		...
		for (BYTE i=0; i<SHOP_RACK_INDEX_MAX; i++) 
		{
			...
			if (pItem != NULL) pItem->setHeroOption(pItem->getHeroOption());
			if (pItem != NULL) pItem->setEnchantLevel(pItem->getEnchantLevel());
			if (pItem != NULL) pItem->setHeroOptionAttr(pItem->getHeroOptionAttr());
