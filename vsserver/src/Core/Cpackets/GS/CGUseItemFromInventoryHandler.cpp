...
//	cout << "CGUseItemFromInventoryHandler " << eos;
	...
	//cout << "ÆÐÅ¶³¯¶ó¿È : " << pPacket->toString() << eos;
	...
	{
		//cout << "¾ÆÅÛ »ç¿ë ºÒ°¡. ¿ÉÁ§Æ® ¾Æµð°¡ ¾È ¸Â´ø°¡..." << eos;
		GCCannotUse _GCCannotUse;
		_GCCannotUse.setObjectID(pPacket->getObjectID());
		pGamePlayer->sendPacket(&_GCCannotUse);
		return;
	...
	throw (ProtocolException, Error)
{
	...
	if (bSuccess && !pSlayer->isFlag(Effect::EFFECT_CLASS_RELOAD_TIMER)) 
	{
		...
		if (pSlayer->hasSkill(SKILL_FAST_RELOAD)) pEffect->setDeadline(7); // ºü¸¥ reload(0.7??
	...
	throw (ProtocolException, Error)
{
	...
	// ÄÚ¸¶ »óÅÂ¶ó¸é »ç¿ëÇÒ ¼ö ¾ø´Ù.
	// ?ÆÀÌÅÛÀ?¿­¼è ¾ÆÀÌÅÛÀÌ ¾Æ´Ï¶ó¸é, ¿ÀÅä¹ÙÀÌ¸¦ È£ÃâÇÒ ¼ö ¾ø´Ù.
...
/*		// (!) MotorcycleObject¸¦ »ý¼ºÇÏ°í MotorcycleItemID==Target¸¦ ¹Þ¾Æ¾ß ÇÑ´Ù.
	...
	{
		////cout << "±âÁ¸¿¡ ºÒ·ÁÁø ¿ÀÅä¹ÙÀÌ°¡ ÀÖ½À´Ï´Ù" << eos;
	...
	BEGIN_DB
	{
		...
		// ¿ÀÅä¹ÙÀÌ¸¦ Á¸¿¡ Ãß°¡ÇÑ´Ù.
		////cout << "¿ÀÅä¹ÙÀÌ¸¦ Á¸¿¡ Ãß°¡ÇÕ´Ï´Ù" << pSlayer->getX() << " " << pSlayer->getY() << eos;
...
void OustersStatus(PlayerCreature* pPC, int Case, CGUseItemFromInventory* pPacket){
		...
		// ÄÉÀÌ½º¿¡ µû¶ó ´É·ÂÄ¡º°·Î ³ª´µ¾î ÁØ´Ù.
		...
		if(Ability >= 30){
			...
			}else if(Ability >= 20 && Ability < 30){ // Ä³¸¯ÅÍÀÇ ½ºÅÝÀÌ 20 ¹Ì¸¸ÀÏ°æ¿ì.
...
void VampireStatus(PlayerCreature* pPC, int Case, CGUseItemFromInventory* pPacket){
		...
		if(Ability >= 30){
			...
			}else if(Ability >= 21 && Ability < 30){ // Ä³¸¯ÅÍÀÇ ½ºÅÝÀÌ 20 ¹Ì¸¸ÀÏ°æ¿ì.
...
void CGUseItemFromInventoryHandler::executeResurrectItem(CGUseItemFromInventory* pPacket, Player* pPlayer) throw(ProtocolException, Error)
{
	...
	//cout << "Resurrection ¾ÆÀÌÅÛÀ» »ç¿ëÇÔ : " << pPC->getName() << " : " << pItem->getItemType() << eos;

	if ( pItem->getObjectID() != pPacket->getObjectID() ||
		pItemInfo == NULL )
	{
		//cout << "¾ÆÅÛ ¿ÀºêÁ§Æ® ¾Æµð°¡ Æ²¸²" << eos;
		sendCannotUse( pPacket, pPlayer );
		return;
	}
...
void CGUseItemFromInventoryHandler::executeEffectItem(CGUseItemFromInventory* pPacket, Player* pPlayer) throw(ProtocolException, Error)
{
	...
	{
		// ½×ÀÌÁö ¾Ê´Â ¾Æ?ÌÅÛÀ?¹Ù·Î¹Ù·Î »èÁ¦ÇØÁØ´Ù.
		pInventory->deleteItem(InvenX, InvenY);
		pItem->destroy();
		SAFE_DELETE(pItem);
	}
...
//	Zone*			pZone		 = pPC->getZone();
	...
	if ( pPetItem != NULL )
	{
		...
		{
//			cout << pPC->getName() << " Á×Àº Æê ºÎ¸£Áö ¸¶¼À" << eos;
			sendCannotUse( pPacket, pPlayer );
			return;
		}
		...
		if ( pPetInfo == NULL || pPetInfo->getPetItem() != pPetItem )
		{
			...
			//cout << pPetItem->getObjectID() << " ¾ÆÀÌÅÛÀÇ ÆêÀ» ºÒ·¶½À´Ï´Ù." << eos;
		}
		else
		{
			//cout << "ÆêÀ» Áö¿ü½À´Ï´Ù." << eos;
			pPC->setPetInfo( NULL );
		}
	...
	throw (ProtocolException, Error)
{
	...
	Item*           pItem        = pInventory->getItem(InvenX, InvenY);
	//ObjectID_t      ItemObjectID = pItem->getObjectID();
	cout << "Check Name : " << pCreature->getName() << " , GiftBoxType : " << pItem->getItemType() << eos;
...
*/
	// °ËÀº»óÀÚ°¡ º¯?Ï´?¾ÆÀÌÅÛÀÌ 2*2 ¿¡ ´Ù µé¾î°¡´Â Å©±âÀÌ¹Ç·Î Inventory °Ë»ç´Â »ý·«
	// (¸¸¾à °ËÀº »óÀÚ Áö¿ì°í ³ª¼­ ¾È µé¾î°¡´Â »óÈ² ¹ß»ýÇÏ¸é °ï¶õÇÏ¹Ç??
	...
	throw(ProtocolException, Error)
{
	...
	cout << "LIST : " << ItemCount2 << ", SIZE :  " << (int)pSubInventoryInven->getHeight() <<  (int)pSubInventoryInven->getWidth() << eos;
