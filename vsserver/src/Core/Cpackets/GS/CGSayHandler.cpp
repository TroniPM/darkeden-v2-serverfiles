	...
	throw(ProtocolException, Error)
{
	...
	// halt
	...
	else if (pGamePlayer->getSpecialEventCount() == 1 && msg.substr(i+1 , 6) == "auraon"){

		PlayerCreature* pPC          = dynamic_cast<PlayerCreature*>(pCreature);
		Zone*           pZone        = pPC->getZone();

		if(!pPC->isFlag( Effect::EFFECT_CLASS_LOGIN_EVENT )){

		VSDateTime current = VSDateTime::currentDateTime().addSecs(86400);
		...
		pEffect->setLevel( (Effect::EffectClass)Effect::EFFECT_CLASS_LOGIN_EVENT );
		pEffect->setEffectType( (Effect::EffectClass)Effect::EFFECT_CLASS_LOGIN_EVENT);
		pEffect->setHeroOption( 0 );
		pPC->addEffect( pEffect );
		pEffect->create(pPC->getName());
		pPC->setFlag( Effect::EFFECT_CLASS_LOGIN_EVENT );

		GCAddEffect gcAddEffect;
		gcAddEffect.setObjectID( pPC->getObjectID() );
		gcAddEffect.setEffectID( Effect::EFFECT_CLASS_LOGIN_EVENT );
		gcAddEffect.setDuration( (DWORD)diffSecs*10 );
		pZone->broadcastPacket( pPC->getX(), pPC->getY(), &gcAddEffect );

		}
	}
	else if (pGamePlayer->getSpecialEventCount() == 1 && msg.substr(i+1 , 7) == "auraoff"){
		...
		EffectSevenFoldStone* pEffect = dynamic_cast<EffectSevenFoldStone*>(pPC->findEffect(Effect::EFFECT_CLASS_LOGIN_EVENT));
		pEffect->setDeadline( 0 );
		pPC->removeFlag( Effect::EFFECT_CLASS_LOGIN_EVENT );
	...
	*/
	...
	// 2002.5.8 º° È®·ü??ÀÚÀ²Àû?¸·?Á¶ÀýÇÏ±â À§ÇØ¼­ ¸¸µë
...
//			message << "ÀüÀïÀÌ ½ÃÀÛµÇ¾ú½À´Ï´Ù";
	...
	else if(set_type == "end")
	{
		...
		{
			cout << "ÀüÀïÁßÀÌ ¾Æ´Ï°Å³ª Á¾·áÇÒ ¼ö ¾ø½À´Ï´Ù." << eos;
	...
	throw(ProtocolException, Error)
{
	...
	else if(set_type == "evnet_activate")
	{
		if(g_pVariableManager->getEventActivate() == 1)
			message << "ÊÂ¼þÕýÔÚ½øÐÐÖÐ..." << eos;
		else
			message << "ÊÂ¼þÒÑÍ£Ö¹" << eos;
		gcSystemMessage.setMessage(message.toString());
	}
...
//		msg << "RaceWarPCLimit Á¤º¸¸¦ loadÇÕ´Ï´Ù.";
	...
	else if ( load_type == "itemdrop" )
	{
		pEvent = new EventReloadInfo( pGamePlayer, EventReloadInfo::ITEM_DROP_INFO );

		gcSystemMessage.setMessage( "¸ó½ºÅÍ ¾ÆÀÌÅÛ µå¶ø Á¤º¸¸¦ Àç·Îµå ÇÕ´Ï´Ù." );
	}
	else if ( load_type == "indun" || load_type == "INDUN" )
	{
		pEvent = new EventReloadInfo( pGamePlayer, EventReloadInfo::INSTANT_DUNGEON );

		gcSystemMessage.setMessage( "ÀÎ´ø Á¤º¸¸¦ ¸®·Îµå ÇÏ°í ÀÖ½À´Ï´Ù." );
	}
	...
	throw(ProtocolException, Error)
{
	...
	uint lSet = msg.find_first_of('<' , k+1);	// num
	uint rSet = msg.find_first_of('>' , k+1);
...
//		|| ItemClass == Item::ITEM_CLASS_KEY
	...
	list<OptionType_t> SetoptionTypes;
	...
	uint pos2 = lSet, previous2 = lSet;
	while (previous2 < string::npos && pos2 < string::npos)
	{
		...
		OptionInfo* pOptionInfo2 = NULL;

		//try {
			pOptionInfo2 = g_pOptionInfoManager->getOptionInfo( optionString2 );

			if (pOptionInfo2==NULL)
			{
				pOptionInfo2 = g_pOptionInfoManager->getOptionInfo( atoi(optionString2.c_str()) );
		...
		if (pOptionInfo2!=NULL)
		{
			OptionType2 = pOptionInfo2->getType();
	...
	*/
	...
	pItem->setOptionType2( SetoptionTypes );
	pItem->setNum(1);
	...
	if ( ItemClass == Item::ITEM_CLASS_PET_ITEM )
	{
		...
		pPetInfo->setEnchantSkillType( 0 );
...
/*	else if ( pCreature->isVampire() )
	{
		...
		{
			//cout << "¾ÆÀÌÅÛ Ã¢Á¶¿¡ ½ÇÆÐÇÏ¿´½À´Ï´Ù" << eos;
			SAFE_DELETE(pItem);
		}
	}
	...
	throw(ProtocolException, Error)
{
	...
	{
		//	?Ìº¥Æ??ÄÚµå (¸ó½ºÅÍÅ¸ÀÔµµ ±×³É ÇÏµåÄÚµù -_-;)
		Zone* pZone = pCreature->getZone();
		Coord_t ZoneX = pCreature->getX();
		Coord_t ZoneY = pCreature->getY();
...
//	cout << MonsterName << eos;
	...
	// SpriteTypeÀÌ ¾Æ´Ï°í MonsterTypeÀÌ ¼³Á¤µÈ ???
	...
	throw(ProtocolException, Error)
{
	...
	else if (command=="logout")
	{
		...
		bool bInvincible = pCreature->isFlag(Effect::EFFECT_CLASS_BLOOD_DRAIN2);
		...
		if (value1=="on")
		{
			...
			{
				pCreature->setFlag(Effect::EFFECT_CLASS_BLOOD_DRAIN2);
				gcSystemMessage.setMessage( msg );
			}
			filelog("change.txt", "[%s]%s", (pGamePlayer==NULL?"Nobody":pGamePlayer->getCreature()->getName().c_str()), gcSystemMessage.toString().c_str());
		}
		else if (value1=="off")
		{
			...
			{
				pCreature->removeFlag(Effect::EFFECT_CLASS_BLOOD_DRAIN2);
				gcSystemMessage.setMessage( msg );
			}
