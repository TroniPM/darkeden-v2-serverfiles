	...
	try 
	{
		...
		//f(pItem->getSilver() == pPrevItem->getSilver()){
			...
			/*lse
			{
				...
				if (!pInventory->getEmptySlot(pItem, pt2))
				{
					throw Error("인벤토리에 자리없다.");
				}
				...
				Merge = true;
				_TPOINT pt2;
				CoordInven_t X = pt2.x;
				CoordInven_t Y = pt2.y;

				Item* pNewItem = g_pItemFactoryManager->createItem( pItem->getItemClass(), pItem->getItemType(), list<OptionType_t>() );
				pNewItem->setNum(pItem->getNum());
				pNewItem->setSilver(pItem->getSilver());
				if(pItem->getHeroOption() != -1){
				pNewItem->setHeroOption(pItem->getHeroOption());
				}
				pItem->destroy();
				pPC->getZone()->getObjectRegistry().registerObject( pNewItem );
				pInventory->addItem( X, Y, pNewItem);
				pNewItem->create( pPC->getName(), STORAGE_INVENTORY, 0, X, Y );
				
				GCCreateItem gcCreateItem;
				makeGCCreateItem( &gcCreateItem, pNewItem, X, Y );
...
//			pZone->broadcastPacket(ZoneX , ZoneY, &_GCDeleteObject , pPC);
		...
		/*if(Reload){
				// 아이템을 리로드 한다.
			for (int x=0; x<pInventory->getWidth(); x++)
			{
				for (int y=0; y<pInventory->getHeight(); y++)
				{
						InventorySlot& slot  = pInventory->getInventorySlot(x, y);
						Item*          pItem2 = slot.getItem();

						if (pItem2 != NULL && pItem2->getItemClass() == Item::ITEM_CLASS_EFFECT_ITEM || pItem2->getItemClass() == Item::ITEM_CLASS_EVENT_STAR && pItem2->getObjectID() != pItem->getObjectID()){

							GCDeleteInventoryItem gcDeleteInventoryItem;
							gcDeleteInventoryItem.setObjectID(pItem2->getObjectID());
