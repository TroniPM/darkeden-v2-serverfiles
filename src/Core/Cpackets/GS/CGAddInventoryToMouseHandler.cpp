	...
	
{
	...
	else
	{
		...
		pNewItem->setEnchantLevel(pItem->getEnchantLevel()); 
		pNewItem->setHeroOption(pItem->getHeroOption()); 
		pNewItem->setHeroOptionAttr(pItem->getHeroOptionAttr()); 
		pItem->setNum(1);
		...
		if(!SubInventoryTrue)
		pNewItem->create(pPC->getName(), STORAGE_INVENTORY, StorageID, InvenX, InvenY);
		else
		pNewItem->create(pPC->getName(), STORAGE_BOX, StorageID, InvenX, InvenY);

		//pNewItem->setNum(NewNum); // 위에서 했는데 또 하네. -_-;
		//pNewItem->save(pPC->getName(), STORAGE_INVENTORY, 0, InvenX, InvenY);
		// item저장 최적화. by sigi. 2002.5.13
		sprintf(pField, "Num=%d, Storage=%d, StorageID=%d", NewNum, Storage, StorageID);
