	...
	throw (ProtocolException, Error)
{
	...
	try 
	{
		...
		if (pPrevItem != NULL) 
		{
			...
			bool bisSame = true;
			// 아이템 클래스가 같을때 숫자를 올려 주고 마우스에 있는 ?痼?없앤다.
			if (canStack(pItem, pPrevItem))
			{
				...
				if( bisSame ) {
					...
					if (pItem->getNum() + pPrevItem->getNum() > MaxStack) 
					{
						...
						char pField[80];
						sprintf(pField, "Num=%d, Storage=%d, StorageID=%d, X=%d, Y=%d", MaxStack, Storage, StorageID, InvenX, InvenY);
					...
					else 
					{
						...
						//pPrevItem->save(pPC->getName(), Storage, 0, InvenX, InvenY);
						// item저장 최적화. by sigi. 2002.5.13
						char pField[80];
						sprintf(pField, "Num=%d, Storage=%d, StorageID=%d, X=%d, Y=%d", pPrevItem->getNum(), Storage, StorageID, InvenX, InvenY);
						pPrevItem->tinysave(pField);
							...
							&& pPrevItem->getNum() == 50 )
					{
						...
						if(!SubInventoryTrue)
						pNewItem->create( pPC->getName(), STORAGE_INVENTORY, StorageID, pt.x, pt.y );
						else
						pNewItem->create( pPC->getName(), STORAGE_BOX, StorageID, pt.x, pt.y );
					...
					//pItem->save(pPC->getName(), Storage, 0, InvenX, InvenY);
					// item저장 최적화. by sigi. 2002.5.13
					sprintf(pField, "Storage=%d, StorageID=%d, X=%d, Y=%d", Storage, StorageID, InvenX, InvenY);
					pItem->tinysave(pField);
		...
		if (Success) 
		{
			...
			// 인벤토리에 골동품을 놓을경우 
			if(pItem != NULL && pItem->getItemClass() == Item::ITEM_CLASS_STONE_ABILITY){
			...
			if ( pItem != NULL && pItem->getItemClass() == Item::ITEM_CLASS_EVENT_TREE)
			{
				...
				if ( itemtype <= 11 )
				{
					...
					if ( pt.x != -1 && pt.y != -1 )
					{
						...
						if(!SubInventoryTrue)
						pTreeItem->create( pPC->getName(), STORAGE_INVENTORY, StorageID, pt.x, pt.y );
						else
						pTreeItem->create( pPC->getName(), STORAGE_BOX, StorageID, pt.x, pt.y );
