	 ...
	 throw (Error)
{
	...
	try 
	{
		...
		if (pCreature->isSlayer()) 
		{
			...
			if ( SkillType == SKILL_TANNING ) pSkillSlot = ((Slayer *)pCreature)->hasSkill(SKILL_SOUL_CHAIN);
			if (pSkillSlot == NULL && SkillType != SKILL_TANNING) bSuccess = false;
		...
		else if (pCreature->isVampire()) 
		{
			...
			// BITE OF DEATH 워울프 체크는 isAbleToUseObjectSkill ?효【?한다.
			if (pVampireSkillSlot == NULL && SkillType != SKILL_BITE_OF_DEATH && SkillType != SKILL_TANNING) bSuccess = false;
			if (!isAbleToUseObjectSkill(pVampire)) bSuccess = false;
			if (SkillType == SKILL_TANNING) bSuccess =true;
