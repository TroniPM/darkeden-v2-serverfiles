...

{
	...
	string msg1 = pPacket->getMessage();
	uint i = msg1.find_first_of('>' , 0);
	StringStream msg;
	msg << pCreature->getName() << " " << msg1.substr(i+1, 52);

	GCGlobalChat gcGlobalChat;
	gcGlobalChat.setMessage(msg.toString());
	gcGlobalChat.setRace(pCreature->getRace());
	gcGlobalChat.setColor( color );
	g_pZoneGroupManager->broadcast( &gcGlobalChat);
	GCUseOK gcUseOK;
	pGamePlayer->sendPacket( &gcUseOK );
