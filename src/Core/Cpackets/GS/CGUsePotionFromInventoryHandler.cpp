	...
	
{
	...
	if (pCreature->isSlayer())
	{
		...
		if (pSlayer->isFlag(Effect::EFFECT_CLASS_ACTIVATION) || pSlayer->isFlag(Effect::EFFECT_CLASS_HYPERACTIVE_INCENSE))
		{
	...
	else if (pCreature->isVampire())
	{
		...
		// Activation Effect가 걸려있다면 회복속도가 2배가 된다.
		if (pVampire->isFlag(Effect::EFFECT_CLASS_HYPERACTIVE_INCENSE))
		{
				RegenPeriod = (RegenPeriod>>1);

				RegenPeriod = max(RegenPeriod, 1);
		}
