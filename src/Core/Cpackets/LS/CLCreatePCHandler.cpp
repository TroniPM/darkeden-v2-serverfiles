...
/*			if ( nSTR < 10 ) bInvalidAttr = true;
		...
		if ( pPacket->getDomainType() == 1){ bladelevel = 150;  nSTR = 315; nDEX  = 80; nINT = 80;}
		if ( pPacket->getDomainType() == 2){ swordlevel = 150; nSTR = 315; nDEX  = 80; nINT = 80;}
		if ( pPacket->getDomainType() == 4){ gunlevel = 150; nSTR = 80; nDEX  = 315; nINT = 80;}
		if ( pPacket->getDomainType() == 3){ enchantlevel = 150; nSTR = 80; nDEX  = 80; nINT = 315; }
		if ( pPacket->getDomainType() == 5){ heallevel = 150; nSTR = 80; nDEX  = 80; nINT = 315;}
		...
		*/
		...
		pStmt->executeQuery(
				"INSERT INTO Slayer (Race, Name, PlayerID, Slot, ServerGroupID, Active, AdvancementClass, AdvancementGoalExp, Sex, HairStyle, HairColor, SkinColor, Phone, STR, STRExp, STRGoalExp, DEX, DEXExp, DEXGoalExp, INTE, INTExp, INTGoalExp, Bonus, Rank, RankExp, RankGoalExp, HP, CurrentHP, MP, CurrentMP, ZoneID, XCoord, YCoord, Sight, Gold, BladeLevel, SwordLevel, GunLevel, EnchantLevel, HealLevel, Alignment, Shape, HelmetColor, JacketColor, PantsColor, WeaponColor, ShieldColor, creation_date, KillPoint) VALUES ('%s', '%s', '%s', '%s', %d, 'ACTIVE', 1, 1512381, '%s', '%s', %d, %d, 0, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, 64, 27, 221, 13, 0, %d, %d, %d, %d, %d, 7500, %d, %d, %d, %d, %d, %d, now(), 500)",
				race.c_str(),
				pPacket->getName().c_str(), 
				pLoginPlayer->getID().c_str(), 
				Slot2String[pPacket->getSlot()].c_str(), 
				...
				(int)pPacket->getHairColor(), 
				(int)pPacket->getSkinColor(),
				(int)nSTR, nSTRExp, nSTRGoalExp, 
				(int)nDEX, nDEXExp, nDEXGoalExp, 
				(int)nINT, nINTExp, nINTGoalExp,
				4, 1, 0, RankGoalExpSlayer,
				(int)pPacket->getSTR()*2,
				(int)pPacket->getSTR()*2,
				(int)pPacket->getINT()*2,
				(int)pPacket->getINT()*2,
		...
		{
			pStmt->executeQuery(
					"INSERT INTO Vampire ( Name, PlayerID, Slot, ServerGroupID, Active, AdvancementClass, AdvancementGoalExp, Sex, SkinColor, STR, DEX, INTE, HP, CurrentHP, Level, Bonus, ZoneID, XCoord, YCoord, Sight, Alignment, Exp, GoalExp, Rank, RankExp, RankGoalExp, Shape, CoatColor,KillPoint) VALUES ( '%s', '%s', '%s', %d, 'ACTIVE', 1, 1512381, '%s', %d, 20, 20, 20, 50, 50,  150, 451, 61, 101, 221, 13, 7500, 0, %d, 1, 0, %d, %d, 377,500 )",
					pPacket->getName().c_str(),
					pLoginPlayer->getID().c_str(),
					Slot2String[pPacket->getSlot()].c_str(),
					(int)CurrentServerGroupID,
		...
		{
			pStmt->executeQuery(
					"INSERT INTO Ousters ( Name, PlayerID, Slot, ServerGroupID, Active, AdvancementClass, AdvancementGoalExp, Sex, STR, DEX, INTE, BONUS, SkillBonus, HP, CurrentHP, MP, CurrentMP, ZoneID, XCoord, YCoord, Sight, Alignment, Exp, GoalExp, Level, Rank, RankExp, RankGoalExp, CoatColor, HairColor, ArmColor, BootsColor,KillPoint ) Values ( '%s', '%s', '%s', %d, 'ACTIVE', 1, 1512381, 'FEMALE', %d, %d, %d, 451, 1199, 50, 50, 50, 50, 62, 34, 35, 13, 7500, 0, %d, 150, 1, 0,	%d, 377, %d, 377, 377,500 )",
					pPacket->getName().c_str(),
					pLoginPlayer->getID().c_str(),
					Slot2String[pPacket->getSlot()].c_str(),
					(int)CurrentServerGroupID,
				...
				pStmt->executeQuery("INSERT INTO GoodsListObject (World, PlayerID, Name, GoodsID, Num, Status) Values (1, '%s', '%s', 913, 1, 'NOT')", pLoginPlayer->getID().c_str(), pPacket->getName().c_str()); 
				pStmt->executeQuery("INSERT INTO GoodsListObject (World, PlayerID, Name, GoodsID, Num, Status) Values (1, '%s', '%s', 914, 1, 'NOT')", pLoginPlayer->getID().c_str(), pPacket->getName().c_str()); 
		...
		if (pPacket->getRace()==RACE_SLAYER){
			...
			pStmt->executeQuery("INSERT IGNORE INTO FlagSet (OwnerID, FlagData) VALUES ('%s','00000000001')", pPacket->getName().c_str());
				...
				pStmt->executeQuery("INSERT INTO CharterStatus ( OwnerID ) Values ( '%s' )", pPacket->getName().c_str());
