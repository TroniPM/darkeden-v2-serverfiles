...
// 
// ·Î±×ÀÎ ¼­¹ö´Â ÀÌ ÆÐÅ¶À» ¹ÞÀ¸¸é, ÁöÁ¤µÈ Name + PCType ¸¦ °¡Áø PC ¸¦ ·ÎµùÇÑ´Ù.?// ÀÌ PC°¡ ÇÃ·¹ÀÌ¾îÀÇ Ä³¸¯ÅÍ¶ó¸é ÀÌ PC°¡ ±âÁ¸¿¡ ÀÖ´ø Á¸À» ¾Ë¾Æ³»¼­
...
//add by viva
		cout<<"LocalHost by viva:"<<pLoginPlayer->getSocket()->getLocalHost()<<endl;
		cout<<"TelnetHost by viva:"<<pLoginPlayer->getSocket()->getHost()<<endl;

		bool IsProxy = false;
		pResult = pStmt->executeQuery("SELECT ProxyIP FROM ProxyIP");
		while(pResult->next())
		{
			if(pLoginPlayer->getSocket()->getHost() == pResult->getString(1))
			{
				cout<<"This is ProxyIP by viva:"<<pResult->getString(1)<<endl;
				IsProxy = true;
			}
		}
		if(IsProxy)
			g_pGameServerManager->sendPacket(pLoginPlayer->getSocket()->getHost() , pGameServerInfo->getUDPPort(), &lgIncomingConnection);
		else
			g_pGameServerManager->sendPacket(pLoginPlayer->getSocket()->getLocalHost() , pGameServerInfo->getUDPPort(), &lgIncomingConnection);
//by viva for Notice                       
