...
/*			if (pSlayer->isFlag(Effect::EFFECT_CLASS_SNIPING_MODE))
			...
			if (bSuccess)
			{
				...
				Assert(pSkillHandler != NULL);
   			 	pSkillHandler->execute(pSlayer, pPacket->getInventoryItemObjectID(), X, Y, TX, TY, pSkillSlot);
   			 }
             else
             {
                 GCSkillFailed1 _GCSkillFailed1;
...
/*			if (pVampire->isFlag(Effect::EFFECT_CLASS_INVISIBILITY))
			...
			if (bSuccess) 
			{
				SkillHandler* pSkillHandler = g_pSkillHandlerManager->getSkillHandler(SkillType);
				Assert(pSkillHandler != NULL);
   			 	pSkillHandler->execute(pVampire, pPacket->getInventoryItemObjectID(), X, Y, TX, TY, pVampireSkillSlot);
   			 }
             else
             {
                 GCSkillFailed1 _GCSkillFailed1;
		...
		else if (pCreature->isOusters()) 
		{
			...
			if (bSuccess) 
			{
				SkillHandler* pSkillHandler = g_pSkillHandlerManager->getSkillHandler(SkillType);
				Assert(pSkillHandler != NULL);
   			 	pSkillHandler->execute(pOusters, pPacket->getInventoryItemObjectID(), X, Y, TX, TY, pOustersSkillSlot);
   			 }
             else
             {
                 GCSkillFailed1 _GCSkillFailed1;
