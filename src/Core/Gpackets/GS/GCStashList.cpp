	 ...
	 throw ( ProtocolException , Error )
{
	...
	for (i=0; i<nTotal; i++)
	{
		...
		for (int j=0; j<optionSize2; j++)
		{
			...
			item.optionType2.push_back( optionType2 );
		}
		...
		iStream.read(item.HeroOption );
		iStream.read(item.HeroOptionAttr );
	...
	for (r=0; r<STASH_RACK_MAX; r++)
	{
		for (i=0; i<STASH_INDEX_MAX; i++)
		{
			if (m_bExist[r][i])
			{
				...
				BYTE optionSize2 = item.optionType2.size();
				oStream.write(optionSize2);
				list<OptionType_t>::const_iterator iOption2;
				for (iOption2=item.optionType2.begin(); iOption2!=item.optionType2.end(); iOption2++)
				{
				...
				oStream.write(item.HeroOption );
				oStream.write(item.HeroOptionAttr );
	...
	throw()
{
	...
	switch (pItem->getItemClass())
	{
		case Item::ITEM_CLASS_AR:
			...
			m_pItems[rack][index].optionType2   = pItem->getOptionTypeList2();
			...
			m_pItems[rack][index].HeroOption = pItem->getHeroOption();
			m_pItems[rack][index].HeroOptionAttr = pItem->getHeroOptionAttr();
			break;
