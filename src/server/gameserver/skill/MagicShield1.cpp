...
// 슬레이어 셀프 핸들러
//////////////////////////////////////////////////////////////////////////////
void MagicShield1::execute(Slayer* pSlayer, SkillSlot* pSkillSlot, CEffectID_t CEffectID)
	
{
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected && !bEffected2)
		{
			...
			// 이팩트 클래스를 만들어 붙인다.
			EffectMagicShield1* pEffect = new EffectMagicShield1(pSlayer);
			pEffect->setDeadline(output.Duration);
			pEffect->setLevel(output.Damage);
			pSlayer->addEffect(pEffect);
			pSlayer->setFlag(Effect::EFFECT_CLASS_MAGIC_SHIELD1);
...
//			increaseSkillExp(pSlayer, DomainType, pSkillSlot, pSkillInfo, _GCSkillToSelfOK1);
			...
			SkillSlot* pSkillSlot2 = pSlayer->hasSkill( SKILL_MAGIC_SHIELD2 );
			if(pSkillSlot2 != NULL){
