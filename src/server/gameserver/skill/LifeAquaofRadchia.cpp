...
#include "EffectLifeAquaofRadchia.h"
#include "Creature.h"
#include "RankBonus.h"
...
// 뱀파이어 오브젝트 핸들러
//////////////////////////////////////////////////////////////////////////////
void LifeAquaofRadchia::execute(Ousters* pOusters, ObjectID_t TargetObjectID, OustersSkillSlot* pOustersSkillSlot, CEffectID_t CEffectID)
...
// 뱀파이어 타일 핸들러
//////////////////////////////////////////////////////////////////////////////
void LifeAquaofRadchia::execute(Ousters* pOusters, ZoneCoord_t X, ZoneCoord_t Y, OustersSkillSlot* pOustersSkillSlot, CEffectID_t CEffectID)
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bTileCheck && bSatisfyRequire)
		{
			...
			for (int j=-2; j <= 2; j++)
			{
				...
				if (rect.ptInRect(tileX, tileY))
				{
					...
					{
						// 같은 effect가 있으면 지운다.
						Effect* pOldEffect = tile.getEffect(Effect::EFFECT_CLASS_LIFE_AQUA_OF_RADCHIA);
						if (pOldEffect != NULL)
						{
						...
						// 이펙트 클래스를 생성한다.
						EffectLifeAquaofRadchia* pEffect = new EffectLifeAquaofRadchia(pZone , tileX, tileY);
...
//								pEffect->affect(pTarget);
			...
			for(list<Creature*>::const_iterator itr = watcherList.begin(); itr != watcherList.end(); itr++)
			{
				...
				if (bBelong == false && canSee(pWatcher, pOusters) == false)
				{
					...
					if (!pWatcher->isPC())
					{
						//cout << "LifeAquaofRadchia : 왓처 리스트가 PC가 아닙니다." << endl;
...
LifeAquaofRadchia g_LifeAquaofRadchia;
