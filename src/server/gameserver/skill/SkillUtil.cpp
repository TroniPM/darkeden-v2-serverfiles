...
Damage_t computeDamage(Creature* pCreature, Creature* pTargetCreature, int CriticalBonus, bool& bCritical)
{
	...
	try 
	{
		if (pCreature->isSlayer()) 
		{
			...
			Damage = computeSlayerDamage(pSlayer, pTargetCreature, bCriticalHit);
			// ÇÃ·¹ÀÌ¾î ¿¡°Ô °ø°Ý½Ã µ¥¹ÌÁö Áõ°¡ ¿É¼ÇÀÌ ÀÖÀ» °æ¿ì
			//if(pPC->getAddPlayerDamage() != 0){
			//	Damage += getPercentValue(Damage, (pPC->getAddPlayerDamage()/10));
			//}
	
			// Å©¸®Æ¼ÄÃ È÷Æ®´Ù. ÀÌ È÷Æ®°¡ ¶ã °æ¿ì µ¥¹ÌÁö°¡ 150%Áõ°¡ÇÏ°í ¿É¼Ç¿¡ µû¶ó ¼öÄ¡°¡ ´õ Áõ°¡ÇÑ´Ù.
			if (bCritical)
			{
				Damage = getPercentValue(Damage, 150);
				//if(pPC->getCriticalAddDamage() !=0)
				//Damage += getPercentValue(Damage, pPC->getCriticalAddDamage());
			}
		} 
		else if (pCreature->isVampire()) 
		{
	...
	if (bCritical && pTargetCreature->isMonster())
	{
		...
		// Å©¸®Æ¼ÄÃ Ãß°¡ µ¥¹ÌÁö ¿É¼ÇÀÌ ÀÖÀ» °æ¿ì µ¥¹ÌÁö¸¦ Ãß°¡·Î ´õ ³Ö´Â´Ù.
		//if(pCreature->isPC()){
		//PlayerCreature* pPC = dynamic_cast<PlayerCreature*>(pCreature);
		//if(pPC->getCriticalAddDamage() !=0)
		//Damage += getPercentValue(Damage, pPC->getCriticalAddDamage());
...
Damage_t computeVampireDamage(Vampire* pVampire, Creature* pTargetCreature, bool bCritical)
{
	...
	// ¹«±â¸¦ µé°í ÀÖ´Ù?? min, max¿¡ ¹«±âÀÇ min, max¸¦ °è»êÇØ ÁØ´Ù.
...
Damage_t computeSlayerSilverDamage(Creature* pCreature, int Damage, ModifyInfo* pMI)
{
	...
	{
		// ??°è¿­ÀÇ ¹«±â¶ó¸é, Àº Åº¾ËÀÌ ³ª°¡´Â °ÍÀÌ¹Ç·Î,
		// ¹«±â ÀÚÃ¼ÀÇ ÀºÀ» ÁÙÀÌ¸é ¾È ?È´? ÀÌ°ÍÀº ¿ÜºÎ¿¡¼­,
...
RankExp_t computeRankExp2(int myLevel, int otherLevel) // by sigi. 2002.12.31
{
	RankExp_t rankExp = 0;
	...
	if (myLevel!=0 && otherLevel!=0)
	{
		...
		// by sigi. 2002.12.31
		rankExp = getPercentValue( rankExp, g_pVariableManager->getVariable( ZONE_RANK_EXP ) );
	}
...
// Creature¸¦ Á×¿´À»¶§ÀÇ È¿°ú
//
// ?×À?»ç¶÷¿¡°Ô KillCount¸¦ Áõ°¡½ÃÄÑÁØ´Ù. --> °è±Þ °æÇèÄ¡
// by sigi. 2002.8.31
//////////////////////////////////////////////////////////////////////////////
void
affectKillCount(Creature* pAttacker, Creature* pDeadCreature)
{
	...
	if (pAttacker->isSlayer())
	{
		...
		// ½½·¹ÀÌ¾î??°æ¿ì ¹«±â¸¦ µé°í ÀÖÁö ¾Ê´Ù¸é ¹«½ÃÇÑ´Ù.
		...
		/*if (pDeadCreature->isVampire())
		{
			...
			// Á×ÀÎ ½½·¹ÀÌ¾î´Â Å³Æ÷ÀÎÆ®¸¦ È¹µæ, ¹ìÆÄÀÌ¾î´Â ÇÏ¶ô.
			if(pVampire->getKillPoint() >= pVampire->getLevel()){
			pSlayer->setKillPoint(pSlayer->getKillPoint()+otherLevel*2);
			pVampire->setKillPoint(pVampire->getKillPoint()-otherLevel);

			GCModifyInformation gcFameMI;
			gcFameMI.addLongData( MODIFY_KILL_POINT, pSlayer->getKillPoint() );
			pAttacker->getPlayer()->sendPacket( &gcFameMI );

			gcFameMI.addLongData( MODIFY_KILL_POINT, pVampire->getKillPoint() );
			pDeadCreature->getPlayer()->sendPacket( &gcFameMI );
			}
		}
		// ½½·¹ÀÌ¾î°¡ ¾Æ¿ì½ºÅÍ½º¸¦ ?×À?°æ¿ì
		else if (pDeadCreature->isOusters())
		{
			...
			// Á×ÀÎ ½½·¹ÀÌ¾î´Â Å³Æ÷ÀÎÆ®¸¦ È¹µæ, ¹ìÆÄÀÌ¾î´Â ÇÏ¶ô.
			if(pOusters->getKillPoint() >= pOusters->getLevel()){
			pSlayer->setKillPoint(pSlayer->getKillPoint()+otherLevel*2);
			pOusters->setKillPoint(pOusters->getKillPoint()-otherLevel);
	...
	else if (pAttacker->isVampire())
	{
		...
		/*if (pDeadCreature->isSlayer())
		{
			...
			// Á×ÀÎ ½½·¹ÀÌ¾î´Â Å³Æ÷ÀÎÆ®¸¦ È¹µæ, ¹ìÆÄÀÌ¾î´Â ÇÏ¶ô.
			if(pSlayer->getKillPoint() >= pSlayer->getLevel()){
			pVampire->setKillPoint(pVampire->getKillPoint()+otherLevel*2);
			pSlayer->setKillPoint(pSlayer->getKillPoint()-otherLevel);
		...
		if (pDeadCreature->isMonster())
		{
			...
			if (pMonster->isMaster())
			{
				// last killÇÑ »ç¶÷??°æÇèÄ¡ ÇÑ¹ø ´õ ¸Ô´Â´Ù.
	...
	else if (pAttacker->isOusters())
	{
		...
		/*if (pDeadCreature->isSlayer())
		{
			...
			// Á×ÀÎ ½½·¹ÀÌ¾î´Â Å³Æ÷ÀÎÆ®¸¦ È¹µæ, ¹ìÆÄÀÌ¾î´Â ÇÏ¶ô.
			if(pSlayer->getKillPoint() >= pSlayer->getLevel()){
			pOusters->setKillPoint(pOusters->getKillPoint()+otherLevel*2);
			pSlayer->setKillPoint(pSlayer->getKillPoint()-otherLevel);
		...
		if (pDeadCreature->isMonster())
		{
			...
			if (pMonster->isMaster())
			{
				// last kill??»ç¶÷Àº °æÇèÄ¡ ÇÑ¹ø ´õ ¸Ô´Â´Ù.
...
//					addSimpleCreatureEffect( pPC, Effect::EFFECT_CLASS_CAN_ENTER_GDR_LAIR, 216000 );
		...
		//if ( pMonster->getMonsterType() == 906 )
		//{
		//	addSimpleCreatureEffect( pPC, Effect::EFFECT_CLASS_CASTLE_OF_DENSE, 3600 );
		//}
...
//				cout << "ÆêÀ½½ÄÀÌº¥Æ®:" << iClass << ", " << itemType << endl;
	...
	if (PartyID != 0)
	{
		...
		if ( pPC->isAdvanced() )
			pLPM->shareAdvancementExp(PartyID, pAttacker, computeCreatureExp(pDeadCreature,100));
	}
	else
	{
		...
		{
			pAttackPC->increaseAdvancementClassExp(computeCreatureExp(pDeadCreature,100),true);
		}

		// ÆÄÆ¼¿¡ °¡ÀÔµÇ¾îÀÖÁö ¾Ê´Ù¸é È¥ÀÚ ¿Ã¶ó°£´Ù.
		otherLevel += 130;
		RankExp_t rankExp = computeRankExp( myLevel, otherLevel );
		if( pAttacker->getZoneID() >= 11 && pAttacker->getZoneID() <= 94 || pAttacker->getZoneID() == 1801){
		rankExp  = computeRankExp2( myLevel, otherLevel );
		} 
		if ( pDeadCreature->isMonster() )
		{
			Monster* pMonster = dynamic_cast<Monster*>(pDeadCreature);
...
//					pVampireAttacker->getEffectManager()->deleteEffect( Effect::EFFECT_CLASS_MEPHISTO );
		...
		else if (pAttacker->isMonster())
		{
			...
			if (pMonsterAttacker->getHP(ATTR_CURRENT)*3 < pMonsterAttacker->getHP(ATTR_MAX))
			{
				...
				// ÇÑ¹ø °è»êÀ» ÇÏ°í ³ª¸é, Á×±â Àü±îÁö´Â ´Ù½Ã ?è»êÇÏÁ?¾Êµµ·Ï
			   ...
			   bool canKillTarget, bool canSteal)
{
	...
	|| pTargetCreature->isDead())
	{
		...
		// return?ªÀ¸·?ÇöÀç HP¸¦ ³Ñ°ÜÁà¾ß Á¤»óÀÌ°ÚÁö¸¸
		// return°ªÀ» »ç¿ëÇÏ´Â ºÎºÐÀÌ ¾ø¾î¼­ ÀÏ´Ü ¹«??ÇÑ?
		// by sigi. 2002.9.5
		return 0;
	}

//	bool canKillTarget = (SkillType==SKILL_ACID_SWAMP)?false:true;
	// ¿Ø¸¸ÇÑ ½ºÅ³ ´Ù ÇÇÈíµÇµµ·Ï Ã³¸®.
	if(SkillType != SKILL_PROMINENCE && SkillType != SKILL_HELLFIRE){canSteal = true;}
	Zone* pZone = pTargetCreature->getZone();
	Assert(pZone != NULL);

	// Áúµå·¹ ·¹¾î¿¡¼­
	if ( pZone->getZoneID() == 1412 || pZone->getZoneID() == 1413 || pZone->getZoneID() == 5030 || pZone->getZoneID() == 5040 || pZone->getZoneID() == 5050 )
	{
	...
	// ?Æ¸¶°Ôµ?ÀÌÆåÆ®°¡ °É·ÁÀÖ??°æ¿ì ¼öÁ¤±¸½½(?)ÀÇ HP¸¦ ±ð¾ÆÁÖ°í Å¸°ÙÀº °ø°Ý¹ÞÁö ¾Ê´Â´Ù.
	// SKILL_ARMAGEDDONÀÏ °æ¿ì ¾Æ¸¶°Ô??ÀÌÆåÆ® ÀÚÃ¼ÀÇ µ¥¹ÌÁöÀÌ¹Ç·Î ±×³É Å¸°ÙÀ» °ø°ÝÇÏ´Â ÂÊÀ¸·Î ³Ñ¾î°£´Ù.
	...
	if ( pTargetCreature != NULL && pTargetCreature->isFlag( Effect::EFFECT_CLASS_MOLE_SHOT ) )
	{
			cout << "È¿°ú ¹ÞÀ½"<<endl;
...
//				return 0;
	...
	//cout << " ¿©±â´Â ¹®Á¦ ¾øÀ½ 1.902 " << endl;
			...
			int HitAddMagic = pPC->getHitAddMagic();
			int HitAddPhysic = pPC->getHitAddAttack();

			// °ø°ÝÀÚ°¡ ¾Æ¿ì½ºÅÍÁîÀÌ°í ¹Ø¿¡ ¶óµåÅ°¾Æ »ý¸í¼ö°¡ ÀÖÀ» °æ¿ì °ø°Ý·ÂÀ» Áõ°¡ ½ÃÄÑÁØ´Ù.
			// ´Ü, ¸¶¹ý °ø°ÝÀÏ °æ¿ì¿¡¸¸ °ø°Ý·ÂÀÌ Áõ°¡ÇÏ°Ô µÇ¾îÁø´Ù.
			if(pAttacker->isOusters() && pSkillProperty->isMagic()){
				bool bTileCheck = false;

				VSRect rect(0, 0, pZone->getWidth()-1, pZone->getHeight()-1);
				if (rect.ptInRect(pAttacker->getX(), pAttacker->getY())) bTileCheck = false;
				...
				int oX, oY;
					...
					{
						int tileX = pAttacker->getX()+oX;
						int tileY = pAttacker->getY()+oY;
							if (rect.ptInRect(tileX, tileY))
							{
								Tile& tile = pZone->getTile(tileX, tileY);
								if (tile.canAddEffect())
								{
									if ( tile.getEffect(Effect::EFFECT_CLASS_LIFE_AQUA_OF_RADCHIA) != NULL )
									{
			...
			// ¹°¸® °ø°Ý·Â Áõ°¡ ¿É¼ÇÀÌ ÀÖÀ¸¸é ÆÛ¼¾Æ®Áö·Î ¿Ã¶ó?¡°?µÈ´Ù.
			if(HitAddPhysic != 0 && bPhysicDamage){
			Damage  += getPercentValue(Damage, (HitAddPhysic/10));
			}

			// ¸¶¹ý °ø°Ý·Â Áõ°¡ ¿É¼Ç??ÀÖÀ¸¸é ÆÛ¼¾Æ®Áö·Î ¿Ã¶ó°¡°Ô µÈ´Ù.
			if(HitAddMagic != 0 && bMagicDamage ){
			Damage  += getPercentValue(Damage, (HitAddMagic/10));
			}
...
//				//cout << "µ¥¹ÌÁö º¸³Ê½º Àû¿ë : " << Damage << " + " << PhysicBonusDamage << endl;
	...
	/*if (pTargetCreature != NULL)
	{
		...
		{
		if(pPC->getHitSleep() != 0 && pPC->getHitSleep() >= Random(1,1000))
		{
	...
	if(pAttacker->isPC()){
	int PartyID = pAttacker->getPartyID();
	if (PartyID != 0)
	{
		// ÆÄÆ¼¿¡ °¡ÀÔµÇ¾î ÀÖ´Ù¸é ·ÎÄÃ ÆÄÆ¼ ¸Å´ÏÀú¸¦ ÅëÇØ 
		// ÁÖÀ§ÀÇ ÆÄÆ¼¿øµé°ú °æÇèÄ¡¸¦ °øÀ¯ÇÑ´Ù.
		LocalPartyManager* pLPM = pAttacker->getLocalPartyManager();
		Assert(pLPM != NULL);
		pLPM->shareAttackBloodBurst(PartyID, pAttacker, pTargetCreature, 1);
	}
	else
	{
		// ÆÄÆ¼¿¡ °¡ÀÔµÇ¾îÀÖÁö ¾Ê´Ù¸é È¥ÀÚ ¿Ã¶ó°£´Ù.
		...
		// Å¸°Ý½Ã È®·ü·Î ¸ó½ºÅÍ¿¡°Ô¼­ µ·À» È¹µæ ÇÒ ¼ö ÀÖ´Ù.
	/*if(pTargetCreature->isMonster()){
			if(pPC->getHitAddGold() != 0 && pPC->getHitAddGold() >= Random(1,100)){
			int gold = getPercentValue(Damage, pPC->getHitAddGold());
			GCModifyInformation gcMI;
			if ( pAttacker->isSlayer() )
			{
				Slayer* pSlayer = dynamic_cast<Slayer*>(pAttacker);
				pSlayer->setGoldEx(pSlayer->getGold()+gold);
			...
			{
				Vampire* pVampire = dynamic_cast<Vampire*>(pAttacker);
				pVampire->setGoldEx(pVampire->getGold()+gold);
			...
			{
				Ousters* pOusters = dynamic_cast<Ousters*>(pAttacker);
				pOusters->setGoldEx(pOusters->getGold()+gold);

				gcMI.addLongData( MODIFY_GOLD, pOusters->getGold() );
			}

			pAttacker->getPlayer()->sendPacket( &gcMI );		
			}

			if(pPC->isAdvanced() && pPC->getHitAddExp() != 0 && pPC->getHitAddExp() >= Random(1,100)){
			pPC->increaseAdvancementClassExp(computeCreatureExp(pTargetCreature,10),true);
			}	
		}*/
	}
	}
	...
	int PartyID2 = pTargetCreature->getPartyID();
	if(pAttacker != NULL){
	if (PartyID2 != 0)
	{
		// ÆÄÆ¼¿¡ °¡ÀÔµÇ¾î ÀÖ´Ù¸é ·ÎÄÃ ÆÄÆ¼ ¸Å´ÏÀú¸¦ ÅëÇØ 
		// ÁÖÀ§ÀÇ ÆÄÆ¼¿øµé°ú °æÇèÄ¡¸¦ °øÀ¯ÇÑ´Ù.
		LocalPartyManager* pLPM = pTargetCreature->getLocalPartyManager();
		Assert(pLPM != NULL);
		pLPM->shareDefenseBloodBurst(PartyID2, pTargetCreature, pAttacker, 1);
	}
	else
	{
		// ÆÄÆ¼¿¡ °¡ÀÔµÇ¾îÀÖÁö ¾Ê´Ù¸é È¥ÀÚ ¿Ã¶ó°£´Ù.
		increaseDefenseBurstPoint(pAttacker, pTargetCreature, 1);
	}
	}
	}
	...
	if ( pTargetCreature != NULL && !pTargetCreature->isSlayer() )
	{
		...
		if(pAttacker != NULL && pAttacker->isMonster() &&  pTargetCreature != NULL && pTargetCreature->isPC() ){
			...
			if (pPetInfo != NULL && pPetInfo->getEnchantSkillType() == 1 && pAttacker->getMoveMode() != Creature::MOVE_MODE_FLYING && !pAttacker->isFlag(Effect::EFFECT_CLASS_STUN) && (rand()%100)<10 )
			{
	...
	if (pTargetCreature->isSlayer())
	{
		...
		PlayerCreature* pSlayerPC = dynamic_cast<PlayerCreature*>(pTargetCreature);

		// ¾Æ¿ì½ºÅÍÁî??¹ìÆÄÀÌ¾î¸¦ Á×ÀÎ °æ¿ì
		Slayer* pSlayer = dynamic_cast<Slayer *>(pTargetCreature);
		bool bSetDamage = false;

		// ÆøÁÖÇü °ÔÀÌ??Ã¤¿ì±â À§ÇÑ ºÎºÐ
		// °°Àº Á¾Á·ÀÏ ?æ¿ì¿¡´?¿Ã¶ó°¡Áö ¾Ê´Â´Ù
		/*if(!pAttacker->isSlayer())
		{
			Attr_t DefenseLine;
			DefenseLine = pSlayer->getDefenseBloodBurstPoint();
	...
	//cout << " ¿©±â´Â ¹®Á¦ ¾øÀ½ 7 " << endl;
		if ( pSlayer->isFlag(Effect::EFFECT_CLASS_REQUITAL) && pAttacker != NULL && pSkillProperty != NULL && pSkillProperty->isMelee() )
		{
			...
			counterDamage += getPercentValue(counterDamage,20);
		...
		// ¸¶½ºÅÍ ·¹¾î¿¡¼­ ¶ß´Â ±×¶ó¿îµå ¾îÅÃ(¶¥¿¡¼­ Æ¢¾î³ª¿À´Â ºÒ±âµÕ) ¸ÂÀ¸¸é ¿À¶ó½Çµå ¹«½ÃÇÏ°í HP ´â°Ô ÇÑ´Ù. 2003. 1.16. Sequoia
		if (pSlayer->isFlag(Effect::EFFECT_CLASS_AURA_SHIELD) && SkillType != SKILL_GROUND_ATTACK )
		{
			...
			Result = max(0, (int)pSlayer->getHP(ATTR_CURRENT) - (int)getPercentValue(Damage, 30));
...
/*				pSlayer->deleteEffect(Effect::EFFECT_CLASS_AURA_SHIELD);
				...
				// ±â¼úÀ» ´Ù½Ã ¾µ ¼ö ÀÖµµ·Ï ±â¼ú µô·¹?Ì¸?³¯·ÁÁØ´Ù.
...
/*			// ¾ÈÀüÁö´ë Ã¼Å©
			...
			if ( checkZoneLevelToHitTarget(pAttacker) )
			{
				...
				else if (pAttacker->isMonster())
				{
					...
					// ¸ó½ºÅÍ°¡ ¿ª µ¥¹ÌÁö¸¦ ¹ÞÀ» °æ¿ì¿¡µµ »þÇÁ½Çµå ¾²°í °ø°Ý¹ÞÀº ½½·¹?Ì¾î¿¡°?¿ì¼±±ÇÀÌ ÁÖ¾îÁø´Ù.
...
//		//cout << "after " << Damage << endl;
		...
		// Mephisto ÀÌ???°É·ÁÀÖÀ¸¸é HP 30% ÀÌÇÏÀÏ¶§ Ç®¸°´Ù.
...
//				pVampire->getEffectManager()->deleteEffect( Effect::EFFECT_CLASS_MEPHISTO );
	...
	//cout << " ¿©±â´Â ¹®Á¦ ¾øÀ½ OUSTERS 1 " << endl;
		...
		if (pOusters->isFlag( Effect::EFFECT_CLASS_SQUALLY_BARRIER1 ) &&  pSkillProperty->isMelee() )
		{
			...
			if ( pEffect != NULL && pEffect->getLevel() != 0 )
			{
				Damage -= getPercentValue( Damage, pEffect->getLevel() );
			}
		}
		if (pOusters->isFlag( Effect::EFFECT_CLASS_SQUALLY_BARRIER2 ) &&  pSkillProperty->isMelee() )
		{
	...
	//cout << " ¿©±â´Â ¹®Á¦ ¾øÀ½ OUSTERS 7 " << endl;
		...
		{
			// °ø°ÝÀÚ°¡ ½½·¹ÀÌ¾î¶ó¸é µ¥¹ÌÁö¿¡ Àº µ¥¹ÌÁö°¡ Ãß°¡µÉ ¼ö°¡ ÀÖ´Ù.
			// ¾Æ¿ì?ºÅÍ½º´?Àº µ¥¹ÌÁö¸¦ 1.5¹è ¹Þ´Â´Ù.
...
//cout << " ¿©±â´Â ¹®Á¦ ¾øÀ½ MONSTER 19"	<< endl;
	...
	{
		if(pTargetCreature != NULL && pAttacker != NULL){
...
//				&& !pWeapon->isUnique())
			{
				...
				if (Result == 0) // ¹«±â°¡ ³»±¸?µ°?0ÀÌ¶ó¸é ÆÄ±«ÇÑ´Ù.
				{
...
//			&& pGear->getItemClass()!=Item::ITEM_CLASS_VAMPIRE_AMULET) 
		{
			// ¼±ÅÃµÈ ½½¶ù¿¡ Á¸?çÇÏ´?¾ÆÀÌÅÛÀÌ ¾ç¼Õ ¹«±â¶ó¸é, 
...
//	}
	...
	if (pAttacker->isSlayer())
	{
		...
		// Åõ¸í??»óÅÂ¶ó¸é, Detect invisibility ¸¶¹ýÀÌ °É·ÁÀÖ¾î¾ß º¼ ¼ö ÀÖ´Ù.
	...
	switch (SkillType)
	{
		...
		// ÃÑÀ¸·Î ÇÏ´Â °ø°ÝÀº ³¯¾Æ?Ù´Ï´?»ó´ë¿¡°Ôµµ °¡´ÉÇÏ´Ù.
		case SKILL_ATTACK_ARMS:
			return true;
...
// ´Ü ½½·¹ÀÌ¾î °°Àº °æ¿ì¿¡´Â Sacrifice °°Àº ?ÌÆåÆ??ºÙ¾îÀÖÀ¸¸é,
...
int decreaseMana(Creature* pCaster, int MP, ModifyInfo& info)
{
	...
	if (pCaster->isSlayer())
	{
		...
		// Magic Brain ??ÀÖ´Ù¸é MP ¼Ò¸ð·® 25% °¨¼Ò
...
bool verifyDistance(Creature* pCreature, ZoneCoord_t X, ZoneCoord_t Y, Range_t Dist)
{
	...
	// ?ø°ÝÀÚ°?¼­ ÀÖ´Â À§Ä¡°¡ ½½·¹ÀÌ¾î ?ÈÀüÁ?´ë¶ó? 
	// ½½·¹ÀÌ¾î°¡ ?Æ´?ÀÚ´Â ±â¼úÀ» »ç¿ëÇÒ ¼ö ¾ø´Ù.
	...
	// ¾Æ¿ì½ºÅÍ½º°¡ ¾Æ´Ñ ÀÚ´Â ±â¼úÀ» »ç¿ëÇÒ ¼ö ¾ø´Ù.
		...
		return false;
	// ¿ÏÀü ?ÈÀüÁö´ë¶ó¸?½½?¹ÀÌ¾îµ?¹ìÆÄÀÌ¾îµç ±â¼úÀ» »ç¿ëÇÒ ¼ö ¾ø´Ù.
...
bool verifyDistance(Creature* pCreature, Creature* pTargetCreature, Range_t Dist)
{
	...
	// ¹æ¾îÀÚ°¡ ¹ìÆÄÀÌ¾î¶ó¸é ±â¼úÀº ¸ÂÁö ¾Ê´Â´Ù.
		...
		return false;
	// ¹æ¾îÀÚ°¡ ¼­ ÀÖ´Â À§Ä¡°¡ ¾Æ¿ì½ºÅÍ½º ¾ÈÀüÁö?ëÀ??
...
//////////////////////////////////////////////////////////////////////////////
// °¢ Á¸ÀÇ PK ?¤Ã??µû¶ó, PK°¡ µÇ´À³Ä ¾È µÇ´À³Ä¸¦ Á¤ÇÑ´Ù.
//////////////////////////////////////////////////////////////////////////////
...
// (¼¿ÇÁ ±â¼úÀÏ °æ¿ì, Á¸ ·¹º§À» Ã¼?©ÇÏ´?ÇÔ¼ö´Ù...)
//////////////////////////////////////////////////////////////////////////////
...
bool checkZoneLevelToHitTarget(Creature* pTargetCreature)
{
	...
	// ½½·¹ÀÌ¾î ¾ÈÀüÁö´ë¿¡¼­ ½½·¹ÀÌ¾î´Â ±â¼ú¿¡ ¸ÂÁö ¾Ê´Â??
...
//////////////////////////////////////////////////////////////////////////////
// ¼ºÇâÀ» º¯°æÇÑ´Ù.
// ±â¼úÀ» »ç¿ëÇÏ°Å³ª, PK¸¦ ÇÒ ¶§ »ý±â´Â ¼ºÇâ º¯È­¸¦ °è»êÇÏ´Â ÇÔ?ö´?
//////////////////////////////////////////////////////////////////////////////
	...
	Creature* pAttacker, ModifyInfo* pMI, ModifyInfo* pAttackerMI)
{
	...
	if (pAttacker != NULL) 
	{
		// ÀÌº¥Æ® °æ±âÀå¿¡¼­´Â ¼ºÇâÀÌ ¾È¹Ù²î°Ô µÇ´Â ?ÚµåÀÌ´?
		// ZoneInfo¿¡ ³Ö°í, Zone¿¡¼­ ÀÐÀ» ¼ö ÀÖ°Ô ÇÏ¸é ÁÁ°ÚÁö¸¸,
		// °©ÀÚ±â ¶³¾îÁø ?ÏÀÌ¶?±ÍÂú´Ù´Â ÀÌÀ¯·Î ÇÏµå ÄÚµùÀÌ´Ù. - -; 
		// 2002.8.21. by sigi
	...
	if (bSameRace)
	{
		...
		if (pAttackPC->hasEnemy(TargetName) && pTargetPC->isDead()) 
		{
			...
			{
				// ¼±°øÀÚ ??ºÆ??ÀÖ´Ù´Â ¸»Àº ¼±°øÀÚ¸¦ Áö¿öÁÖ´Â ÀÌÆåÆ®°¡ ¹«Á¶°Ç ÀÖ´Ù´Â ¾ê±âÀÌ´Ù. µû¶ó¼­ NULLÀÌ µÉ ¼ö ¾ø´Ù.
			...
			// ¼±°øÀÚÀÇ ¸®½ºÆ®¿¡ ¹æ¾îÀÚÀÇ ÀÌ¸§ÀÌ ÀÖ°í, ÀÚ½ÅÀÇ ¼ºÇâÀÌ Good ¶Ç´Â Neutral ÀÌ¶ó¸é Á¤´ç¹æÀ§·Î ÀÎÁ¤ÇÏ°í, ¼ºÇâÀÌ ¶³¾îÁöÁö´Â ¾Ê°Ô ÇÑ´Ù.
		if (!(bdecrease && pAttackPC->hasEnemy(TargetName) && g_pAlignmentManager->getAlignmentType(AttackAlignment) >= NEUTRAL)) 
		{
			...
			if (bdecrease) 
			{
				// ¸¸¾à ¹æ¾îÀÚÀÇ ?±°øÀ?¸®½ºÆ®¿¡ ³» ÀÌ¸§ÀÌ ÀÖ´Ù¸é, °ø°ÝÀÚ´Â ³ª»Û³ÑÀÌ´Ù.
				// ¼±°øÀÚÀÇ ¸®½ºÆ®¿¡ ÀÌ¸§ÀÌ ÀÖ´Ù´Â °ÍÀº ¾ÆÁ÷ ÀÌÆåÆ®°¡ ºÙ¾îÀÖ´Ù´Â ¾ê±âÀÌ´Ù.
				// ¼ºÇâÀ» È¸º¹½ÃÅ°´Â ÀÌÆåÆ®´Â ÇÑ¼ø°£¿¡ ÇÏ³ª ÀÌÇÏ·Î Á¸ÀçÇÒ ¼ö ÀÖ´Ù. Áßº¹?ÇÁ?¾Ê´Â´Ù.
				...
				{
					// ¾ø´Ù¸é ÃÖÃÊ??¼±°øÇÏ´Â °ÍÀÌ´Ù »õ ÀÌÆåÆ®¸¦ »ý¼ºÇØ¼­ ºÙÀÌ°í 5ºÐ°£ Áö¼Ó µÉ °ÍÀÌ´Ù.
					// ¹æ¾îÀÚ¿¡°Ô ?ç¶óÁ??ÀÌÆåÆ®¸¦ ºÙ¿©¾ß ÇÔÀ» ÀØÁö ¸»¾Æ¾ß ÇÑ´Ù.
...
void increaseAlignment(Creature* pCreature, Creature* pEnemy, ModifyInfo& mi)
{
	...
	// ¸ó½ºÅÍ°¡ ¾ÆÁ÷ »ì¾ÆÀÖÀ» °æ¿ì¿¡´Â ¼ºÇâÀÌ º¯È­?ÇÁ?¾Ê´Â´Ù.
	...
	else if (pCreature->isOusters())
	{
		...
		// ¼ºÇâÀÌ 0ÀÌ»óÀÎ °æ¿ì¿¡??¸ó½ºÅÍ¸¦ Á×¿©µµ ¼ºÇâÀÇ º¯È­°¡ ¾ø´Ù.
		if (OldAlignValue > 0) return;
		...
		BYTE STRMultiplier, BYTE DEXMultiplier, BYTE INTMultiplier, ModifyInfo& _ModifyInfo)
{
	...
	if (PartyID != 0)
	{
		// ÆÄ?¼¿?°¡ÀÔµÇ¾î ÀÖ´Ù¸é ·ÎÄÃ ÆÄÆ¼ ¸Å´ÏÀú¸¦ Åë??
		// ÁÖÀ§ÀÇ ÆÄÆ¼?øµé°?°æÇèÄ¡¸¦ °øÀ¯ÇÑ´Ù.
...
//////////////////////////////////////////////////////////////////////////////
// ½½·¹ÀÌ¾î ´É·ÂÄ¡ (STR, DEX, INT) °æÇèÄ¡¸¦ °è?êÇÑ´?
//////////////////////////////////////////////////////////////////////////////
...
/*	SkillLevel_t	MaxDomainLevel	= pSlayer->getHighestSkillDomainLevel();
	...
	// ½½·¹ÀÌ¾î ´É·ÂÄ¡´Â µµ¸ÞÀÎ ·¹º§ 100ÀÌÀü¿¡´Â ÃÑÇÕ 300À¸·Î Á¦ÇÑ µÈ´Ù.(±âÁ¸Ã³·³ 50, 200, 50 À¸·Î..)¶ÇÇÑ ±× ÀÌÈÄÀÇ °æÇèÄ¡´Â ´©ÀûµÇÁö ¾Ê´Â´Ù. 
	// ±×¸®°í µµ¸ÞÀÎ ·¹º§ ÀÌ 100À» ³Ñ¾î¼­¸é ´Ù½Ã ´É·ÂÄ¡ °æÇèÄ¡°¡ ´©ÀûµÇ¾î ´É·ÂÄ¡°¡ ¿Ã¶ó°¡±â ½ÃÀÛ?Ñ´?
	...
	if ( DEXMultiplier != 0 && CurDEXGoalExp == 0 && CurDEX < AttrBound )
	{
		...
		if (CurSTR + CurDEX + CurINT >= TotalAttrBound ) 
		{
			...
			// ¹Î?¸À?¿À¸¦ °æ¿ì STR³ª INTÁß ³ôÀº°ÍÀ» ¶³¾îÆ®¸®°í, °°À» °æ??STR¸¦ ¶³¾îÆ®¸°´Ù.
			if (CurSTR >= CurINT) 
			{
				decreaseSTR(pSlayer);
			} 
	...
	// ±âÁ¸ÀÇ ´É·ÂÄ¡¿Í ºñ±³?Ø¼?º¯°æµÈ ´É·ÂÄ¡¸¦ º¸³»ÁØ´Ù.
	if (bInitAll)
	{
...
void increaseSkillExp(Slayer* pSlayer, SkillDomainType_t DomainType, SkillSlot*  pSkillSlot, SkillInfo*  pSkillInfo, ModifyInfo& _ModifyInfo)
{
	...
	if (CurrentLevel < LimitLevel) 
	{
		...
		// ÄüÆÄÀÌ¾î´Â ³ªÁß¿¡ DB¸¦ ¼öÁ¤ÇØ¾ß ÇÒ °ÍÀÌ??
...
//		pSlayer->setSkillDomainExp(Domain, NewExp);
		...
		if (NewGoalExp == 0 && CurDomainLevel != SLAYER_MAX_DOMAIN_LEVEL) 
		{
			...
			// ÇöÀç ·¹º§¿¡¼­ ¹è¿ï ¼ö ÀÖ´Â ?â¼úÀ?ÀÖ´ÂÁö º»´Ù.
...
/*		if (DiffExp != 0)
		...
		if (isLevelUp && DomainLevelSum > SLAYER_MAX_DOMAIN_LEVEL) 
		{
			...
			// °á±¹ ds[j]ÀÇ °ªÀº ÇöÀç µµ?ÞÀÎ°?°°Áö ¾ÊÀº °¡Àå ³ôÀº ·¹º§ÀÇ µµ¸ÞÀÎÀÌ´Ù.
...
//			pSlayer->setSkillDomainExp(DownDomainType, DownDomainSumExp);
		...
		if (isLevelUp && DomainLevelSum >= GRADE_GRAND_MASTER_LIMIT_LEVEL) 
		{
				...
				&& !pSlayer->isFlag(Effect::EFFECT_CLASS_GRAND_MASTER_SLAYER))
			{
				...
				// affect()¾È¿¡¼­.. Flag°É¾îÁÖ°í, ÁÖÀ§¿¡ broadcastµµ ?ØÁØ´?
				pEffect->affect();
			}
...
//			<< ",Exp = " << (int)pBeforeExpInfo->getAccumExp() 
		...
		if (NewLearnSkillType != 0) 
		{
			// ¹è¿ï ¼ö ÀÖ´Â ±â¼úÀÌ ÀÖ°í ÀÌ¹Ì ¹è¿ì??¾ÊÀº »óÅÂ¶ó¸é ±â¼úÀ» ¹è¿ï ¼ö ÀÖ´Ù´Â ÆÐÅ¶À» ³¯¸°´Ù.
...
//	if ( NewGoalExp > 0 )
	if ( NewGoalExp > 0 || curLevel == OUSTERS_MAX_LEVEL )
	{
		...
		// °æÇèÄ¡ ¼¼ÀÌºê Ä«¿îÆ®°¡ ÀÏÁ¤ ¼öÄ¡¿¡ ´Ù´Ù¸£¸é ¼¼ÀÌºêÇÏ??
		// Ä«¿îÆ®¸¦ ÃÊ±â??ÃÄ?ÁØ´Ù. 
		if (ExpSaveCount > OUSTERS_EXP_SAVE_PERIOD)
		{
			StringStream attrsave;
...
//			<< ",Exp = " << (int)pBeforeExpInfo->getAccumExp() 
		...
		if (NewLearnSkillType != 0) 
		{
			// ¹è¿ï ¼ö ÀÖ´Â ±â¼úÀÌ ÀÖ°í ÀÌ¹Ì ¹è¿ìÁö ¾ÊÀº »óÅÂ¶ó¸é ±â¼úÀ» ¹è¿ï ¼ö ÀÖ´Ù??ÆÐÅ¶À» ³¯¸°´Ù.
			if (pOusters->hasSkill(NewLearnSkillType) == NULL) 
			{
				// GCLearnSkillReadyÀÇ m_SkillType¿¡ level upµÈ µµ¸ÞÀÎÀÇ °¡Àå ÃÖ±Ù
				// ±â¼úÀ» ´ëÀÔÇÑ´Ù. Áï, Å¬¶óÀÌ¾ðÆ® ±× ´ÙÀ½ ½ºÅ³À» ¹è¿ï¼ö ÀÖ??..
				GCLearnSkillReady readyPacket;
...
void increaseFame(Creature* pCreature, uint amount)
{
	...
	// ´ÙÀÌ³ª¹Í Á¸¾È¿¡¼­´Â ¸í¼º??¿Ã·ÁÁÖÁö ¾Ê´Â´Ù.
	if ( pCreature->getZone() != NULL && pCreature->getZone()->isDynamicZone() )
		return;
	
	// ·ÎÄÃ ÆÄÆ¼°¡ Á¸ÀçÇÑ?Ù¸? ÆÄÆ¼¿øÀÇ ¼ýÀÚ¿¡ µû?ó¼?¿Ã¶ó°¡´Â ¼öÄ¡°¡ º¯ÇÑ´Ù.
    ...
    else if (pCreature->isVampire())
    {
		...
		if (NewFame != OldFame) 
		{
			...
			// ?¼ÀÌºêÇÏµ?¾È ÇÏµç, ¸í¼ºÄ¡ ¼¼ÆÃÀº ÇØÁà¾ß ÇÑ´Ù.
...
//////////////////////////////////////////////////////////////////////////////
// ºí·¯µå ¹ö½ºÆ® (°ø°Ý Æ÷ÀÎÆ®)
//////////////////////////////////////////////////////////////////////////////
void increaseAttackBurstPoint(Creature* pAttacker, Creature* pTargetCreature, Attr_t amout)
{
	// °ø°ÝÀÚ,ÇÇ°Ý¹ÞÀºÀÚ ?Ç´?°ø°ÝÀÚ°¡ ÇÇ°Ý¹ÞÀºÀÚ°¡ µ¿ÀÏÇÒ°æ¿ì ¸®ÅÏ½ÃÅ²´Ù.
	//cout << " ºí·¯µå ¹ö½ºÆ® Å×½ºÆ® 1 " <<endl;
	if (pAttacker == NULL) return;
	//cout << " ºí·¯µå ¹ö½ºÆ® Å×½ºÆ® 2 " <<endl;
	if (pTargetCreature == NULL) return;
	//cout << " ºí·¯µå ¹ö½ºÆ® Å×½ºÆ® 3 " <<endl;
	if (pTargetCreature == pAttacker) return;
	//cout << " ºí·¯µå ¹ö½ºÆ® Å×?ºÆ?4" <<endl;

		// °ø°ÝÀÚ´Â °ø°Ý Æ÷ÀÎÆ®°¡ ¿Ã¶ó°£´Ù.
		if(pAttacker != NULL)
		{
				if(pAttacker->isSlayer()){
				if (pAttacker->isSlayer() == pTargetCreature->isSlayer()) return;
				Slayer* pSlayerAttacker = dynamic_cast<Slayer*>(pAttacker);

					if(pSlayerAttacker->getAttackBloodBurstPoint() < ATTACK_BURST_MAX){
						pSlayerAttacker->setAttackBloodBurstPoint(pSlayerAttacker->getAttackBloodBurstPoint()+amout);
						GCModifyInformation gcFameMI;	
						gcFameMI.addLongData( MODIFY_ATTACK_BLOOD_BURST_POINT, pSlayerAttacker->getAttackBloodBurstPoint() );
						pAttacker->getPlayer()->sendPacket( &gcFameMI );
					}
					pSlayerAttacker->saveExps();
				}
				else if(pAttacker->isVampire()){
				if (pAttacker->isVampire() == pTargetCreature->isVampire()) return;
				Vampire* pSlayerAttacker = dynamic_cast<Vampire*>(pAttacker);
				...
				if (pAttacker->isOusters() == pTargetCreature->isOusters()) return;
				Ousters* pSlayerAttacker = dynamic_cast<Ousters*>(pAttacker);
...
{
	// °ø°ÝÀÚ,ÇÇ°Ý¹ÞÀºÀÚ ¶Ç´Â °ø°ÝÀÚ°¡ ÇÇ°Ý¹ÞÀºÀÚ°¡ µ¿ÀÏÇÒ°æ¿ì ¸®ÅÏ½ÃÅ²´Ù.
	//cout << " ºí·¯µå ¹ö½ºÆ® Å×½ºÆ® 1 " <<endl;
	if (pAttacker == NULL) return;
	//cout << " ºí·¯µå ¹ö½ºÆ® Å×½ºÆ® 2 " <<endl;
	if (pTargetCreature == NULL) return;
	...
	//cout << " ºí·¯µå ¹ö½ºÆ® Å×½ºÆ® 4" <<endl;
		...
		// ¸ÂÀº »ç¶÷Àº µðÆæ½º ¹æ¾î°¡ ¿Ã?ó°£´?
		if(pTargetCreature != NULL)
		{
				if(pTargetCreature->isSlayer()){
				if (pAttacker->isSlayer() == pTargetCreature->isSlayer()) return;
				Slayer* pSlayerAttacker = dynamic_cast<Slayer*>(pTargetCreature);

					if(pSlayerAttacker->getDefenseBloodBurstPoint() < DEFENSE_BURST_MAX){
						pSlayerAttacker->setDefenseBloodBurstPoint(pSlayerAttacker->getDefenseBloodBurstPoint()+amout);
						GCModifyInformation gcFameMI;	
						gcFameMI.addLongData( MODIFY_DEFENSE_BLOOD_BURST_POINT, pSlayerAttacker->getDefenseBloodBurstPoint() );
						pTargetCreature->getPlayer()->sendPacket( &gcFameMI );
					}
				}
				else if(pTargetCreature->isVampire()){
				if (pAttacker->isVampire() == pTargetCreature->isVampire()) return;
				Vampire* pSlayerAttacker = dynamic_cast<Vampire*>(pTargetCreature);
				...
				if (pAttacker->isOusters() == pTargetCreature->isOusters()) return;
				Ousters* pSlayerAttacker = dynamic_cast<Ousters*>(pTargetCreature);
...
{
	// °ø°ÝÀÚ,ÇÇ°Ý¹Þ?ºÀ?¶Ç´Â ?ø°ÝÀÚ°?ÇÇ°Ý¹ÞÀºÀÚ°¡ µ¿ÀÏÇÒ°æ¿ì ¸®ÅÏ½ÃÅ²´Ù.
	//cout << " ºí·¯µå ¹ö½ºÆ® Å×½ºÆ® 1 " <<endl;
	if (pAttacker == NULL) return;
	//cout << " ºí·¯µå ¹ö½ºÆ® Å×½ºÆ® 2 " <<endl;
	if (pTargetCreature == NULL) return;
	...
	//cout << " ºí???¹ö½ºÆ® Å×½ºÆ® 4" <<endl;
		...
		if(pAttacker != NULL)
		{
				...
				Slayer* pSlayerAttacker = dynamic_cast<Slayer*>(pAttacker);
					...
					if(pSlayerAttacker->getPartyBloodBurstPoint() < PARTY_BURST_MAX){
						pSlayerAttacker->setPartyBloodBurstPoin(pSlayerAttacker->getPartyBloodBurstPoint()+amout);
						GCModifyInformation gcFameMI;	
						gcFameMI.addLongData( MODIFY_PARTY_BLOOD_BURST_POINT, pSlayerAttacker->getPartyBloodBurstPoint() );
						pAttacker->getPlayer()->sendPacket( &gcFameMI );
					}
					pSlayerAttacker->saveExps();
				}
...
void increaseDefensePartyBurstPoint(Creature* pAttacker, Creature* pTargetCreature, Attr_t amout)
{
	...
	//cout << " ºí·¯µå ¹ö½ºÆ® Å×½ºÆ® 2 " <<endl;
	if (pTargetCreature == NULL) return;
	//cout << " ºí·¯??¹ö½ºÆ® Å×½ºÆ® 3 " <<endl;
	if (pTargetCreature == pAttacker) return;
	//cout << " ºí·¯µå ¹ö½ºÆ® Å×½ºÆ® 4" <<endl;
		
		// ¸ÂÀº »ç¶÷Àº µðÆæ½º ¹æ¾î°¡ ¿Ã¶ó°£´Ù.
		if(pTargetCreature != NULL)
		{
				if(pTargetCreature->isSlayer()){
				if (pAttacker->isSlayer() == pTargetCreature->isSlayer()) return;
...
int getSplashVictims(Zone* pZone, int cx, int cy, Creature::CreatureClass CClass, list<Creature*>& creatureList, int splash)
{
	...
	for (int i=0; i<9; i++)
	{
		...
		if (rect.ptInRect(tilex, tiley))
		{
			...
			if (rTile.hasCreature(Creature::MOVE_MODE_FLYING))
			{
				...
				{
					// ¾Æ´Ñ °æ¿ì¿¡´Â CreatureClass°¡ ?°À?°æ¿ì¿¡¸¸ ´õÇÑ´Ù.
					creatureVector.push_back(pCreature);
				}
			}
		}
	...
	// ½ºÇÃ·¡½Ã µ¥¹ÌÁö¸¦ ?ÔÈ?³ðµéÀÇ ¼ýÀÚº¸´Ù ÇöÀç ÀÖ´Â Å©¸®ÃÄ°¡ Àû´Ù¸é,
	...
	// ½º?Ã·¡½?µ¥¹ÌÁö¸¦ ÀÔ??³ðº¸´Ù ÇöÀç Á¸ÀçÇÏ´Â Å©¸®ÃÄµéÀÌ ¸¹´Ù¸é,
	...
	else
	{
		...
		// ±×·¯¸é ÀÌ ¹è¿­?¡¼?2¸¦ Á¦?ÅÇ?Áà¾ß ÇÑ´Ù.
		// µÚ¿¡¼­ºÎÅÍ ¾ÕÀ¸·Î ÇÑÄ­¾¿ ¿Å?ÜÁà¾?ÇÑ´Ù.
		// (0, 1, 3, 4, 5, 5...)
		...
		for (int x=-range; x<=range; x++)
		{
			...
			if (rect.ptInRect(tilex, tiley))
			{
				...
				if (rTile.hasCreature(Creature::MOVE_MODE_FLYING))
				{
					...
					if (CClass == Creature::CREATURE_CLASS_MAX)
					{
						// CREATURE_CLASS_MAX°¡ ÆÄ¶ó?ÌÅÍ·?³Ñ¾î¿À´Â °æ¿ì¿¡´Â ¹«Á¶°Ç ´õÇÏÀÚ.
						creatureVector.push_back(pCreature);
					}
					else if (pCreature->getCreatureClass() == CClass)
					{
...
//////////////////////////////////////////////////////////////////////////////
// ´É·ÂÄ¡°¡ ÇÏ³ª?óµ??ó½ÂÇßÀ?¶§, HP¿Í MP¸¦ ¸¸¶¥À¸·Î Ã¤¿öÁÖ´Â ÇÔ¼ö´Ù. 
...
void healCreatureForLevelUp(Slayer* pSlayer, ModifyInfo& _ModifyInfo, SLAYER_RECORD* prev)
{
	...
	// ?É·ÂÄ¡°?»ó½ÂÇßÀ¸´Ï ¹«¾ð°¡ ºÎ°¡ÀûÀÎ ´É·ÂÄ¡°¡ º¯ÇßÀ¸¹Ç·Î º¸?»ÁØ´?
...
//////////////////////////////////////////////////////////////////////////////
// ´É·ÂÄ¡°¡ »ó½ÂÇßÀ» ¶§, HP¸¦ ¸¸¶¥À¸·Î Ã¤¿öÁÖ´Â ÇÔ¼ö´Ù. ¹ìÆÄ?Ì¾î¿?
// -- 2002.01.14 ±è¼º¹Î
//////////////////////////////////////////////////////////////////////////////
...
void executeSkillFailNormal(Creature* pCreature, SkillType_t SkillType, Creature* pTargetCreature, BYTE Grade)
{
	...
	// ¼¿ÇÁ ½ºÅ³ÀÌ³ª Å¸ÀÏ ½ºÅ³ÀÎ ?æ¿ì¿¡´?NULL·Î parameter°¡ ³Ñ¾î¿À´Â °ÍÀÌ Á¤»óÀÌ´Ù.
...
void executeSkillFailNormalWithGun(Creature* pCreature, SkillType_t SkillType, Creature* pTargetCreature, BYTE RemainBullet)
{
	...
	// ObjectSkillÀÏ °æ¿ì, »ó´ë¹æÀÇ OID°¡ Á¸ÀçÇÑ´Ù¸é ÆÐÅ¶¿¡´Ù ½Ç¾î¼­ º¸³»ÁØ´Ù.
	// ¼¿ÇÁ ½ºÅ³ÀÌ³ª ???½ºÅ³ÀÎ °æ¿ì¿¡´Â NULL·Î parameter°¡ ³Ñ¾î¿À´Â °ÍÀÌ Á¤»óÀÌ´Ù.
	// (Å¬?óÀÌ¾ðÆ?¡¼???¿ÇÁ³?Å¸ÀÏ ½ºÅ³ÀÌ ½ÇÆÐÇØ¼­ ³¯¾Æ¿À´Â GCSkillFailed2ÀÏ °æ¿ì¿¡´Â,
	// TargetObjectID¸¦ ÀÐÁöµµ ¾Ê´Â??)
	if (pTargetCreature != NULL)
	{
		gcSkillFailed2.setTargetObjectID(pTargetCreature->getObjectID());
	}
...
void decreaseHP(Zone* pZone, Creature* pCreature, int Damage, ObjectID_t attackerObjectID)
{
		...
		&& !pCreature->isFlag(Effect::EFFECT_CLASS_NO_DAMAGE))
	{
		...
		// attackerObjectID°¡ pCreature¸¦ Á×ÀÎ °æ¿ì??KillCount Ã³¸®
		// by sigi. 2002.9.9
...
Damage_t computePureSlayerDamage(Slayer* pSlayer)
{
	...
	// ¹«±â¸¦ µé°í ÀÖ´Ù¸é, min, max¿¡ ¹«±âÀÇ min, max¸¦ °è»ê??ÁØ´Ù.
	if (pItem != NULL && pSlayer->isRealWearingEx(Slayer::WEAR_RIGHTHAND))
	{
		MinDamage += pItem->getMinDamage();
		MaxDamage += pItem->getMaxDamage();
...
// ?¡°?Á¡»çÀÌ¸¦ °É¾î??°¥ ??ÀÖ´Â°¡? ( Å©¸®ÃÄ·Î ¸·Èù °æ¿ì´Â Á¦¿Ü )
