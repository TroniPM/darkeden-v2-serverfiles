...
#include "BloodyScarify.h"		//—™÷Æ¿””°
#include "BloodCurse.h"			//—™÷Æ?÷‰
#include "ShineSword.h"			//…¡“´÷ÆΩ£
#include "BombCrashWalk.h"		//æﬁ≈⁄∫‰’®
#include "SatelliteBomb.h"		//Œ¿–«∫‰ª˜ (»À¿‡«π–¬ººƒ‹)
#include "IllusionInversion.h"	//ø÷≤¿ª√æı(»À¿‡∑® ¶)
#include "HeavenGround.h"		//ÃÏ…ÒΩµ¡Ÿ
// ƒß¡È
#include "DummyDrake.h"			// µ¬¿◊øÀø˛¿‹(ª∑®)
#include "HydroConvergence.h"   // ∏¥∫œÀÆ¡∆(ÀÆ∑®)
#include "SummonClay.h"			// Õ¡∑®
#include "HeterChakram.h"		// œƒ≤º¿˚ª˘“Ú(√Ù’Ω)
#include "SkyFire.h"                                               //386 ÃÏ¿◊ªª® ADD BY RALLSER
...
#include "DeadlyClaw.h"                                                                    //391÷¬…À¡—◊¶ add by rallser
...
{
	output.Damage = 55 + (input.STR/40) + input.SkillLevel/20;
	output.Delay  = 50; // 1√ 
...
//	output.Damage = 2 + (input.STR/20) + (input.SkillLevel/15);
	output.Damage = 70 + (input.STR/30) + (input.SkillLevel/15);
	output.Delay  = 60; // 0.7√ 
...
{
	output.Damage = 110 + input.STR/15 + input.SkillLevel/10;
	output.Delay  = 80; // 0.8√ 
...
{
	output.Damage = 90 + input.STR/20 + input.SkillLevel/10;
	output.Delay  = 70; // 0.7√ 
...
//	output.Duration = (10 + input.STR/10 + input.SkillLevel/2) * 10;
	...
	output.Delay	= output.Duration;		// Delay øÕ Duration ¿Ã ∞∞¥Ÿ. by bezz 2003.3.5
...
void LightningHand::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	...
	// ≈¨∂Û¿Ãæ∆ÆøÕ¿« µø±‚»≠ πÆ¡¶∑Œ µÙ∑π¿Ã∞° æ¯∞≈≥™ ¡∂±› ¥ı ¬™æ∆æﬂ «—¥Ÿ.
	output.Delay  = 4; // 0.2√ 
...
{
	// by sigi. 2002.12.3
	output.Damage = 130 + input.STR/12 + input.SkillLevel/8;
	output.Delay  = 90; // 1√ 
...
{
	output.Damage   = 10 + input.SkillLevel/2; // ¿⁄Ω≈¿« + ∞™
	output.ToHit    = 5  + input.SkillLevel/3; // ∆ƒ∆º¿« + ∞™
//	output.Duration = (30 + input.STR/10 + input.SkillLevel*2/3) * 10;
	...
	//output.Damage   = 30 + input.SkillLevel/2; // ¿⁄Ω≈¿« + ∞™
	//output.ToHit    = 15 + input.SkillLevel/4; // ∆ƒ∆º¿« + ∞™
...
{
	output.Damage = 80 + input.STR/20 + input.SkillLevel/20;	// by sigi. 2002.12.3
	output.Delay  = 60; // 1√ 
}
...
//	output.Damage = 2 + input.STR/20 + input.SkillLevel/15;	// by sigi. 2002.12.3
	output.Damage = 120 + input.STR/20 + input.SkillLevel/15;
	output.Delay  = 70; // 0.7√ 
}

void TripleBreak::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 160 + input.STR/20 + input.SkillLevel/10;	// by sigi. 2002.12.3
	output.Delay  = 80; // 0.7√ 
}

void WildSmash::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 200 + input.STR/15 + input.SkillLevel/10;	// by sigi. 2002.12.3
	output.Delay  = 90; // 0.8√ 
}

void GhostBlade::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage   = 5 + input.STR/30 + input.SkillLevel/10;
	//output.Duration = (30 + input.SkillLevel/4) * 10;
	//output.Duration = (10 + input.STR/10 + input.SkillLevel/2) * 10;
	output.Duration = (30 + input.STR/10 + input.SkillLevel/2) * 10;
	//output.Delay    = (5 - input.SkillLevel/33) * 10; // 5->2√ 
...
{
	// penalty∞™¿∫ Berserker.cppø° ¿÷¥Ÿ.
	//output.Damage    = (input.STR/20) * (1 + input.SkillLevel/20); //  µ•πÃ¡ˆ ∫∏≥ Ω∫
	//output.ToHit    = (input.DEX/10) * (1 + input.SkillLevel/10); //  ≈ı»˝ ∫∏≥ Ω∫

	// 2002.12.06 ¿Â»´√¢
	output.Damage    = (input.STR/20) * (1 + input.SkillLevel/25); //  µ•πÃ¡ˆ ∫∏≥ Ω∫
//	output.ToHit    = (input.DEX/10) * (1 + input.SkillLevel/12); //  ≈ı»˝ ∫∏≥ Ω∫
	output.ToHit    = 10 + (1 + input.SkillLevel/12); //  ≈ı»˝ ∫∏≥ Ω∫
...
//	output.Damage = 30 + ( 15 * input.SkillLevel/50 );
	...
	output.ToHit    = 10 + input.SkillLevel/20; // ≈©∏Æ∆ºƒ√ »Æ∑¸ ¡ı∞°ƒ°
	//output.Delay    = (5 - input.SkillLevel / 50)*10; // 2√ ∏¶ ººπ¯¿∏∑Œ ≥™¥©∏È 0.66666 = 0.7√ 
...
void Typhoon::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	...
	//output.Duration = 10; // 1√  (≈∏∞Ÿ¿Ã ªÊªÊ µπæ∆∞°¥¬ Ω√∞£) - effectæ¯¥Ÿ -_-;
	//output.Delay    = 15; // 1.5√ 
	output.Duration = (1 + input.SkillLevel/100)*10; // 1√  (≈∏∞Ÿ¿Ã ªÊªÊ µπæ∆∞°¥¬ Ω√∞£) - effectæ¯¥Ÿ -_-;
...
void HeadShot::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	...
	// SkillUtil.cpp¿« computeArmsWeaponDamageBonus()ø°º≠ ∞ËªÍµ«π«∑Œ.. ¡ﬂ∫π µ•πÃ¡ˆ¥Ÿ. - -; by sigi. 2002.12.3
	if (input.IClass == Item::ITEM_CLASS_SG)
	{
		switch (input.Range)
		{
...
//	output.Damage = 10 + (input.INTE/10) + input.SkillLevel/5;
	...
	output.Delay  = 10; // 1√ 
//	output.Range = 2 + (input.SkillLevel/25); // æ≤¡ˆµµ æ ¥¬ ∞™ ø÷ ≥÷æÓ≥˘¿ª±Ó.. 8/18
...
void Bless::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	...
	{
		output.Damage   = 20 + ((input.INTE/25) + (input.Range/16.6));
	}
	else
	{
		output.Damage   = 16 + ((input.INTE/30) + (input.Range/16.6));
	}

	output.Duration = 10*10*6*30;
...
void Striking::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	...
	{
		output.Damage   = 16 +  ((input.INTE/35)+(input.Range/16.6));
	}
	output.Duration = 10*10*6*30;
	output.Delay    = (6 - input.SkillLevel/33) * 10; // 6->3√ 
...
void CureAll::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	if (input.TargetType == SkillInput::TARGET_OTHER) 
	{
		output.Damage = 50 + getPercentValue(input.INTE,25);
	}
	else
	{
		output.Damage = 35 + getPercentValue(input.INTE,25);
	}
...
void CureCriticalWounds::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	if (input.TargetType == SkillInput::TARGET_OTHER)
	{
		output.Damage = 10 + getPercentValue(input.INTE,17);
	}
	else
	{
		output.Damage = 10 + getPercentValue(input.INTE,17);
...
//	output.Damage = 10 + input.INTE/10 + input.SkillLevel/3; 
	output.Damage = 18 + input.INTE/5 + input.SkillLevel/2.5; 
...
void VigorDrop::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	...
	// 2002.12.06 ¿Â»´√¢
//	output.Damage = 10 + input.INTE/10 + input.SkillLevel/3; 
	output.Damage = 18 + input.INTE/5 + input.SkillLevel/3; 
//	output.Delay = (8 - input.SkillLevel/20)*10;
	output.Delay = (5 - input.SkillLevel/33)*10;
}
...
{
	output.Damage   = 20 + ((input.INTE/20)+(input.Range/3.3))*3.6;
...
void BloodyWall::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	...
	output.Duration =  max(150, (input.DEX/15 + input.INTE/10) * 10);
...
void PoisonStorm::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	...
	//2002.12.06 ¿Â»´√¢
	output.Damage = 40 + ((input.INTE/30)+(input.Range/3.3))*1.3;  

	// 2003.3.20 Sequoia µÙ∑π¿Ã πÃ¥œ∏ÿ ≥÷æ˙¥Ÿ.
...
void AcidStorm::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	...
	// 2002.12.06 ¿Â»´√¢
	output.Damage = 30 + ((input.INTE/25)+(input.Range/3.3))*1.3;  
...
{
	// µ•πÃ¡ˆ ∫∏≥ Ω∫¥¬ ø©±‚º≠ ∞ËªÍ«œ¡ˆ æ ∞Ì, 
	// Vampire::initAllStat()ø°º≠ ∞ËªÍ«—¥Ÿ.
	output.Damage   = 0; 
...
{
	// 2003.3.20 by Sequoia √÷¥Î∞™¿Ã √ﬂ∞°µ«æ˙¥Ÿ.
...
{
	output.Damage   = min((3+rand()%5), input.INTE/100);	// ∫–Ω≈ ∞≥ºˆ
	output.Duration = min(80, 30 + (input.INTE-20)/3) * 10;
	output.Delay    = max(3, 5 - (input.INTE-20)/10) * 10;
}
...
void SharpShield::computeOutput(const SkillInput& input, SkillOutput& output)
{
	...
	output.Delay	= output.Duration;		// Delay ??Duration ¿Ã ∞∞¥Ÿ. by bezz 2003.3.5
}
...
{
	output.Duration = ( 15 + ( ( input.SkillLevel / 25 ) * 10 ) ) * 10 + 450;
...
//		output.Damage = (int)( ( input.DEX / 10.0 ) * ( 1.0 + ( (float)(input.SkillLevel) / 22.5 ) ) );
		output.Damage = 10 + ((input.DEX/15)+(input.Range/6.6))*1.1;
	}
	else
	{
//		output.Damage = (int)( ( input.DEX / 10.0 ) * ( 4.0/3.0 + ( (float)(input.SkillLevel) / 45.0 ) ) );
		output.Damage = 10 + ((input.DEX/15)+(input.Range/4.3))*1.2;
		if ( input.SkillLevel == 30 ) output.Damage = (int)(output.Damage * 1.1);
	}
...
void NymphRecovery::computeOutput(const SkillInput& input, SkillOutput& output)
{
	if ( input.SkillLevel <= 15 )
	{
		if ( input.TargetType == SkillInput::TARGET_OTHER )
		{
			output.Damage = 15 + ((input.INTE/15)+(input.Range/3.3))*1.1;
		}
		else
		{
			output.Damage = 15 + ((input.INTE/15)+(input.Range/3.3))*1.3;
		}
	}
	else
	{
		if ( input.TargetType == SkillInput::TARGET_OTHER )
		{
			output.Damage = 15 + ((input.INTE/10)+(input.Range/3.3))*1.3;
		}
		else
		{
			output.Damage = 15 + ((input.INTE/10)+(input.Range/3.3))*1.3;
...
void GreatHeal::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	if (input.TargetType == SkillInput::TARGET_OTHER) 
	{
		output.Damage = 100 + ((input.INTE/15)+(input.Range/6.3));
	}
	else
	{
		output.Damage = 125 + ((input.INTE/15)+(input.Range/6.3));
	}
...
void BlazeWalk::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	...
	output.Damage = 2 * ( 1 + input.SkillLevel/25 ) + input.STR/10;
}
...
{
	output.Duration = 10*10*6*30;
	output.Damage = 10 + ((input.INTE/25)+(input.Range/12.6))*1.3;
	output.Delay = 30;
}
...
{
	output.Duration = ( 10 + input.INTE/20 ) * 10;
	output.Delay    = (max(3,  10 - input.DEX/50 ) * 10)*2;
//	output.Range    = (  3 + (input.SkillLevel-15) / 25);
}
...
void GroundBless::computeOutput(const SkillInput& input, SkillOutput& output)
{
	if ( input.TargetType == SkillInput::TARGET_SELF )
	{
		output.Damage   = 20 + ((input.INTE/25) + (input.Range/12.6));
	}
	else
	{
		output.Damage   = 16 + ((input.INTE/30) + (input.Range/12.6));
	}
...
void SharpChakram::computeOutput(const SkillInput& input, SkillOutput& output)
{
	if ( input.SkillLevel <= 15 )
	{
		output.Damage = 11 + ((input.DEX/20) + (input.Range/12.6));
	}
	else
	{
		output.Damage = 11 + ((input.DEX/20) + (input.Range/9.9));
//		if ( input.SkillLevel == 30 ) output.Damage = (int)(output.Damage * 1.3);
	}
...
void SummonWaterElemental::computeOutput(const SkillInput& input, SkillOutput& output)
{
	...
	if ( input.SkillLevel <= 15 )
	{
		output.Damage = 8 +  ((input.INTE/25)+(input.Range/9.9));
		output.Damage = min(output.Damage,20);
	}
	else
	{
		output.Damage = 8 + ((input.INTE/25)+(input.Range/7.7));
		if ( input.SkillLevel == 30 ) output.Damage = (int)(output.Damage*1.1);
	}
}
...
{
	output.Damage = 40 + (input.INTE/20)+(input.SkillLevel/2);
	output.Delay = 20;
}
...
{
	output.Damage = 100 + ((input.INTE/20)+(input.Range/3.3))*1.3;
	output.Delay = 18;
	output.Range = 7;
}
...
void SetAfire::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	...
	output.Damage = 50 + ((input.STR/6) + (input.DEX/10)+(input.Range/3.3));
	output.Delay = min(1,80 - (input.STR/6) + (input.DEX/10));
	if(output.Delay <= 0){
	output.Delay = 0;
	}
}
...
void IceHorizon::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	...
	if( input.SkillLevel <= 15)
	{
		output.Damage = 150 + ((input.INTE/25)+(input.Range/3.3))*1.3;
	}
	else
	{
		output.Damage = 200 + ((input.INTE/20)+(input.Range/3.3))*1.3;
	}
}
...
void FuryOfGnome::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	if( input.SkillLevel <= 15 )
	{
		...
		output.Damage   = 70 + ((input.INTE/10.0)+(input.Range/3.3)) + (1 + (input.SkillLevel/6.0));
	}
	else
	{
		output.Duration = min(10, (int)(5 * ( 4.0/3.0 + (input.SkillLevel/45.0))) );
		output.Damage   = 70 + ((input.INTE/5.0)+(input.Range/3.3)) + (1 + (input.SkillLevel/3.0));
	}
	output.Duration *= 10;	
...
{
	output.Damage = 50 + ((input.INTE/30)+(input.Range/3.3)) + min(input.Range,10);
...
{
	output.Damage = 80 + getPercentValue(input.INTE,25);
...
void HeavenGround::computeOutput(const SkillInput& input, SkillOutput& output)
{
	...
	//cout << "ÃÏ…ÒΩµ¡Ÿ—” ±:" << (int)output.Duration << endl;
...
void SummonClay::computeOutput(const SkillInput& input, SkillOutput& output)
{
	...
	//cout << "’≥Õ¡’ŸªΩ:" << (int)output.Duration << endl;
	output.Delay = output.Duration;
}
// œƒ≤º¿˚ª˘“Ú (ƒß’Ω)
...
{
	output.Damage = 120 + ((input.STR/15+input.DEX/25)+(input.Range/3.3))*1.3;
	output.Duration = 0;
	output.Delay = 0;
}
void CutStorm::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 200 + ((input.STR/15) + (input.Range/3.3))*1.2;
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void XRLMissile::computeOutput( const SkillInput& input, SkillOutput& output )
{
	output.Damage = 90 + ((input.DEX/15) + (input.Range/5))*1.3;
	output.ToHit  = 20;
	output.Duration = (input.Range-1) * 3;
	output.Delay = 0;
	output.Range	= 8;
...
{
	output.Damage = 120 + ((input.INTE/30)+(input.Range/3.3))*1.2;
	output.Delay = 20;
}
void BrambleHalo::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage	= 40 + (( input.INTE/15 )+(input.Range/9.9))*1.7;
	output.Duration = ( 25 + (input.SkillLevel/2) ) * 10;
	output.Delay	= output.Duration/3;
	output.Tick		= 10;
}
void DeadlyClaw::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 60 +((input.STR/15)+(input.DEX/25))+(input.Range/3.3)*1.2;
	output.Delay = 0;
}
void VoodooRing::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 100 + ((input.INTE/20) + (input.Range/3.3))*1.6;
	output.Duration = min(70, 10 + (input.INTE/10) * 10);
	output.Delay = 70;
}
void PenetrateWheel::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 120 + ((input.STR/15 + input.DEX/20)+(input.Range/3.3))*1.4;
	output.Duration = max(20,min(150,(int)(( 5.0 + (input.DEX/30.0) * ( 4.0/3.0 + (30/45.0) ) ) * 10)));
	output.Delay = 0;
}
void FireMeteor::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 150 + ((input.INTE/15)+(input.Range/3.3))*1.3;
	output.Delay = 20;
}
void BigRockfall::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 120 + ((input.INTE/20)+(input.Range/3.3))*1.3;
	output.Delay = 0;
}
void RapidFreeze::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 150 + ((input.INTE/23)+(input.Range/3.3))*1.1;
	output.Range = 7;
}
void EaglesEye1::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 15 + ((input.DEX/25)+(input.Range/9.9))/2;
	// ¿Ã±€æ∆¿Ã ±‚∫ª 30√ ∏¶ ¡¶∞¯«œ∞Ì µ¶Ω∫ø° ∫Ò∑ «œø© ¡ˆº”Ω√∞£¿∫ ∞Ëº” ¡ı∞°«—¥Ÿ.
	output.Duration = 600*20;
	output.Delay = 100;
}
void EaglesEye2::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 40 + ((input.DEX/25)+(input.Range/9.9))/2;
	// ¿Ã±€æ∆¿Ã 2¥¬ ±‚∫ª 50√  ¡¶∞¯ø° µ¶Ω∫ ∫Ò∑  ¡ˆº”Ω√∞£ ¡ı∞°«—¥Ÿ.
	output.Duration = 600*20;
	output.Delay = 100;
}
void MagicShield1::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 5 + ((input.STR/15 + input.DEX/25)+(input.Range/9.9))/6;
	output.Duration = 600*20;
	output.Delay = 100;
}
void MagicShield2::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 10 + ((input.STR/15 + input.DEX/25)+(input.Range/9.9))/5;
	output.Duration = 600*20;
	output.Delay = 100;
}
void BloodyShout1::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 200 + input.STR/10;
	output.Duration = 600*20;
	output.Delay = 100;
}
void BloodyShout2::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 400 + input.STR/5;
	output.Duration = 600*20;
	output.Delay = 100;
}
...
void EnergyBurst::computeOutput(const SkillInput& input, SkillOutput& output)
{
	//	output.Duration = (10 + input.INTE/20 + input.SkillLevel/10)*10;
	output.Duration = (10 + input.INTE/20 + 100/6)*9;
	//cout << "ÃÏ…ÒΩµ¡Ÿ—” ±:" << (int)output.Duration << endl;

	output.Delay = output.Duration;//(10 - 100/33) * 10;
	// edit by Coffee 2007-5-8 
	output.Damage = min(80,( input.INTE / 18 ) * ( 1 + ( 100 / 33 ) ));
	output.Tick     = 20;
}
void BatStorm::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 30+ ((input.STR/25 + input.DEX/30)+(input.Range/6.6))*1.4;  
	output.Duration = 30;
	output.Tick = 10;
	output.Delay = 30;
}
...
{
	output.Damage = 60 + ((input.STR/25 + input.DEX/30)+(input.Range/6.6))*1.4;  
	output.Duration = 30;
	output.Tick = 10;
	output.Delay = 30;
}
...
{
	output.Duration = 200 + input.DEX/5;
	output.Delay = output.Duration;
}
void SquallyBarrier2::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Duration = 250 + input.DEX/5;
	output.Delay = output.Duration;
}
// ÃÏ…ÒΩµ¡Ÿ (»À¿‡◊£∏£–¬ººƒ‹)
void MagmaDetonation1::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Duration = 40;
	output.Delay = 40;
	output.Damage = 40 + ((input.STR/15)+(input.Range/3.3));
	output.Tick     = 10;
}
void MagmaDetonation2::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Duration = 40;
	output.Delay = 40;
	output.Damage = 80 + ((input.STR/15)+(input.Range/3.3));
	output.Tick     = 10;
}
void BlazeWalk2::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Delay = 0;
	output.Damage = 20 + ((input.STR/15)+(input.Range/3.3))*1.7;
	if ( input.TargetType == SkillInput::TARGET_OTHER )
	{
	output.Range = 7;
	}else{
...
void Gladiator::computeOutput(const SkillInput& input, SkillOutput& output)
{
	if ( input.TargetType == SkillInput::TARGET_OTHER )
	{
	output.Damage = 40 + ((input.STR/15)+(input.Range/9.9))/3;  
	output.Duration = 600;
	}else{
	output.Damage = 40 + ((input.STR/15)+(input.Range/9.9))/2;  
	output.Duration = 600 * 20;
	}
	output.Delay = 200;
}
...
{
	output.Duration = 80 + (input.STR/5);
	output.Damage = 60 + (input.STR/5) + (input.DEX/10);
	output.Delay = 90;
	output.Tick     = 5;
}
// Œ¿–«∫‰ª˜ (»À¿‡?π–¬ººƒ?
...
{
	output.Damage	= 150 + ((input.DEX/15)+(input.Range/3.3))*1.4;
	output.Duration	= 6; //0.6 √Î
	output.Delay	= 20;
	output.Range	= 8;
}
...
{
	output.Duration = 10*10*6*30;
	output.Damage = 30 + ((input.INTE/25)+(input.Range/6.6))*1.3;
	output.Delay = 30;
}
void Bless2::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	if ( input.TargetType == SkillInput::TARGET_SELF )
	{
		output.Damage   = 20 + ((input.INTE/20)+(input.Range/12.6));
	}
	else
	{
		output.Damage   = 16 + ((input.INTE/25)+(input.Range/12.6));
	}
...
void Striking2::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	...
	{
		output.Damage   = 16 +  ((input.INTE/35)+(input.Range/12.6));
	}
...
{
	output.Damage	= 25 + ((input.INTE/25)+(input.Range/3.3));
	output.Duration = ( 15 + (input.SkillLevel/2) ) * 10;
	output.Delay	= output.Duration;
	output.Tick		= 10;
}
void GreatHeal2::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	if (input.TargetType == SkillInput::TARGET_OTHER) 
	{
		output.Damage = 100 + ((input.INTE/10)+(input.Range/3.3));
	}
	else
	{
		output.Damage = 120 + 5 + ((input.INTE/10)+(input.Range/3.3));
	}
...
{
	output.Damage = 50 + ((input.INTE/25)+(input.Range/3.3));
	output.Delay = ( 3 - (min(input.Range,10)/10) ) * 10;
	output.Range = 5 + (min(input.Range,10)/5);
	output.Duration = 0;
}
void BloodySkull::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	...
	// 2002.12.06 ¿Â»´√¢
	output.Damage = 10 + ((input.INTE/10)+(input.Range/3.3))*3.1;  
...
{
	output.Damage =  5 + input.DEX/10;
	output.Duration = 55;
	output.Delay = 550;
}
void AllysterWind::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 400 + ((input.DEX/2) + (input.Range/3.3))*2.2;
	output.Duration = 400;
	output.Delay = 600*2+10;
}
void FlameSight::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Duration = (60 + input.INTE/20 * 60) * 10; // 60~360√ 
	output.Delay    = (5 - input.INTE/33) * 10; // 5->2√ 

	// ∆ƒ∆º ∫∏≥ Ω∫∏¶ ∞ËªÍ«—¥Ÿ.
	//output.Damage   = getPercentValue(output.Damage, PartyEffectBoost[input.PartySize]);
	output.Duration = getPercentValue(output.Duration, PartyDurationBoost[input.PartySize]);
}
void LifeAquaofRadchia::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Duration = 900;
	output.Delay = output.Duration;
	output.Tick = 25;
	output.Damage = 250 + ((input.INTE/20)+(input.Range/3.3))*2.2;
}
void SpiralMegalith::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 250 + ((input.INTE/20) + (input.Range/3.3))*1.4;
	output.Delay = 0;
}
void BatBreak::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 10 + ((input.STR/5 + input.DEX/15)+(input.Range/3.3))*3.3;
	output.ToHit    = 10 + 100/20; // ≈©∏Æ∆ºƒ√ »Æ∑¸ ¡ı∞°ƒ°
	output.Delay    = 20; // by sigi. 2002.12.3
}
void ChaosSplitter::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 180 + ((input.STR/15 + input.DEX/25)+(input.Range/3.3))*1.2;
	output.Delay = 0;
}
void SummonDeadlyGland::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 220 + ((input.INTE / 20) + (input.Range/3.3))*1.5; 
	output.Duration = input.INTE * 10 / 25 + 50;
	output.Delay = output.Duration;
}
void SuddenBlast::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 250 + ((input.STR/15 + input.DEX/25)+(input.Range/3.3))*1.4;
	output.Delay = 20;
	output.Range = 7;
}
void TorrentialLife::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 50 + 50;
	output.Range = 20 + (100/10);
	output.Delay = 100;
	output.Duration = 300;
}
void MeteoBlaze::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 300 + ((input.INTE / 15) + (input.Range/3.3))*1.5; 
	output.Delay = (5 - 100/33)*10;
}
void CyclingWarRock::computeOutput(const SkillInput& input, SkillOutput& output)
{

	if ( input.TargetType == SkillInput::TARGET_OTHER )
	{
	output.Damage = 20 + ((input.INTE / 35.0) + (input.Range/6.6));  
	output.Duration = 600;
	}else{
	output.Damage = 40 + ((input.INTE / 30) + (input.Range/3.3)) * 2;  
	output.Duration = (600 * 3)/3;
	}

	output.Delay = output.Duration/3;
}
void LightingShock::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 200 + ((input.STR/25 + input.DEX/15)+(input.Range/3.3))*1.3;
	output.Duration = 20;
	output.Delay = 40;
}
void SolaFlare::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 240 + ((input.INTE/20)+(input.Range/3.3))*1.4;
	output.Duration = 12;
	output.Delay = 30;
}
void ViaCrucis::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 200 + ((input.INTE/30)+(input.Range/3.3));
	output.Duration = 10;
	output.Delay = 30;
}
void MagmaticEruption::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 200 + ((input.STR/15)+(input.Range/3.3))*1.7;
	output.Duration = 80;
	output.Delay = 80;
}
...
{
	output.Delay = 30;
	output.Damage = 250 + ((input.DEX+input.STR)/10.0 + (input.Range/3.3))*2;
	output.Tick = 3;
	output.Duration = 10;
}
void Cerberus::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Delay	= 6;
	output.Damage	= 40 + input.STR/6 + input.DEX/10;
}
void BogleB::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Delay	= 6;
	output.Damage	= 100 + input.STR/6 + input.DEX/10;
}
void BogleH::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Delay	= 6;
	output.Damage	= 100 + input.STR/6 + input.DEX/10;
}
void Manticoret::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 80 + input.INTE/5; // by bezz. 2002.12.10
	output.Delay  = max(1, 3-input.DEX/50-input.INTE/50) * 10; // by bezz. 2002.12.10
}
void MassacreMenegroth::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Delay	= 60;
	output.Damage	= 200 + input.STR/5 + input.DEX/10;
}
void PlumyBoar::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 80 + (input.INTE/5);
	output.Delay = 20;
	output.Range = 7;
}
void Cabracam::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage	= 300 + input.STR/5 + input.DEX/5;
	output.Delay = 60;
	output.Range = 7;
}
void GaeBolt::computeOutput( const SkillInput& input, SkillOutput& output )
{
	output.Damage = 100 + ((input.DEX/15) + (input.Range/3.3))*1.2;
	output.ToHit  = 60 + (input.DEX/20);
	output.Delay = 0;
	output.Range	= 8;
}
...
void TiffaugesBalberithBlue::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 200 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesBalberithRed::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 400 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesBerithBlue::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 250 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesBerithRed::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 450 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesGomoryABlue::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 400 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesGomoryARed::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 500 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesGomoryBBlue::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 250 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesGomoryBRed::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 350 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesValkyrjaBlue::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 400 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘?æ–ßπ?
}
void TiffaugesValkyrjaRed::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 600 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesKrsnikABlue::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 600 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesKrsnikARed::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 800 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesKrsnikBBlue::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 400 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesKrsnikBRed::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 600 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesPaimonBlue::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 300 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesPaimonRed::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 600 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesMorgothABlue::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 1000 + (input.INTE);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesMorgothARed::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 1200 + (input.INTE);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesMorgothBBlue::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 600 + (input.INTE);
	output.Duration = 50;
	output.Tick = 5;
}
void TiffaugesMorgothBRed::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 800 + (input.INTE);
	output.Duration = 50;
	output.Tick = 5;
}
void TiffaugesMorgothCBlue::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 500 + (input.INTE);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesMorgothCRed::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 800 + (input.INTE);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesGusionABlue::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 1000 + (input.STR+input.DEX);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesGusionARed::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 1500 + (input.STR+input.DEX);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesGusionBBlue::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 500 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesGusionBRed::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 800 + (input.STR);
	output.Delay = 0;
	output.Duration = 20;//—” ±œ‘ æ–ßπ˚
}
void TiffaugesGusionCBlue::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 600 + (input.STR);
	output.Duration = 50;
	output.Tick = 5;
}
void TiffaugesGusionCRed::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Damage = 800 + (input.STR);
	output.Duration = 50;
	output.Tick = 5;
...
void ViciousGuidance::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	if ( input.TargetType == SkillInput::TARGET_PC )
	{
		output.Damage = 50 + ((input.INTE/20)+(input.Range/3.3))*1.4;
		output.Tick = 15;
		output.Range = ( input.INTE/5 ) + ( input.SkillLevel/10 );
		output.Range = min( output.Range, 70 );
	}
	...
	{
		output.Damage = 150 + ((input.INTE/20)+(input.Range/3.3))*1.4;
		output.Tick = 10;
		output.Range = ( input.INTE/3 ) + ( input.SkillLevel/10 );
		output.Range = min( output.Range, 90 );
	}
...
void SpectorInverse::computeOutput( const SkillInput& input, SkillOutput& output )
{
	...
	output.Damage = 250 + ((input.INTE/30)+(input.Range/3.3))*1.6;
	output.Delay = 100;
	output.Range = 5;
}
...
{
	output.Delay = 4;
	output.Damage = 300 + ((input.STR/25 + input.DEX/15)+(input.Range/3.3))*2.1;
}
void FlameSpike::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 300 + ((input.INTE / 15) + (input.Range/3.3))*1.8;
	output.Delay=0;
}
void Blizzard::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 200 + ((input.INTE / 23) + (input.Range/3.3))*1.1;
	output.Duration = 15;
	output.Delay = 25;
}
void Demolisher::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 270 + ((input.INTE/20) + (input.Range/3.3))*1.2;
	output.Delay = 60;
}
void DragonHurricane::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Damage = 300 + ((input.STR/15) + (input.Range/3.3))*2.4;
	output.Range = min(200,80 + (input.STR/10) + (min(input.Range,10)*4));
	output.Tick = min(200,50 + (input.STR/10) + (min(input.Range,10)*4));
	output.Duration = (20 + min(input.Range,10))*10;
	output.Delay = 300;
}
void ShadyDouple::computeOutput(const SkillInput& input, SkillOutput& output) 
{
	output.Delay = 40;
	output.Damage = 10 + (input.INTE / 2);
}
void TrueColorofVampLoad::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Duration = 600*5;
	output.Damage = 30 + getPercentValue(input.INTE,20);
	output.Delay = 30;
}
void TheRoarOfAwakenWarrior::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Duration = 600*5;
	output.Damage = 30 + getPercentValue(input.INTE,20);
	output.Delay = 30;
}
void TheRageofGuardianSpirit::computeOutput(const SkillInput& input, SkillOutput& output)
{
	output.Duration = 600*5;
	output.Damage = 30 + getPercentValue(input.INTE,20);
	output.Delay = 30;
