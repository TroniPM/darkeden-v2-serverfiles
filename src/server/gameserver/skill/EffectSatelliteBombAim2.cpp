	...
	
{
	...
	if ( !pCastCreature->isFlag( Effect::EFFECT_CLASS_SKILL_SATELLITE_BOMB_AIM2 ) )
		return;

	// Effect를 없애고 알린다.
	pCastCreature->removeFlag( Effect::EFFECT_CLASS_SKILL_SATELLITE_BOMB_AIM2 );

	GCRemoveEffect gcRemoveEffect;
	gcRemoveEffect.setObjectID( pCastCreature->getObjectID() );
	gcRemoveEffect.addEffectList( Effect::EFFECT_CLASS_SKILL_SATELLITE_BOMB_AIM2 );
	...
	for ( int x=-2; x<=2; x++ )
	{
		for ( int y=-2; y<=2; y++ )
		{
			...
			int DamageModifier = SBADamageModify2[x+2][y+2];
			...
			for ( ; itr != oList.end(); itr++ )
			{
				...
				if ( pObject->getObjectClass() == Object::OBJECT_CLASS_CREATURE )
				{
					...
					if ( pCastCreature->isSlayer() )
					{
						...
						SkillSlot* pSkillSlot   = pSlayer->hasSkill( SKILL_SATELLITE_BOMB2 );
						if ( pSkillSlot != NULL )
...
//							Vampire* pVampire = dynamic_cast<Vampire*>(pCreature);
							...
							::setDamage( pCreature, FinalDamage, pCastCreature, SKILL_SATELLITE_BOMB2, &gcMI ); // ::추가 by Sequoia
						...
						else if ( pCreature->isMonster() )
						{
							...
							::setDamage( pMonster, FinalDamage, pCastCreature, SKILL_SATELLITE_BOMB2 );	// ::추가 by Sequoia
	...
	if ( bHit && pCastCreature->isSlayer() )
	{
		...
		{
			SkillSlot* pSkillSlot   = pSlayer->hasSkill( SKILL_SATELLITE_BOMB2 );
			SkillInfo* pSkillInfo   = g_pSkillInfoManager->getSkillInfo( SKILL_SATELLITE_BOMB2 );
	...
	gcAddEffectToTile.setEffectID( Effect::EFFECT_CLASS_SKILL_SATELLITE_BOMB_FIRE2 );
	...
	
{
	...
	else
	{
		...
		pCreature->removeFlag( Effect::EFFECT_CLASS_SKILL_SATELLITE_BOMB_AIM2 );
