...
// 뱀파이어 오브젝트 핸들러
//////////////////////////////////////////////////////////////////////////////
void SummonDeadlyGland::execute(Vampire* pVampire, ObjectID_t TargetObjectID, VampireSkillSlot* pVampireSkillSlot, CEffectID_t CEffectID)
...
// 뱀파이어 타일 핸들러
//////////////////////////////////////////////////////////////////////////////
void SummonDeadlyGland::execute(Vampire* pVampire, ZoneCoord_t X, ZoneCoord_t Y, VampireSkillSlot* pVampireSkillSlot, CEffectID_t CEffectID)
...
/*			Tile& tile = pZone->getTile(X, Y);
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && bTileCheck)
		{
			...
			Monster* pMonster = new Monster( 897 );
//			pMonster->setFlag( Effect::EFFECT_CLASS_NO_DAMAGE );
			...
			pMonster->setHP( 2000 + ( 10 * pVampire->getINT() ), ATTR_MAX );
			pMonster->setHP( 2000 + ( 10 * pVampire->getINT() ), ATTR_CURRENT );
//			pMonster->setProtection( 600 + ( 3 * pVampire->getINT() ), ATTR_CURRENT );
			...
			{
				filelog("SummonDeadlyGland.log", "%s", e.toString().c_str());
				return;
			}
			Creature* pCastCreature = dynamic_cast<Creature*>(pMonster);
			Assert(pCastCreature != NULL);

			EffectDeadlyGland* pEffectDeadlyGland = new EffectDeadlyGland(pMonster);
			pEffectDeadlyGland->setUserObjectID( pVampire->getObjectID() );
			pEffectDeadlyGland->setDamage(output.Damage);
			pEffectDeadlyGland->setDeadline(output.Duration);
			pEffectDeadlyGland->setSendEffectClass( Effect::EFFECT_CLASS_DEADLY_GLAND );
			pEffectDeadlyGland->affect();
			pMonster->addEffect(pEffectDeadlyGland);
...
SummonDeadlyGland g_SummonDeadlyGland;
