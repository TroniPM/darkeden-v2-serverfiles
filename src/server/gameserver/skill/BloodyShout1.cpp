...
// 슬레이어 셀프 핸들러
//////////////////////////////////////////////////////////////////////////////
void BloodyShout1::execute(Vampire* pSlayer, VampireSkillSlot* pSkillSlot, CEffectID_t CEffectID)
...
//		SkillLevel_t      SkillLevel = pSkillSlot->getExpLevel();
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected && !bEffected2)
		{
			...
			// 이팩트 클래스를 만들어 붙인다.
			EffectBloodyShout1* pEffect = new EffectBloodyShout1(pSlayer);
			pEffect->setDeadline(output.Duration);
			pEffect->setLevel(output.Damage);
			pSlayer->addEffect(pEffect);
			pSlayer->setFlag(Effect::EFFECT_CLASS_BLOODY_SHOUT1);
