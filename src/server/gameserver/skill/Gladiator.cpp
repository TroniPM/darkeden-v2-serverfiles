...
// 슬레이어 오브젝트 핸들러
//////////////////////////////////////////////////////////////////////////////
void Gladiator::execute(Slayer* pSlayer, ObjectID_t TargetObjectID, SkillSlot* pSkillSlot, CEffectID_t CEffectID)
	
{
	...
	try 
	{
		...
		bool bEffected   = pTargetSlayer->isFlag(Effect::EFFECT_CLASS_GLADIATOR);
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected)
		{
			...
			// 이펙트를 만들어 붙인다.
			EffectGladiator* pEffect = new EffectGladiator(pTargetCreature);
			pEffect->setDeadline(output.Duration);
			pEffect->setProtectionBonus(output.Damage);
			pEffect->setDamageBonus((output.Damage/2));
			pTargetCreature->setFlag(Effect::EFFECT_CLASS_GLADIATOR);
...
// 슬레이어 셀프 핸들러
//////////////////////////////////////////////////////////////////////////////
void Gladiator::execute(Slayer* pSlayer, SkillSlot* pSkillSlot, CEffectID_t CEffectID)
	
{
	...
	try 
	{
		...
		bool bHitRoll    = HitRoll::isSuccessMagic(pSlayer, pSkillInfo, pSkillSlot);
		bool bEffected   = pSlayer->isFlag(Effect::EFFECT_CLASS_GLADIATOR);
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected)
		{
			...
			// 이펙트를 만들어 붙인다.
			EffectGladiator* pEffect = new EffectGladiator(pSlayer);
			pEffect->setDeadline(output.Duration);
			pEffect->setProtectionBonus(output.Damage);
			pEffect->setDamageBonus((output.Damage/2)*10);
