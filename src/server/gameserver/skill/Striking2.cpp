...
// 슬레이어 오브젝트 핸들러
//////////////////////////////////////////////////////////////////////////////
void Striking2::execute(Slayer* pSlayer, ObjectID_t TargetObjectID, SkillSlot* pSkillSlot, CEffectID_t CEffectID)
	
{
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected && pItem != NULL && !bEffected2)
		{
			...
			// 이펙트 오브젝트를 생성해서 붙인다.
			EffectStriking2* pEffect = new EffectStriking2(pTargetSlayer);
			pEffect->setDeadline(output.Duration);
			pEffect->setTargetItem(pItem);
			pEffect->setDamageBonus(output.Damage);
			pTargetSlayer->setFlag(Effect::EFFECT_CLASS_STRIKING2);
...
// 슬레이어 셀프 핸들러
//////////////////////////////////////////////////////////////////////////////
void Striking2::execute(Slayer* pSlayer, SkillSlot* pSkillSlot, CEffectID_t CEffectID)
	
{
	...
	try 
	{
		...
		if (bManaCheck && bTimeCheck && bRangeCheck && bHitRoll && !bEffected && !bEffected2 && pItem != NULL)
		{
			...
			// 이펙트 오브젝트를 생성해서 붙인다.
			EffectStriking2* pEffect = new EffectStriking2(pSlayer);
			pEffect->setDeadline(output.Duration);
			pEffect->setDamageBonus(output.Damage);
			pEffect->setTargetItem(pItem);
			pSlayer->setFlag(Effect::EFFECT_CLASS_STRIKING2);
