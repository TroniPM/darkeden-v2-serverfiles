	...
	throw()
{
	...
	else if (pCreature->isMonster())
	{
		...
		exp += pMonsterInfo->getExp();
		if(pMonsterInfo->getEnhanceExpRatio() > 0){exp += getPercentValue(exp, pMonsterInfo->getEnhanceExpRatio());}
		// 5월 15일 이벤트 몬스터는 흡혈을 ?聆舊?않으므로 죽었을??전부 보상한다.
...
bool isAbleToMove(Creature* pCreature)
{
	...
	if (pCreature->isFlag(Effect::EFFECT_CLASS_COMA)
		...
		|| pCreature->isFlag(Effect::EFFECT_CLASS_FREEZE_RING1)
		|| pCreature->isFlag(Effect::EFFECT_CLASS_STUN)
...
//		|| pCreature->isFlag(Effect::EFFECT_CLASS_SANCTUARY)
		...
		|| pCreature->isFlag(Effect::EFFECT_CLASS_MIST_OF_SOUL1)
		// 2003.1.2 by Sequoia
		...
		|| pCreature->isFlag(Effect::EFFECT_CLASS_DEADLY_GLAND)
	) return false;
...
bool isAbleToUseSelfSkill(Creature* pCreature, SkillType_t SkillType)
{
	...
	{
		if (SkillType != SKILL_BITE_OF_DEATH && SkillType != SKILL_UN_TRANSFORM && SkillType != SKILL_RAPID_GLIDING && SkillType != SKILL_CHAOS_SPLITTER && SkillType != SKILL_Deadly_Claw && SkillType != SKILL_SET_AFIRE )
		{
			return false;
		}
	}
...
//		|| pCreature->isFlag(Effect::EFFECT_CLASS_SANCTUARY)
	...
	if (pCreature->isSlayer())
	{
		...
		switch (itemClass)
		{
			...
			{
				CoatZapInfo* pInfo = dynamic_cast<CoatZapInfo*>(g_pItemInfoManager->getItemInfo( pItem->getItemClass(), pItem->getItemType() ));
				if(pInfo->getRaceValue() == pCreature->getCreatureClass()  || pInfo->getRaceValue() == 7){
				return true;
				break;
				}
	...
	else if (pCreature->isVampire())
	{
		...
		switch (pItem->getItemClass())
		{
			...
			case Item::ITEM_CLASS_VAMPIREWING_ITEM :
...
// 그랜드 마스터인데 그랜드 마스터 이펙트를 클라이언트에게 보?뼉殮?곤?蔥?경우 불러준다.
// 2002. 1. 13. Sequoia
// 사용되지 않음. -.-
...
//		pVampire->tinysave(query);
	...
	// 성전환 성공이면 아우스터즈일리는 절대 없으므??슬레?潔楮?뱀파이어 테이블 모두 정보가 있다.
	Statement* pStmt = NULL;
...
/*	if ( pt.x != -1 )           // 떨어뜨리는데 성공했다면
	...
	else
	{
		...
		if ( pt.x == -1 )
		{
			filelog("FlagWar.log", "-_- 그래도 깃발 떨어뜨??자리가 없다.... X됐다.");
...
/*		pStmt->executeQuery("INSERT INTO GoldMedalCount (PlayerID, getTime) VALUES ('%s', now())",
		...
		{
			char buffer[256];
			sprintf(buffer, "삶땡꽈튿鑒綠댐돕%d몸.圈玖코휭헝꽝옘바렘寮??",
					pResult->getInt(1));
			GCSystemMessage gcSM;
