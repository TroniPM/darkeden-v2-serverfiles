...
void SkillInfo::setSkillCondition( const string& condition )
{
	if ( condition == "" ) return;
	...
	do
	{
		...
		if ( identifier == "STR" ) setSkillSTR( (int)atoi( require.c_str() ) );
		else if ( identifier == "DEX" ) setSkillDEX( (int)atoi( require.c_str() ) );
		else if ( identifier == "INT" ) setSkillINT( (int)atoi( require.c_str() ) );
...
string SkillInfo::toString() const
{
	...
	msg << "SkillInfo ("
		...
		<< ",SkillSTR:"	<< (int)m_SkillSTR
		<< ",SkillDEX:"	<< (int)m_SkillDEX
		<< ",SkillINT:"	<< (int)m_SkillINT
	...
	
{
	...
	BEGIN_DB
	{
		...
		pResult = pStmt->executeQuery("Select 
			...
			Melee, Magic, Physic, SkillPoint, LevelUpPoint, RequireSkill, Condition, ElementalDomain, CanDelete, SkillCondition
			from SkillBalance");
		...
		while (pResult->next()) 
		{
			...
			pSkillInfo->setSkillCondition(pResult->getString(++i));
...
SkillType_t SkillInfoManager::getSkillTypeByLevel(SkillDomainType_t SkillDomain , Level_t SkillLevel)
	throw(OutOfBoundException)
{
	__BEGIN_TRY
