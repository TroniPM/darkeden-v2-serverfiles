...
try {
	...
	// MonsterAI를 위해 존의 영역을 구?僅淄爭塚?사각형을 생성한다.
...
try {
	...
	for (ZoneCoord_t y=0; y<m_Height; y++)
	{
		for (ZoneCoord_t x=0; x<m_Width; x++)
		{
			...
			// 마스터 레어인 경?? block이 하나라도 안 된 곳을 찾는다.
			if ((flag & 0x07)!=0x07 && (isMasterLair() || m_ZoneID == 3002) )
			{
				m_EmptyTilePositions.push_back( BPOINT( (BYTE)x, (BYTE)y ) );
			}
	...
	throw(ProtocolException, Error)
{
	...
	if (newTile.isBlocked(pCreature->getMoveMode())
        ...
        || newTile.hasEffect() && 
		 ...
		 || newTile.getEffect(Effect::EFFECT_CLASS_TIFFAUGES_REALWORLD_LEFT_UP)
		 || newTile.getEffect(Effect::EFFECT_CLASS_TIFFAUGES_REALWORLD_RIGHT_UP)
		 || newTile.getEffect(Effect::EFFECT_CLASS_TIFFAUGES_OTHERWORLD_LEFT_UP)  
		 || newTile.getEffect(Effect::EFFECT_CLASS_TIFFAUGES_OTHERWORLD_RIGHT_UP))
		|| oldTile.getEffect(Effect::EFFECT_CLASS_SANCTUARY) != NULL
		|| oldTile.getEffect(Effect::EFFECT_CLASS_TIFFAUGES_REALWORLD_LEFT_UP) != NULL
		|| oldTile.getEffect(Effect::EFFECT_CLASS_TIFFAUGES_REALWORLD_RIGHT_UP) != NULL
		|| oldTile.getEffect(Effect::EFFECT_CLASS_TIFFAUGES_OTHERWORLD_LEFT_UP) != NULL
		|| oldTile.getEffect(Effect::EFFECT_CLASS_TIFFAUGES_OTHERWORLD_RIGHT_UP) != NULL
		)
	{
		GCMoveError 	gcMoveError(cx, cy); 
		pPlayer->sendPacket(&gcMoveError);
			...
			if (pCreature->isSlayer() || pCreature->isOusters())
			{
				...
				for (; itr != objectList.end() && (*itr)->getObjectPriority() <= OBJECT_PRIORITY_BURROWING_CREATURE ; itr++) 
				{
					if ((*itr)->getObjectClass() == Object::OBJECT_CLASS_CREATURE)
					{
						...
						{
							// Detect Invisibility 이펙트가 있거??뱀??이어?볼 수 있다
...
//#ifdef __XMAS_EVENT_CODE__
			Monster* pMonster = dynamic_cast<Monster*>(pCreature);
					...
					if(getZoneID() == 1801 || (isDynamicZone() && getDynamicZone()->getTemplateZoneID() == 1803)){
					pMonster->setEffectClass(595);
					EffectMonsterMasterEffect1* pEffect = new EffectMonsterMasterEffect1(pMonster);
					pEffect->setDeadline(999999);
					pMonster->addEffect( pEffect );
					pMonster->setFlag(pEffect->getSendEffectClass());
					}
			if ( isDynamicZone() )
			{
...
// 존재하지 않을 경우인데.. 이때 예외를 던짐으로써 그 처리를 상?㎰“?맡기면
// 될 듯...
//--------------------------------------------------------------------------------
	...
	{ 
		...
		if (IClass == Item::ITEM_CLASS_CORPSE) 
		{
			...
			if (itemType == SLAYER_CORPSE)
			{
				...
				if ( isDynamicZone() )DelayTime = DELAY_MASTER_LAIR_DECAY_CORPSE;
			}
			...
			else if (itemType == MONSTER_CORPSE)
			{
				...
				if ( isDynamicZone() )DelayTime = DELAY_MASTER_LAIR_DECAY_MASTER_CORPSE;
...
//				pEffectDecayCorpse->setNextTime(999999);
		...
		else
		{
			...
			else
			{
				...
				if ( isDynamicZone() )DelayTime = DELAY_MASTER_LAIR_DECAY_ITEM;
...
// broadcast packet
//
// 특정 ?맙?존재하는, owner를 제외한 모든 PC 에게 지정된 패킷을 전송한다.
//
//--------------------------------------------------------------------------------
		...
		for (ZoneCoord_t iy = max(0, cy - maxViewportUpperHeight - 1), endy = min(m_Height - 1, cy + maxViewportLowerHeight + 1) ; iy <= endy ; iy++) 
		{
			...
			for (slist<Object*>::const_iterator itr = objectList.begin() ; itr != objectList.end() ; itr++) 
			{
				...
				if (oldVS == OUT_OF_SIGHT && newVS != OUT_OF_SIGHT)
				{
					...
					// owner 에게 전송한다. 
					// *NOTES*
					// 가장 출현 확률이 높은 객체 CLASS 가 case 앞부?極?나와야 한다.
					//--------------------------------------------------------------------------------
...
//			bool bCanSeeThere = (pPC->getVisionState(ix, iy) >= IN_SIGHT);	// 순수 시야만으로 볼 수 있나?
			...
			for (slist<Object*>::const_iterator itr = objectList.begin() ; itr != objectList.end() ; itr++) 
			{
				...
				//--------------------------------------------------------------------------------
				//
				// 각 객체??OBJECT CLASS에 따라서 적합한 GCAddXXX 패킷을 만들어서
				// owner 에게 전송한다. 
				//
				// *NOTES*
				//
								...
								case Creature::CREATURE_CLASS_VAMPIRE :
									{
										if (bCanSee)
										{
											// PC가 ObservingEye 이펙트를 가지??있다면 이펙트를 가져온다.
//											EffectObservingEye* pEffectObservingEye = NULL;
//											if ( pPC->isFlag( Effect::EFFECT_CLASS_OBSERVING_EYE ) )
//											{
//												pEffectObservingEye = dynamic_cast<EffectObservingEye*>(pPC->findEffect( Effect::EFFECT_CLASS_OBSERVING_EYE ) );
...
//////////////////////////////////////////////////////////////////////////////
// 몬스터가 존의 (x,y)에 새로 리젠되었을 경우, 시야 영역안에 존재하는 ?醍?PC들에게
						...
						case Object::OBJECT_CLASS_CREATURE :
							{
								...
								switch (pCreature->getCreatureClass()) 
								{
									...
									case Creature::CREATURE_CLASS_SLAYER :
										{
											//--------------------------------------------------------------------------------
											// 이전 ?쪄?【??보이지 않다가, 이번 좌표에서 새로 보이게 된 크리처만
...
//													gcAddSlayer.setEffectInfo(pSlayer->getEffectInfo());
											...
											// 즉 보이지 않는 경우에만 GCAddXXX 패킷을 전송한다. 이?린?하지 않으면, PC
...
//															gcAddVampire.setEffectInfo(pVampire->getEffectInfo());
	...
	//													}
											...
											//--------------------------------------------------------------------------------
											// Q(x2,y2)가 이 크리처의 ?첸?사각형의 경계에 위치하면서, P(x1,y1)은 사각형의 외부, 
					...
					do		
					{
						...
						if (OClass == Object::OBJECT_CLASS_CREATURE)
						{
							...
							else if (CClass == Creature::CREATURE_CLASS_NPC)
							{
								...
								//--------------------------------------------------------------------------------
								//
								// 이전 좌표에서는 ???澯뵀拷?볼 수 없었으나, 도착 좌표에서 이 몬스터를 보게 될
								// 경우 GCAddMonster 패킷을 전송한다.
								//
								//--------------------------------------------------------------------------------
								if (prevVisionState == OUT_OF_SIGHT && curVisionState >= IN_SIGHT) 
...
//		isMonsterInvisibility = pMonster->isFlag(Effect::EFFECT_CLASS_INVISIBILITY);
	...
	//////////////////////////////////////////////////////////////////////////////
	// 시야 영역의 상하좌우 모두 + 1 씩 증가?쳔껜? 
	// 이유는 방향에 따른 ON_SIGHT 영역이 증가?풉?때문이다.
	//////////////////////////////////////////////////////////////////////////////
	for (ZoneCoord_t ix = max(0, x2 - maxViewportWidth - 1), endx = min(m_Width - 1, x2 + maxViewportWidth + 1) ; ix <= endx ; ix++) 
	{
		for (ZoneCoord_t iy = max(0, y2 - maxViewportUpperHeight - 1), endy = min(m_Height - 1, y2 + maxViewportLowerHeight + 1) ; iy <= endy ; iy++) 
	...
	
{
	...
	try
	{
		...
		int hour = VSTime::currentTime().hour();
		int min = VSTime::currentTime().minute();
		int Sec = VSTime::currentTime().second();
		bool m_bExpFreeUser = false;
for(int i=0; i < 3; i++){
	if ( !m_bExpFreeUser && hour == ExpTime[0][i] && min == 0 && Sec == 0 )
	{
		m_bExpFreeUser = true;
		GCSystemMessage gcSystemMessage;
		char msg[100];

		sprintf(msg, "현재 %d 시부터 %d 시까지 경험치를 2배 획득 하실수 있습니다.", hour, hour+1);

		gcSystemMessage.setMessage( msg );
	    	broadcastPacket( &gcSystemMessage );
	}
}
...
// 사용한다. 웬만하면, 타입을 알?틂??getCreature(Creature::CreatureClass,ObjectID_t) 
// 메쏘드를 사용하도록 한다.
//////////////////////////////////////////////////////////////////////////////
...
//
// 이 메쏘드는 찾고자 하는 크리처의 타입(PC,NPC,Monster)를 모를 경우??사용한다.
// 웬만하면, 타입을 알아?뻤?getCreature(Creature::CreatureClass,Name) 
// 메쏘드를 사용하도록 한다.
//////////////////////////////////////////////////////////////////////////////
	...
	
{
	...
	////////////////////////////////////////////////////////////
	// 시야 영역의 상하좌우 모두 + 1 씩 증가시킨다. 
	// 이유는 ?戀藪??醯?ON_SIGHT 영역이 증가되기 때문이다.
	////////////////////////////////////////////////////////////
	...
	throw()
{
	...
	// 뱀파이어의 능력에 따라 들어갈 수 있는 인원과, 지속 ?챨＠?계산한다.
...
//	if ( m_ZoneID == 3001 || m_ZoneID == 71 || m_ZoneID == 72 || m_ZoneID == 73 )
	...
	EffectExpTime* pEffectExpTime = new EffectExpTime( this );
	pEffectExpTime->setDeadline(99999999);
	registerObject( pEffectExpTime );
	addEffect( pEffectExpTime );
	...
	/*if(isDynamicZone() && getDynamicZone()->getTemplateZoneID() == 9009){
		EffectContinualGroundAttack* pEffect = new EffectContinualGroundAttack( this, Effect::EFFECT_CLASS_GROUND_ATTACK, 3 );
		pEffect->setDeadline(99999999);
		pEffect->setNumber( 7, 11 );
