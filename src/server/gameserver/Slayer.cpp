    ...
    
{
	...
	try {
		...
		// 비정상적인 경우라고 해도, 
		// GamePlayer::disconnect()에서 Zone::deleteCreature() 함수를 부르??되므로,
...
//		m_SkillDomainExps[i]   = 0;
	...
	BEGIN_DB
	{
		...
		pResult = pStmt->executeQuery(
			...
			SMSCharge, KillPoint, ShopColor, AttackBloodBurstPoint, DefenseBloodBurstPoint, ADBloodBurstPoint
...
//		for (int j = 0; j < 4; j++) setHotKey(j, pResult->getInt(++i));
		...
		m_pStore->setShopColor(getShopColor());
	...
	
{
	...
	{
		/*
		// StringStream 안?껑? by sigi. 2002.5.9
		StringStream sql;
	...
	
{
	...
	// 현재 기획에서는...능력치가 모자라더라도 아이템을 무조건 사용??수는
	...
	
{
	...
	// 있다. 하지만 아이템에 의한 능력치가 적용이 되지 않는다. 
	// 그러므로 일단 아이템을 해당하는 장착창에다 집어넣는다.
	// ?煐?무기일 경우에는 양손 장착창에다 하나의 아이템 포인터???年?..
	if (isTwohandWeapon(pItem))
	{
		// 양손에 아이템을 들고 있을 경우
		if (isWear(WEAR_RIGHTHAND) && isWear(WEAR_LEFTHAND))
	...
	
{
	...
	{
		// 해당 위치에 반지도 있어야 된다.
		if ( m_pWearItem[part-WEAR_BRACELET_ZAP1+WEAR_WRIST1]==NULL ) return false;
	}
	if (part == WEAR_NECKLACE_ZAP)
	{
		// 해당 위치에 반지도 있어야 된다.
		if ( m_pWearItem[part-WEAR_NECKLACE_ZAP+WEAR_NECK]==NULL ) return false;
	}
	if (part == WEAR_COAT_ZAP)
	{
		// 해당 위치에 반지도 있어야 된다.
		if ( m_pWearItem[part-WEAR_COAT_ZAP+WEAR_BODY]==NULL ) return false;
	}
	if (part == WEAR_LEG_ZAP)
	{
		// 해당 위치에 반지도 있어야 된다.
		if ( m_pWearItem[part-WEAR_LEG_ZAP+WEAR_LEG]==NULL ) return false;
	}
	...
	if (part == WEAR_HEAD_ZAP)
	{
		// 해당 위치에 반지도 있어야 된다.
		if ( m_pWearItem[part-WEAR_HEAD_ZAP+WEAR_HEAD]==NULL ) return false;
	}
	if (part == WEAR_BELT_ZAP)
	{
		// 해당 위치에 반지도 있어야 된다.
		if ( m_pWearItem[part-WEAR_BELT_ZAP+WEAR_BELT]==NULL ) return false;
	}
	if (part == WEAR_HAND3_ZAP)
	{
		// 해당 위치에 반지도 있어야 된다.
		if ( m_pWearItem[part-WEAR_HAND3_ZAP+WEAR_HAND3]==NULL ) return false;
	}
	if (part == WEAR_FOOT_ZAP)
	{
		// 해당 위치에 반지도 있어야 된다.
		if ( m_pWearItem[part-WEAR_FOOT_ZAP+WEAR_FOOT]==NULL ) return false;
	}
...
/*	if ( m_pZone != NULL && m_pZone->isDynamicZone() && m_pZone->getDynamicZone()->getTemplateZoneID() == 4003 )
	...
	// 기본 아이템의 능력치 총합 요구치가 300이 넘으면 옵션을 포함한 ??구치?435까지 올라갈 수 있다.
	...
	SkillDomainType_t ReqDomainType  = pItemInfo->getReqDomainType();
	...
	
{
	...
	m_SlayerInfo.setAdvancementLevel(getAdvancementClassLevel());
	//m_SlayerInfo.setMotorItemType(getMotorcycleType());
	m_SlayerInfo.setJacketItemType(getJacketItemType());
	return m_SlayerInfo;
...
/*
			...
			pGearSlotInfo->setOptionType2(pItem->getOptionTypeList2());
			...
			pGearSlotInfo->setHeroOption(pItem->getHeroOption());
			pGearSlotInfo->setHeroOptionAttr(pItem->getHeroOptionAttr());
...
// get Inventory Info
//////////////////////////////////////////////////////////////////////////////
InventoryInfo* Slayer::getInventoryInfo2() const
	...
	
{
	...
	// 스킬 핸들러에서 쿼리 숫자를 줄?堅??㎸漫?10으로 나누는 부분들은,
	// 서버 다운이 되지 않고, 정상적으로 로그아웃하는 경우에 
	// 세이브를 명시적으로 해주지 않으면 10 이하 올라간 ?觀隙?날아가 버리게 된다.
	// 그러므로 여기서 세이브를 해 준?? 
	...
	*/
	...
	BEGIN_DB
	{
		...
		pStmt->executeQuery( "UPDATE Slayer SET STRGoalExp=%lu, DEXGoalExp=%lu, INTGoalExp=%lu, BladeGoalExp=%lu, SwordGoalExp=%lu, GunGoalExp=%lu, EnchantGoalExp=%lu, HealGoalExp=%lu, ETCGoalExp=%lu, Alignment=%d, Fame=%ld, Rank=%d, RankGoalExp=%lu, AdvancementClass=%u, AdvancementGoalExp=%lu, AdvancedSTR=%u, AdvancedDEX=%u, AdvancedINT=%u, Bonus=%u, KillPoint=%ld, AttackBloodBurstPoint = %d, DefenseBloodBurstPoint = %d, ADBloodBurstPoint = %d WHERE Name='%s'",
								...
								m_AdvancedSTR, m_AdvancedDEX, m_AdvancedINT, m_AdvancedAttrBonus, m_KillPoint, m_AttackBloodBurstPoint, m_DefenseBloodBurstPoint, m_PartyBloodBurstPoint, m_Name.c_str());
