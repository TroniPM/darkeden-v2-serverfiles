	...
	
{
	...
	BEGIN_DB
	{
		...
		*/
		...
		pStmt->executeQuery( "INSERT INTO ARObject (ItemID, ObjectID, ItemType, OwnerID, Storage, StorageID, X, Y, OptionType, OptionType2, Durability, BulletCount, HeroOption, HeroOptionAttr, Grade, ItemFlag) VALUES(%ld, %ld, %d, '%s', %d, %ld, %d, %d, '%s', '%s', %d, %d, %d, %d, %d, %d)",
							m_ItemID, m_ObjectID, getItemType(), ownerID.c_str(), (int)storage, storageID, (int)x, (int)y, optionField.c_str(), optionField2.c_str(), getDurability(), (int)getBulletCount(), (int)getHeroOption(),(int)getHeroOptionAttr(), (int)getGrade(), (int)m_CreateType);
	...
	
{
	...
	BEGIN_DB
	{
		...
		*/
		...
		setOptionTypeToField( getOptionTypeList(), optionField );
		setOptionTypeToField( getOptionTypeList2(), optionField2 );
		pStmt->executeQuery( "UPDATE ARObject SET ObjectID = %ld, ItemType = %d, OwnerID='%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, OptionType='%s', OptionType2='%s', Durability=%d, EnchantLevel=%d, BulletCount=%d, Silver=%d, HeroOption = %d, HeroOptionAttr = %d, Grade=%d WHERE ItemID=%ld",
								m_ObjectID, getItemType(), ownerID.c_str(), (int)storage, storageID, (int)x, (int)y, optionField.c_str(), optionField2.c_str(), getDurability(), (int)getEnchantLevel(), (int)getBulletCount(), (int)getSilver(), (int)getHeroOption(), (int)getHeroOptionAttr(), (int)getGrade(), m_ItemID);
	...
	
{
	...
	BEGIN_DB
	{
		...
		*/
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, OptionType2, Durability, BulletCount, Silver, EnchantLevel, HeroOption, HeroOptionAttr, Grade, ItemFlag FROM ARObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)",
		...
		while (pResult->next())
		{
			try {
				...
				pAR->setOptionType2(optionTypes2);
				...
				pAR->setHeroOption(pResult->getInt(++i));
				pAR->setHeroOptionAttr(pResult->getInt(++i));
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelt)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pAR))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pAR);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pAR);
										}
	...
	
{
	...
	BEGIN_DB
	{
		...
		sql << "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, OptionType2, Durability, BulletCount, Silver, EnchantLevel, HeroOption, HeroOptionAttr, ItemFlag FROM ARObject"
