	...
	
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		pStmt->executeQuery( "UPDATE TrouserObject SET ObjectID=%ld, ItemType=%d, OwnerID='%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, OptionType='%s', OptionType2='%s',Durability=%d, Grade=%d, EnchantLevel=%d, HeroOption=%d, HeroOptionAttr=%d WHERE ItemID=%ld",
								m_ObjectID, getItemType(), ownerID.c_str(), (int)storage, storageID, (int)x, (int)y, optionField.c_str(), optionField2.c_str(), getDurability(), (int)getGrade(), (int)getEnchantLevel(),(int)getHeroOption(),(int)getHeroOptionAttr(), m_ItemID );
	...
	
{
	...
	BEGIN_DB 
	{
		...
		*/
	    ...
	    Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y,OptionType, OptionType2, Durability, Grade, EnchantLevel, HeroOption, HeroOptionAttr, ItemFlag FROM TrouserObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)",
		...
		while (pResult->next())
		{
			try {
				...
				pTrouser->setOptionType2(optionTypes2);

				pTrouser->setDurability(pResult->getInt(++i));
				pTrouser->setGrade(pResult->getInt(++i));
				pTrouser->setEnchantLevel(pResult->getInt(++i));
pTrouser->setHeroOption(pResult->getInt(++i));
pTrouser->setHeroOptionAttr(pResult->getInt(++i));
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelts)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pTrouser))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pTrouser);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pTrouser);
										}
	...
	
{
	...
	BEGIN_DB 
	{
		...
		sql << "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y,"
			<< " OptionType, OptionType2, Durability, EnchantLevel, HeroOption, HeroOptionAttr, ItemFlag FROM TrouserObject"
