	...
	
{
	...
	BEGIN_DB 
	{
		...
		sql << "INSERT INTO FasciaObject "
			...
			<<(int)x << ", " <<(int)y << ", '" << optionField.c_str() << "', '" << optionField2.c_str() << "', " << (int)getGrade() << ", " << (int)getHeroOption() << "," << (int)getHeroOptionAttr() << "," << (int)m_CreateType << ")";
	...
	
{
	...
	BEGIN_DB 
	{
		...
		setOptionTypeToField( getOptionTypeList(), optionField );
		setOptionTypeToField( getOptionTypeList2(), optionField2 );
		pStmt->executeQuery( "UPDATE FasciaObject SET ObjectID=%ld, ItemType=%d, OwnerID='%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, OptionType='%s', OptionType2='%s', Grade=%d, EnchantLevel=%d, HeroOption=%d, HeroOptionAttr=%d WHERE ItemID=%ld",
	...
	
{
	...
	BEGIN_DB 
	{
		...
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, OptionType2, Grade, EnchantLevel, HeroOption, HeroOptionAttr, ItemFlag FROM FasciaObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)", pCreature->getName().c_str() );
		...
		while (pResult->next())
		{
			try {
				...
				pFascia->setOptionType2(optionTypes2);

				pFascia->setGrade(pResult->getInt(++i));
				pFascia->setEnchantLevel(pResult->getInt(++i));
pFascia->setHeroOption(pResult->getInt(++i));
pFascia->setHeroOptionAttr(pResult->getInt(++i));
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelt)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pFascia))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pFascia);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pFascia);
										}
