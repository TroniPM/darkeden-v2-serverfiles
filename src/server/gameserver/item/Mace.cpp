	...
	throw(Error)
{
	...
	BEGIN_DB 
	{
		...
		sql << "INSERT INTO MaceObject "
			...
			<<(int)x << ", " <<(int)y << ", '" << optionField.c_str() << "', '" << optionField2.c_str() << "', " << getDurability() << ", " << (int)getGrade() << "," << (int)getHeroOption() << "," << (int)getHeroOptionAttr() << ", " << (int)m_CreateType << ")";
	...
	throw(Error)
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		setOptionTypeToField( getOptionTypeList(), optionField );
		setOptionTypeToField( getOptionTypeList2(), optionField2 );
		pStmt->executeQuery( "UPDATE MaceObject SET ObjectID=%ld, ItemType=%d, OwnerID='%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, OptionType='%s', OptionType2='%s', Durability=%d, EnchantLevel=%d, Silver=%d, Grade=%d, HeroOption=%d, HeroOptionAttr=%d WHERE ItemID=%ld",
								m_ObjectID, getItemType(), ownerID.c_str(), (int)storage, storageID, (int)x, (int)y, optionField.c_str(), optionField2.c_str(), getDurability(), (int)getEnchantLevel(), (int)getSilver(), (int)getGrade(), (int)getHeroOption(),(int)getHeroOptionAttr(),m_ItemID );
	...
	throw(Error)
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, OptionType2, Durability, EnchantLevel, Silver, Grade, HeroOption, HeroOptionAttr, ItemFlag FROM MaceObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)",
		...
		while (pResult->next())
		{
			try {
				...
				pMace->setOptionType2(optionTypes2);
...
pMace->setHeroOption(pResult->getInt(++i));
pMace->setHeroOptionAttr(pResult->getInt(++i));
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelt)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pMace))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pMace);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pMace);
										}
	...
	throw(Error)
{
	...
	BEGIN_DB 
	{
		...
		sql << "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y,"
			<< " OptionType, OptionType2, Durability, EnchantLevel, Silver, HeroOption, HeroOptionAttr, ItemFlag FROM MaceObject"
