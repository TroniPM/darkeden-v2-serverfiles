	...
	
{
	...
	BEGIN_DB
	{
		...
		*/
		...
		setOptionTypeToField( getOptionTypeList(), optionField );
		setOptionTypeToField( getOptionTypeList2(), optionField2 );
		pStmt->executeQuery("UPDATE BladeObject SET ObjectID=%ld, ItemType=%d, OwnerID= '%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, OptionType='%s', OptionType2='%s', Durability=%d, EnchantLevel=%d, Silver=%d, Grade=%d, HeroOption=%d, HeroOptionAttr=%d WHERE ItemID=%ld",
	...
	
{
	...
	BEGIN_DB
	{
		...
		*/
		...
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, OptionType2, Durability, EnchantLevel, Silver, Grade, HeroOption, HeroOptionAttr, ItemFlag FROM BladeObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4,7, 9)",
		...
		while (pResult->next())
		{
			try {
				...
				pBlade->setOptionType2(optionTypes2);
				...
				pBlade->setHeroOption(pResult->getInt(++i));
				pBlade->setHeroOptionAttr(pResult->getInt(++i));
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelt)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pBlade))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pBlade);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pBlade);
										}
	...
	
{
	...
	BEGIN_DB
	{
		...
		sql << "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y,"
			<< " OptionType, OptionType2, Durability, EnchantLevel, HeroOption, HeroOptionAttr, Silver, ItemFlag FROM BladeObject"
