...
public:
	CoupleRing() throw();
	CoupleRing(ItemType_t itemType, const list<OptionType_t>& optionType) throw();
...
public:
	virtual Item* createItem(ItemType_t ItemType, const list<OptionType_t>& OptionType) throw() { return new CoupleRing(ItemType,OptionType); }
};
