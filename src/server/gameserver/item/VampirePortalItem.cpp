	...
	
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, Charge, TargetZID, TargetX, TargetY FROM VampirePortalItemObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)",
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelts)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pVampirePortalItem))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pVampirePortalItem);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pVampirePortalItem);
										}
