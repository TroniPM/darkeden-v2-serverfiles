	...
	
{
	...
	BEGIN_DB 
	{
		...
		setOptionTypeToField( getOptionTypeList(), optionField );
		setOptionTypeToField( getOptionTypeList2(), optionField2 );
		pStmt->executeQuery( "UPDATE DermisObject SET ObjectID=%ld, ItemType=%d, OwnerID='%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, OptionType='%s', OptionType2='%s', Grade=%d, EnchantLevel=%d, HeroOption=%d, HeroOptionAttr=%d WHERE ItemID=%ld", 
	...
	
{
	...
	BEGIN_DB 
	{
		...
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y,OptionType, OptionType2, Grade, EnchantLevel, HeroOption, HeroOptionAttr, ItemFlag FROM DermisObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)",
		...
		while (pResult->next())
		{
			try {
				...
				pDermis->setOptionType2(optionTypes2);

				pDermis->setGrade(pResult->getInt(++i));
				pDermis->setEnchantLevel(pResult->getInt(++i));
pDermis->setHeroOption(pResult->getInt(++i));
pDermis->setHeroOptionAttr(pResult->getInt(++i));
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelt)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pDermis))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pDermis);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pDermis);
										}
