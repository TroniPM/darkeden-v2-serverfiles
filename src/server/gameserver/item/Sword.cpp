	...
	throw(Error)
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		setOptionTypeToField( getOptionTypeList(), optionField );
		setOptionTypeToField( getOptionTypeList2(), optionField2 );
		pStmt->executeQuery( "UPDATE SwordObject SET ObjectID=%ld, ItemType=%d, OwnerID='%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, OptionType='%s', OptionType2='%s', Durability=%d, EnchantLevel=%d, Silver=%d, Grade=%d, HeroOption=%d, HeroOptionAttr=%d WHERE ItemID=%ld",
	...
	throw(Error)
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y,OptionType, OptionType2, Durability, EnchantLevel, Silver, Grade, HeroOption, HeroOptionAttr, ItemFlag FROM SwordObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)",
		...
		while (pResult->next())
		{
			try {
				...
				pSword->setOptionType2(optionTypes2);
...
pSword->setHeroOption(pResult->getInt(++i));
pSword->setHeroOptionAttr(pResult->getInt(++i));
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelts)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pSword))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pSword);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pSword);
										}
	...
	throw(Error)
{
	...
	BEGIN_DB 
	{
		...
		sql << "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y,"
			<< " OptionType, OptionType2, Durability, EnchantLevel, Silver, HeroOption, HeroOptionAttr, ItemFlag FROM SwordObject"
