	...
	
{
	...
	BEGIN_DB 
	{
		...
		sql << "INSERT INTO EffectItemObject "
			<< "(ItemID,  ObjectID, ItemType, OwnerID, Storage, StorageID, X, Y, Num, HeroOption, ItemFlag) VALUES(" 
			<< m_ItemID << ", "
			<< m_ObjectID << ", " << getItemType() << ", '" << ownerID << "', " 
			<<(int)storage << ", " << storageID << ", " <<(int)x << ", " <<(int)y << ", " 
			<< (int)getNum() << ", " << (int)getHeroOption() << "," << (int)m_CreateType << ")";
	...
	
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		pStmt->executeQuery( "UPDATE EffectItemObject SET ObjectID=%ld, ItemType=%d, OwnerID='%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, Num=%d, HeroOption=%d WHERE ItemID=%ld",
								m_ObjectID, getItemType(), ownerID.c_str(), (int)storage, storageID, (int)x, (int)y, (int)getNum(), (int)getHeroOption(), m_ItemID );
	...
	
{
	...
	BEGIN_DB 
	{
        ...
        pResult = pStmt->executeQuery(
			"SELECT ItemType, Name, EName, Price, Volume, Weight, Ratio, EffectClass, TimeSec, NotEffectClass FROM EffectItemInfo"
		);
		...
		while (pResult->next()) 
		{
			...
			pEffectItemInfo->setNotEffectClass(pResult->getString(++i));
	...
	
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, Num, ItemFlag, HeroOption FROM EffectItemObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)",
		...
		while (pResult->next())
		{
			try {
				...
				pEffectItem->setHeroOption(pResult->getInt(++i));
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelt)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pEffectItem))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pEffectItem);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pEffectItem);
										}
	...
	
{
	...
	BEGIN_DB 
	{
		...
		sql << "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, Num, ItemFlag,Silver FROM EffectItemObject"
