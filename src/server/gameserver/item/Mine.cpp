	...
	
{
	...
	BEGIN_DB
	{
		...
		*/
		...
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, Num FROM MineObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)",
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelt)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pMine))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pMine);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pMine);
										}
