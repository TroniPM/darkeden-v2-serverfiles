	...
	
{
	...
	BEGIN_DB 
	{
		...
		sql << "INSERT INTO SGObject "
			...
			<< " X, Y, OptionType, OptionType2, Durability, BulletCount, Grade, ItemFlag)"
			<< " VALUES(" 
			<< m_ItemID << ", "
			<< m_ObjectID << ", " << getItemType() << ", '" << ownerID << "', " <<(int)storage << ", " << storageID << ", " 
			<<(int)x << ", " <<(int)y << ", '" << optionField.c_str() << "', '" << optionField2.c_str() << "', " << getDurability() << ", " <<(int)getBulletCount() << ", " << (int)getGrade() << "," << (int)m_CreateType << ")";
	...
	
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		setOptionTypeToField( getOptionTypeList(), optionField );
		setOptionTypeToField( getOptionTypeList2(), optionField2 );
		pStmt->executeQuery( "UPDATE SGObject SET ObjectID=%ld, ItemType=%d, OwnerID='%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, OptionType='%s', OptionType2='%s',  Durability=%d, EnchantLevel=%d, BulletCount=%d, Silver=%d, Grade=%d WHERE ItemID=%ld",
								m_ObjectID, getItemType(), ownerID.c_str(), (int)storage, storageID, (int)x, (int)y, optionField.c_str(), optionField2.c_str(), getDurability(), (int)getEnchantLevel(), (int)getBulletCount(), (int)getSilver(), (int)getGrade(), m_ItemID );
	...
	
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y,OptionType, OptionType2, Durability, EnchantLevel, BulletCount, Silver, Grade, ItemFlag FROM SGObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)",
		...
		while (pResult->next())
		{
			try {
				...
				pSG->setOptionType2(optionTypes2);
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelts)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pSG))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pSG);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pSG);
										}
	...
	
{
	...
	BEGIN_DB 
	{
		...
		sql << "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y,"
			<< " OptionType, OptionType2, Durability, EnchantLevel, BulletCount, Silver, ItemFlag FROM SGObject"
