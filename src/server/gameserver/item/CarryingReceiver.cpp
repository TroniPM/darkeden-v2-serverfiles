	...
	throw(Error)
{
	...
	BEGIN_DB 
	{
		...
		setOptionTypeToField( getOptionTypeList(), optionField );
		setOptionTypeToField( getOptionTypeList2(), optionField2 );
		pStmt->executeQuery( "UPDATE CarryingReceiverObject SET ObjectID=%ld, ItemType=%d, OwnerID='%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, OptionType='%s', OptionType2='%s', Grade=%d, EnchantLevel=%d, HeroOption=%d, HeroOptionAttr=%d WHERE ItemID=%ld",
	...
	throw(Error)
{
	...
	BEGIN_DB 
	{
		...
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, OptionType2, Grade, EnchantLevel, HeroOption, HeroOptionAttr, ItemFlag FROM CarryingReceiverObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)",
		...
		while (pResult->next())
		{
			try {
				...
				pCarryingReceiver->setOptionType2(optionTypes2);

				pCarryingReceiver->setGrade(pResult->getInt(++i));
				pCarryingReceiver->setEnchantLevel(pResult->getInt(++i));
				pCarryingReceiver->setHeroOption(pResult->getInt(++i));
				pCarryingReceiver->setHeroOptionAttr(pResult->getInt(++i));
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelt)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pCarryingReceiver))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pCarryingReceiver);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pCarryingReceiver);
										}
