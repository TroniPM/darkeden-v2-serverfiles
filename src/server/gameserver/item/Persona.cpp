	...
	
{
	...
	BEGIN_DB 
	{
		...
		setOptionTypeToField( getOptionTypeList(), optionField );
		setOptionTypeToField( getOptionTypeList2(), optionField2 );
		pStmt->executeQuery( "UPDATE PersonaObject SET ObjectID=%ld, ItemType=%d, OwnerID='%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, OptionType='%s', OptionType2='%s', Durability=%d, Grade=%d, EnchantLevel=%d, HeroOption=%d, HeroOptionAttr=%d WHERE ItemID=%ld",
									m_ObjectID, getItemType(), ownerID.c_str(), (int)storage, storageID, (int)x, (int)y, optionField.c_str(), optionField2.c_str(), getDurability(), getGrade(), (int)getEnchantLevel(),(int)getHeroOption(),(int)getHeroOptionAttr(),
 m_ItemID );
	...
	
{
	...
	BEGIN_DB 
	{
		...
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, OptionType2, Durability, Grade, EnchantLevel, HeroOption, HeroOptionAttr, ItemFlag FROM PersonaObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)",
		...
		while (pResult->next())
		{
			try {
				...
				pPersona->setOptionType2(optionTypes2);

				pPersona->setDurability(pResult->getInt(++i));
				pPersona->setGrade(pResult->getInt(++i));
				pPersona->setEnchantLevel(pResult->getInt(++i));
pPersona->setHeroOption(pResult->getInt(++i));
pPersona->setHeroOptionAttr(pResult->getInt(++i));
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelts)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pPersona))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pPersona);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pPersona);
										}
