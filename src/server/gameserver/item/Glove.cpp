	...
	
{
	...
	BEGIN_DB 
	{
		...
		sql << "INSERT INTO GloveObject "
			...
			<<(int)x << ", " <<(int)y << ", '" << optionField.c_str() << "', '" << optionField2.c_str() << "', "<< getDurability() << ", " << (int)getGrade() << ", " << (int)getHeroOption() << "," << (int)getHeroOptionAttr() << ",
" << (int)m_CreateType << ")";
	...
	
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		setOptionTypeToField( getOptionTypeList(), optionField );
		setOptionTypeToField( getOptionTypeList2(), optionField2 );
		pStmt->executeQuery( "UPDATE GloveObject SET ObjectID=%ld, ItemType=%d, OwnerID='%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, OptionType='%s', OptionType2='%s', Durability=%d, Grade=%d, EnchantLevel = %d, HeroOption=%d, HeroOptionAttr=%d WHERE ItemID=%ld",
	...
	
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, OptionType2, Durability, Grade, EnchantLevel, HeroOption, HeroOptionAttr, ItemFlag FROM GloveObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)",
		...
		while (pResult->next())
		{
			try {
				...
				pGlove->setOptionType2(optionTypes2);

				pGlove->setDurability(pResult->getInt(++i));
				pGlove->setGrade(pResult->getInt(++i));
				pGlove->setEnchantLevel(pResult->getInt(++i));
pGlove->setHeroOption(pResult->getInt(++i));
pGlove->setHeroOptionAttr(pResult->getInt(++i));
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelt)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pGlove))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pGlove);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pGlove);
										}
	...
	
{
	...
	BEGIN_DB 
	{
		...
		sql << "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y,"
			<< " OptionType, OptionType2, Durability, EnchantLevel, HeroOption, HeroOptionAttr, ItemFlag FROM GloveObject"
