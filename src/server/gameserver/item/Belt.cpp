	...
	
{
	...
	BEGIN_DB
	{
		...
		*/
		...
		pStmt->executeQuery( "INSERT INTO BeltObject (ItemID,  ObjectID, ItemType, OwnerID, Storage, StorageID, X, Y, OptionType, OptionType2, Durability, Grade, HeroOption, HeroOptionAttr, ItemFlag) VALUES(%ld, %ld, %d, '%s', %d, %ld, %d, %d, '%s', '%s', %d, %d, %d, %d, %d)",
								m_ItemID, m_ObjectID, getItemType(), ownerID.c_str(), (int)storage, storageID, (int)x, (int)y, optionField.c_str(), optionField2.c_str(), getDurability(), getGrade(), (int)getHeroOption(),(int)getHeroOptionAttr(), (int)m_CreateType );
	...
	
{
	...
	BEGIN_DB
	{
		...
		*/
		...
		setOptionTypeToField( getOptionTypeList(), optionField );
		setOptionTypeToField( getOptionTypeList2(), optionField2 );
		pStmt->executeQuery( "UPDATE BeltObject SET ObjectID=%ld, ItemType=%d, OwnerID= '%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, OptionType='%s', OptionType2='%s', Durability=%d, Grade=%d, EnchantLevel=%d, HeroOption=%d, HeroOptionAttr=%d WHERE ItemID=%ld",
	...
	
{
	...
	BEGIN_DB
	{
		...
		*/
		...
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, OptionType2, Durability, Grade, EnchantLevel, HeroOption, HeroOptionAttr, ItemFlag FROM BeltObject WHERE OwnerID = '%s' AND Storage IN (0, 1, 2, 3, 4, 7, 9)",
		...
		while (pResult->next())
		{
			try {
				...
				setOptionTypeFromField(optionTypes2, optionField2);
								pBelt->setOptionType2(optionTypes2);


				pBelt->setDurability(pResult->getInt(++i));
				pBelt->setGrade(pResult->getInt(++i));
				pBelt->setEnchantLevel(pResult->getInt(++i));
				pBelt->setHeroOption(pResult->getInt(++i));
				pBelt->setHeroOptionAttr(pResult->getInt(++i));
	...
	
{
	...
	BEGIN_DB
	{
		...
		sql << "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y,"
			<< " OptionType, OptionType2, Durability, EnchantLevel, HeroOption, HeroOptionAttr, ItemFlag FROM BeltObject"
