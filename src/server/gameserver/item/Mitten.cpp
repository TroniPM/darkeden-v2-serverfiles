	...
	
{
	...
	BEGIN_DB 
	{
		...
		setOptionTypeToField( getOptionTypeList(), optionField );
		setOptionTypeToField( getOptionTypeList2(), optionField2 );
		pStmt->executeQuery( "UPDATE MittenObject SET ObjectID=%ld, ItemType=%d, OwnerID='%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, OptionType='%s', OptionType2='%s',Durability=%d, Grade=%d, EnchantLevel=%d, HeroOption=%d, HeroOptionAttr=%d WHERE ItemID=%ld",
	...
	
{
	...
	BEGIN_DB 
	{
		...
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, OptionType2, Durability, Grade, EnchantLevel, HeroOption, HeroOptionAttr, ItemFlag FROM MittenObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)", pCreature->getName().c_str() );
		...
		while (pResult->next())
		{
			try {
				...
				pMitten->setOptionType2(optionTypes2);

				pMitten->setDurability(pResult->getInt(++i));
				pMitten->setGrade(pResult->getInt(++i));
				pMitten->setEnchantLevel(pResult->getInt(++i));
pMitten->setHeroOption(pResult->getInt(++i));
pMitten->setHeroOptionAttr(pResult->getInt(++i));
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelts)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pMitten))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pMitten);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pMitten);
										}
