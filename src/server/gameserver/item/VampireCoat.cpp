	...
	
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		pStmt->executeQuery( "UPDATE VampireCoatObject SET ObjectID=%ld, ItemType=%d, OwnerID='%s', Storage=%d, StorageID=%ld, X=%d, Y=%d, OptionType='%s', OptionType2='%s', Durability=%d, Grade=%d, EnchantLevel=%d, HeroOption=%d, HeroOptionAttr=%d WHERE ItemID=%ld",
	...
	
{
	...
	BEGIN_DB 
	{
		...
		*/
		...
		Result* pResult = pStmt->executeQuery( "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y, OptionType, OptionType2, Durability, Grade, EnchantLevel, HeroOption, HeroOptionAttr, ItemFlag FROM VampireCoatObject WHERE OwnerID = '%s' AND Storage IN(0, 1, 2, 3, 4, 7, 9)",
		...
		while (pResult->next())
		{
			try {
				...
				pVampireCoat->setOptionType2(optionTypes2);
...
pVampireCoat->setHeroOption(pResult->getInt(++i));
pVampireCoat->setHeroOptionAttr(pResult->getInt(++i));
							...
							pItem = pInventory->findSubInventoryID(storageID, Item::ITEM_CLASS_SUB_INVENTORY);
								if (pItem != NULL)
								{
									...
									pBeltInventory = ((SubInventory*)pBelts)->getInventory();
										if (pBeltInventory->canAddingEx(x, y, pVampireCoat))
										{
											cout << " 추가완료"<< endl;
											pBeltInventory->addItemEx(x, y, pVampireCoat);
										}else{
											cout << " 버그"<< endl;
											processItemBugEx(pCreature, pVampireCoat);
										}
	...
	
{
	...
	BEGIN_DB 
	{
		...
		sql << "SELECT ItemID, ObjectID, ItemType, Storage, StorageID, X, Y,"
			<< " OptionType, OptionType2, Durability, EnchantLevel, HeroOption, HeroOptionAttr, ItemFlag FROM VampireCoatObject"
