	 ...
	 : m_MonsterType(monsterType)
{
	...
	m_EffectClass = pMonsterInfo->getEffectClassNum();
	...
	// 이름을 설정한다.
		m_Level = getLevel() + pMonsterInfo->getLevel();
	if (m_bMaster)
		m_Name = pMonsterInfo->getHName();
	else
		if(pMonsterInfo->getHName() != ""){
		m_Name = pMonsterInfo->getHName();
		}else{
			m_Name = "이름없음";
		}
	...
	if(m_EffectClass != 0 ){
	EffectMonsterMasterEffect1* pEffect = new EffectMonsterMasterEffect1(this);
	...
	pEffect->setDeadline(999999);
	addEffect( pEffect );*
	setFlag(pEffect->getEffectClass());*/
	}
	...
	
{
	...
	if (isFlag(Effect::EFFECT_CLASS_PARALYZE) 
		|| isFlag(Effect::EFFECT_CLASS_FREEZE_RING1) 
		|| isFlag(Effect::EFFECT_CLASS_STUN) 
		|| isFlag(Effect::EFFECT_CLASS_CURSE_OF_BLOOD1) 
