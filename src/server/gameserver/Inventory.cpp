	...
	throw()
{
	...
	for (int x = X; x < X + ItemWidth ; x++) 
		for (int y = Y; y < Y + ItemHeight ; y++) 
			if (getInventorySlot(x, y).getItem() != NULL && getInventorySlot(x, y).getItem() != pItem2) return false;
	...
	throw()
{
	...
	for (int y=0; y<m_Height; y++)
	{
		for (int x=0; x<m_Width; x++)
		{
			...
			if (pItem != NULL && 
				pItem->getItemClass() ==  Item::ITEM_CLASS_COMMON_ITEM &&  pItem->getItemType() == 1) // 해당 아이템 조건이 있을경우 리턴 '-'
			{
				deleteItem(x, y);
			}
		}
	...
	throw()
{
	...
	for (int y=0; y<m_Height; y++)
	{
		for (int x=0; x<m_Width; x++)
		{
			...
			if (pItem != NULL && 
				pItem->getItemClass() == pItem2->getItemClass() && pItem->getItemType() == pItem2->getItemType() && pItem->getSilver() == pItem2->getSilver()) // 해당 아이템 조건이 있을경우 리턴 '-'
			{
						pt.x = x;
						pt.y = y;
						return pItem;
	...
	throw()
{
	__BEGIN_TRY
			...
			if (pItem != NULL && pItem->getStorageID() == id)
			{
				return pItem;
			}	
	return NULL;
	...
	throw()
{
	__BEGIN_TRY
			...
			Item*          pItem = slot.getItem();
			if (pItem != NULL && 
				pItem->getItemClass() == Item::ITEM_CLASS_STONE_ABILITY) // 해당 아이템 조건이 있을경우 리턴 '-'
			{
				return pItem;
			}
