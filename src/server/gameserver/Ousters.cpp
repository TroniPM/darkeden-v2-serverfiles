	...
	throw (InvalidProtocolException, Error)
{
	...
	BEGIN_DB
	{
		...
		pResult = pStmt->executeQuery(
			...
			Rank, RankGoalExp, HairColor, KillPoint, ShopColor, AttackBloodBurstPoint, DefenseBloodBurstPoint, ADBloodBurstPoint FROM Ousters WHERE Name = '%s' AND Active = 'ACTIVE'",
			m_Name.c_str()
		);
...
//		setRankGoalExp( pResult->getInt(++i) );
		...
		setKillPoint(pResult->getInt(++i));
		setShopColor(pResult->getInt(++i));
		setAttackBloodBurstPoint(pResult->getInt(++i));
		setDefenseBloodBurstPoint(pResult->getInt(++i));
		setPartyBloodBurstPoin(pResult->getInt(++i));
		//setSylphType(pResult->getInt(++i));

		// maxHP를 다시 계산해서 설정해준다.
		// 2002.7.15 by sigi
		// 공식 바뀌면 AbilityBalance.cpp의 computeHP??수정해야한다.
    ...
    m_OustersInfo.setWingSylphType(m_WingSylphType);
    m_OustersInfo.setWingItemType(m_WingItemType);
    m_OustersInfo.setWingBodyColor(m_WingColor);
    m_OustersInfo.setWingEffectColor(m_WingEffectColor);
		...
		&& SystemAvailabilitiesManager::getInstance()->isAvailable( SystemAvailabilitiesManager::SYSTEM_GRAND_MASTER_EFFECT ) )
	{
		...
		/*if (!isFlag(Effect::EFFECT_CLASS_LOGIN_EVENT))
		{
			EffectLoginEvent* pEffect = new EffectLoginEvent(this);
			pEffect->setDeadline(999999);
			getEffectManager()->addEffect( pEffect );
			setFlag(Effect::EFFECT_CLASS_LOGIN_EVENT);
		}*/
	...
	throw()
{
	...
	if (isTwohandWeapon(pItem))
	{
		...
		if (isWear(WEAR_RIGHTHAND) && isWear(WEAR_LEFTHAND))
		{
			...
			{
				// 양손에 검과 방패를 ?欲?있었는데...양손 무기를 들려고 하면,
				// 검은 마우스 포인터에 달아줄 수 있지만, 방패는 어떻게 할 수가 없다.
				// 인벤토리에 넣어줘야 할 텐데, 지금 당장은 어떻게 할 지를 모르겠네...
				// 걍 입을 수 없다는 패킷을 보내주자...
				return;
	...
	throw()
{
	...
	// 다시 옷을 입히는데, 그러면 벗길 때 패킷을 한번, ?禿珦?때 다시 패킷을
	...
	throw()
{
	...
	if (part >= WEAR_BRACELET_ZAP1 && part <= WEAR_BRACELET_ZAP2)
	{
		// 해당 위치에 정령석도 있어야 된다.
		if ( m_pWearItem[part-WEAR_BRACELET_ZAP1+WEAR_RING1]==NULL ) return false;
	}

	if (part >= WEAR_NECKLACE_ZAP1 && part <= WEAR_NECKLACE_ZAP3)
	{
		// 해당 위치에 반지도 있어야 된다.
		if ( m_pWearItem[part-WEAR_NECKLACE_ZAP1+WEAR_NECKLACE1]==NULL ) return false;
	}
	if (part == WEAR_COAT_ZAP)
	{
		// 해당 위치에 반지도 있어야 된다.
		if ( m_pWearItem[part-WEAR_COAT_ZAP+WEAR_COAT]==NULL ) return false;
	}
	if (part == WEAR_BOOTS_ZAP)
	{
		// 해당 위치에 반지도 있어야 된다.
		if ( m_pWearItem[part-WEAR_BOOTS_ZAP+WEAR_BOOTS]==NULL ) return false;
	}
	if (part == WEAR_LEFTHAND_ZAP)
	{
		// 해당 위치에 반지도 있어야 된다.
		if ( m_pWearItem[part-WEAR_LEFTHAND_ZAP+WEAR_LEFTHAND]==NULL ) return false;
	}
	if (part == WEAR_CIRCLET_ZAP)
	{
		// 해당 위치에 반지도 있어야 된다.
		if ( m_pWearItem[part-WEAR_CIRCLET_ZAP+WEAR_CIRCLET]==NULL ) return false;
	}
	if (part == WEAR_ARMSBAND1_ZAP)
	{
		// 해당 위치에 반지도 있어야 된다.
		if ( m_pWearItem[part-WEAR_ARMSBAND1_ZAP+WEAR_ARMSBAND1]==NULL ) return false;
	}
	if (part == WEAR_ARMSBAND2_ZAP)
	{
		// 해당 위치에 반지도 있어야 된다.
		if ( m_pWearItem[part-WEAR_ARMSBAND2_ZAP+WEAR_ARMSBAND2]==NULL ) return false;
	}
...
/*	if ( m_pZone != NULL && m_pZone->isDynamicZone() && m_pZone->getDynamicZone()->getTemplateZoneID() == 4005 )
	...
	SkillType_t ReqSkillLearn = pItemInfo->getSkillLearn();
	...
	throw ()
{
	...
	pInfo->setKillPoint(m_KillPoint);;
	pInfo->setExp(m_GoalExp);
	pInfo->setGold(m_Gold);
	pInfo->setSight(m_Sight);
	pInfo->setBonus(m_Bonus);
	pInfo->setSkillBonus(m_SkillBonus);
	pInfo->setWingSylphType(m_WingSylphType);
	pInfo->setWingItemType(m_WingItemType);
	pInfo->setWingBodyColor(m_WingColor);
	pInfo->setWingEffectColor(m_WingEffectColor);
	// by sigi. 2002.8.30
	...
	pInfo->setAttackBloodBurstPoint(m_AttackBloodBurstPoint);
	pInfo->setDefenseBloodBurstPoint(m_DefenseBloodBurstPoint);
	pInfo->setPartyBloodBurstPoin(m_PartyBloodBurstPoint);
	...
	throw ()
{
    ...
    m_OustersInfo.setMasterEffectColor(m_MasterEffectColor);
    m_OustersInfo.setWingSylphType(m_WingSylphType);
    m_OustersInfo.setWingItemType((int)m_WingItemType);
    m_OustersInfo.setWingBodyColor(m_WingColor);
    m_OustersInfo.setWingEffectColor(m_WingEffectColor);
	...
	throw()
{
	...
	for (; itr != m_SkillSlot.end(); itr++)
	{
		...
		// AttackMelee 등의 기본 공격 ?茱?정보는 보내주지 않아야 한다.
    ...
    throw()
{
	...
	OustersSkillSlot* pDriftingSoul2 = hasSkill(SKILL_ENERGY_BURST);
	if ( pDriftingSoul2 != NULL && m_MPRegenTime < currentTime){
...
/*	char silverDam[40];
	...
	{
		pStmt = g_pDatabaseManager->getConnection("DARKEDEN")->createStatement();
		pStmt->executeQuery( "UPDATE Ousters SET Alignment=%d, Fame=%d, GoalExp=%lu, SilverDamage = %d, Rank=%d, RankGoalExp=%lu, AdvancementClass=%u, AdvancementGoalExp=%lu, KillPoint = %d, AttackBloodBurstPoint = %d, DefenseBloodBurstPoint = %d, ADBloodBurstPoint = %d WHERE Name='%s'",
							m_Alignment, m_Fame, m_GoalExp, m_SilverDamage, getRank(), getRankGoalExp(), getAdvancementClassLevel(), getAdvancementClassGoalExp(), m_KillPoint, m_AttackBloodBurstPoint, m_DefenseBloodBurstPoint, m_PartyBloodBurstPoint, m_Name.c_str() );
