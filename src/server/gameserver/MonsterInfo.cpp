	...
	
{
	...
	m_EnhanceProtectDamage     = 0;
	m_EnhanceExpRatio = 0;
	m_EnhanceMagicDamage = 0;             // 마법 데미지 증가 퍼센트 설정
	m_EnhancePhyDamage = 0;             // 물리 데미지 증가 퍼센트 설정
	m_EnhanceMagicProtection = 0;             // 마법 데미지 감소 퍼센트 설정
	m_EnhancePhyProtection = 0;             // 물리 데미지 감소 퍼센트 설정
	...
	
{
	...
	while (c < enhance.size() - 1)
	{
		...
		else if (part == "PROTECTDAMAGE")     m_EnhanceProtectDamage     = value;
		else if (part == "EXP")     m_EnhanceExpRatio = value;
		else if (part == "MAGICDAMAGE")     m_EnhanceMagicDamage = value;
		else if (part == "PHYDAMAGE")     m_EnhancePhyDamage = value;
		else if (part == "MAGICPROTECTION")     m_EnhanceMagicProtection = value;
		else if (part == "PHYPROTECTION")     m_EnhancePhyProtection = value;
	}
}
		...
		*/
		...
		pResult = pStmt->executeQuery( 
			"SELECT MType, SType, HName, EName, 
				...
				Master, ClanType, DefaultEffects, Chief, NormalRegen, HasTreasure, MonsterClass, SkullType, EffectClass
				FROM MonsterInfo");
		...
		while (pResult->next()) 
		{
			...
			pMonsterInfo->setEffectClassNum(pResult->getInt(++i));
	...
	
{
	...
	BEGIN_DB
	{
		...
		StringStream sql;
		sql << "SELECT MType, SType, HName, EName, Level, STR, DEX, INTE, BSize, Exp, MColor, SColor, Align, AOrder, Moral, Delay, ADelay, Sight, MeleeRange, MissileRange, RegenPortal, RegenInvisible, RegenBat, MMode, AIType, Enhance, UnburrowChance, Master, ClanType, MonsterSummonInfo, DefaultEffects, NormalRegen, EffectClass "
			<< " FROM MonsterInfo";
