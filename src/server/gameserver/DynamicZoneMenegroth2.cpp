...
DynamicZoneMenegroth2::DynamicZoneMenegroth2()
{
	m_MonoObjectID = 0;
	m_CabracamHP = 0;
}
...
void DynamicZoneMenegroth2::init()
{
	Assert( m_pZone != NULL );
...
void DynamicZoneMenegroth2::heartbeat()
{
	switch ( m_OMOAStatus )
	{
	case WAIT_FOR_PC:
...
bool DynamicZoneMenegroth2::checkPC()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth2::addMonster()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth2::addMonster_boss_regen()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth2::addMono()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth2::clearMono()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth2::checkMonoHPcount()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth2::checkMonoEffect()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth2::openGateToOut()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth2::openGateToOutNextStage()
{
	Assert( m_pZone != NULL );

	int targetDynamicZoneType = g_pDynamicZoneInfoManager->getDynamicZoneTypeByZoneID( 1703 );
	...
	DynamicZoneMenegroth3* pAlterOfBlood = dynamic_cast<DynamicZoneMenegroth3*>(pDynamicZone);
	...
	m_pZone->getPCManager()->transportAllCreatures( pDynamicZone->getZoneID(), 33, 95 );
        }
    }
...
void DynamicZoneMenegroth2::timecheck()
{
...
bool DynamicZoneMenegroth2::checkComplete()
{
	Timeval current;
	getCurrentTime( current );
...
void DynamicZoneMenegroth2::processEntering()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth2::checkMonster()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth2::checkbossmonster()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth2::checkMono()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth2::clearMonster()
{
...
void DynamicZoneMenegroth2::TreasureCheck()
{
...
void DynamicZoneMenegroth2::killPC()
{
	Assert( m_pZone != NULL );
...
DEFINE_DYNAMIC_ZONE_FACTORY( DynamicZoneMenegroth2, DYNAMIC_ZONE_MENEGROTH2 )
