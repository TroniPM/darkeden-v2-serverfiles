...
DynamicZoneIkLabSercet::DynamicZoneIkLabSercet()
{
	m_MonoObjectID = 0;
}
...
void DynamicZoneIkLabSercet::init()
{
	Assert( m_pZone != NULL );
...
void DynamicZoneIkLabSercet::heartbeat()
{
	switch ( m_OMOAStatus )
	{
	case WAIT_FOR_PC:
...
bool DynamicZoneIkLabSercet::checkPC()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneIkLabSercet::addMono()
{
...
bool DynamicZoneIkLabSercet::clearMono()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneIkLabSercet::openGateToOut()
{
	Assert( m_pZone != NULL );
...
void DynamicZoneIkLabSercet::timecheck()
{
...
bool DynamicZoneIkLabSercet::checkComplete()
{
	Timeval current;
	getCurrentTime( current );
...
void DynamicZoneIkLabSercet::processEntering()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneIkLabSercet::checkMono()
{
	Assert( m_pZone != NULL );
...
void DynamicZoneIkLabSercet::killPC()
{
	Assert( m_pZone != NULL );
...
DEFINE_DYNAMIC_ZONE_FACTORY( DynamicZoneIkLabSercet, DYNAMIC_ZONE_IK_LAB_SERCET )
