...
public:
	...
	int getEnhanceProtectDamage(void) const { return m_EnhanceProtectDamage; }
	int getEnhanceExpRatio(void) const { return m_EnhanceExpRatio; }
	int getEnhanceMagicDamage(void) const { return m_EnhanceMagicDamage; }
...
private:
	...
	int                m_EnhanceProtectDamage;             // 데미지 감소 퍼센트 설정
	int	         m_EnhanceExpRatio;			// 경험치 배수 결정
	int                m_EnhanceMagicDamage;             // 마법 데미지 증가 퍼센트 설정
	int                m_EnhancePhyDamage;             // 물리 데미지 증가 퍼센트 설정
	int                m_EnhanceMagicProtection;             // 마법 데미지 감소 퍼센트 설정
	int                m_EnhancePhyProtection;             // 물리 데미지 감소 퍼센트 설정
	...
	int					m_EffectClassNum;		// 몬스터 킬시 이펙트가 전체에게 붙는지, 개인에게 붙는지, 전종족에게 붙는지 설정하는 타입 부분.
};
