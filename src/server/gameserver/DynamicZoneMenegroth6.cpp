...
const MONSTER_TEMPLATE MonsterTemplate[6] =
{
	{ 898, 10 },
	{ 899, 10 },
	{ 900, 10 },
...
const MONSTER_TEMPLATE BossTemplate[3] =
{
	{ 904, 1 },
	{ 904, 1 },
	{ 904, 1 }
...
DynamicZoneMenegroth6::DynamicZoneMenegroth6()
{
	m_MonoObjectID = 0;
	m_CabracamHP = 0;
}
...
void DynamicZoneMenegroth6::init()
{
	Assert( m_pZone != NULL );
...
void DynamicZoneMenegroth6::heartbeat()
{
	switch ( m_OMOAStatus )
	{
	case WAIT_FOR_PC:
...
bool DynamicZoneMenegroth6::checkPC()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth6::addMonster()
{
	Assert( m_pZone != NULL );
for(int b=0; b <= 5; ++b){
	for ( int i = 0; i < MonsterTemplate[b].MonsterNum; ++i )
	{
		Monster* pMonster = new Monster( MonsterTemplate[b].MonsterType );
...
bool DynamicZoneMenegroth6::addMonster_boss_regen()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth6::addMono()
{
	Assert( m_pZone != NULL );
	int radomObject = Random(0,2);
for(int b=0; b <= 2; ++b){
	for ( int i = 0; i < BossTemplate[b].MonsterNum; ++i )
	{
		Monster* pMonster = new Monster( BossTemplate[b].MonsterType );
		...
		try
		{
			m_pZone->addCreature( pMonster, OfferingCoordinate[b].X, OfferingCoordinate[b].Y, Directions(rand()%8) );

			if(b == radomObject){m_MonoObjectID = pMonster->getObjectID();}
		}
		catch ( EmptyTileNotExistException& )
		{
			SAFE_DELETE( pMonster );
		...
		char msg[20];
		sprintf( msg, "카브라캄이 X : %d Y: %d 위치에 출몰 하였습니다.", OfferingCoordinate[b].X, OfferingCoordinate[b].Y);
...
bool DynamicZoneMenegroth6::clearMono()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth6::checkMonoHPcount()
{
	Assert( m_pZone != NULL );
...
void DynamicZoneMenegroth6::TreasureCheck()
{
	...
	for (i=0, itr=creatures.begin(); itr!=creatures.end(); i++, itr++)
	{
		...
		if (pCreature->isPC())
		{
			...
			MonsterDropSetup* pMonsterItemDrop = g_pMonsterDropSetupManager->getMonsterDropSetup(60);
...
void DynamicZoneMenegroth6::TreasureCheck2()
{
	...
	for (i=0, itr=creatures.begin(); itr!=creatures.end(); i++, itr++)
	{
		...
		if (pCreature->isPC())
		{
			...
			MonsterDropSetup* pMonsterItemDrop = g_pMonsterDropSetupManager->getMonsterDropSetup(67);
		if ( pMonsterItemDrop != NULL && pMonsterItemDrop->IsDrop() )
		{
			if (ownerCreatureClass == Creature::CREATURE_CLASS_SLAYER)
			{
...
bool DynamicZoneMenegroth6::checkMonoEffect()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth6::openGateToOut()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth6::openGateToOutNextStage()
{
...
void DynamicZoneMenegroth6::timecheck()
{
	const PCManager* pPCManager = m_pZone->getPCManager();
...
bool DynamicZoneMenegroth6::checkComplete()
{
	Timeval current;
	getCurrentTime( current );
...
void DynamicZoneMenegroth6::processEntering()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth6::checkMonster()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth6::checkbossmonster()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth6::checkMono()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneMenegroth6::clearMonster()
{
...
void DynamicZoneMenegroth6::killPC()
{
	Assert( m_pZone != NULL );
...
DEFINE_DYNAMIC_ZONE_FACTORY( DynamicZoneMenegroth6, DYNAMIC_ZONE_MENEGROTH6 )
