...
{ 
	...
	if (m_ReqLevel > 0)
	{
		...
		{
			Slayer* pSlayer = dynamic_cast<Slayer*>(pCreature2);
			current = pSlayer->getHighestSkillDomainLevel();
	...
	if (m_ReqAdv > 0)
	{
		...
		{
			Slayer* pSlayer = dynamic_cast<Slayer*>(pCreature2);
			current = pSlayer->getSlayerInfo2()->getAdvancementLevel();
			if( !pSlayer->isAdvanced() ) return false;
			if (!satisfy(m_LevelOperator, current, m_ReqAdv)) return false;
			return true;
		}
		else if (pCreature2->isVampire())
		{
			Vampire* pVampire = dynamic_cast<Vampire*>(pCreature2);
			current = pVampire->getVampireInfo2()->getAdvancementLevel();
			if( !pVampire->isAdvanced() ) return false;
			if (!satisfy(m_LevelOperator, current, m_ReqAdv)) return false;
		}
		else if (pCreature2->isOusters())
		{
			Ousters* pOusters = dynamic_cast<Ousters*>(pCreature2);
			current = pOusters->getOustersInfo2()->getAdvancementLevel();
			if( !pOusters->isAdvanced() ) return false;
			if (!satisfy(m_LevelOperator, current, m_ReqAdv)) return false;
		}
	}
