...
{
	{ {909,910,911,912},{9,11,7,3},{88, 97},{37,47},{188,189,190,186} },
	{ {909,910,911,916},{9,11,7,3},{15, 24},{27,37},{188,189,190,184} },
	{ {909,910,911,913},{9,11,7,3},{33, 48},{88,112},{188,189,190,187} },
	{ {915,912,913,914},{9,11,7,1},{63, 76},{73,87},{188,189,190,185} },
};

DynamicZoneTiffagues_2_2::DynamicZoneTiffagues_2_2()
{
	m_MonoObjectID = 0;
	m_Stage = 0;
	m_StageMessage = 0;
...
void DynamicZoneTiffagues_2_2::init()
{
	Assert( m_pZone != NULL );
...
void DynamicZoneTiffagues_2_2::heartbeat()
{
	switch ( m_OMOAStatus )
	{
	case WAIT_FOR_PC:
	...
	case STAGE_ING:
		...
		gcSystemMessage.setMessage( " 살아남아 돌아가는걸 축하하지...하하하하 "  );
...
bool DynamicZoneTiffagues_2_2::checkPC()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneTiffagues_2_2::checkMonster()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneTiffagues_2_2::addMono()
{
...
void DynamicZoneTiffagues_2_2::addMonster(int Type)
{
	Assert( m_pZone != NULL );
...
void DynamicZoneTiffagues_2_2::addMsg(int blockType)
{
	StringStream msg;
...
bool DynamicZoneTiffagues_2_2::clearMono()
{
	Assert( m_pZone != NULL );
...
void DynamicZoneTiffagues_2_2::addBlock(int blockType)
{
	for(int i=0; i <= 3; i++){
		for(int s=0; s<3; s++){
...
void DynamicZoneTiffagues_2_2::clearBlock(int blockType)
{
	for(int i=0; i <= 3; i++){
		for(int s=0; s<3; s++){
...
void DynamicZoneTiffagues_2_2::AddEffect(int blockType)
{
...
void DynamicZoneTiffagues_2_2::TreasureCheck()
{
	...
	for (i=0, itr=creatures.begin(); itr!=creatures.end(); i++, itr++)
	{
		...
		if (pCreature->isPC())
		{
			...
			BEGIN_DB
			{
				...
				pStmt->executeQuery( "INSERT INTO EventSimyunTime (OwnerID, GameTower, GameTime, DateTime, DungeonType) VALUES('%s', 0, 0, now(), 8)",
			...
			MonsterDropSetup* pMonsterItemDrop = g_pMonsterDropSetupManager->getMonsterDropSetup(70);
...
void DynamicZoneTiffagues_2_2::TreasureCheck2()
{
	const PCManager* pPCManager = m_pZone->getPCManager();
	const hash_map< ObjectID_t, Creature* > & creatures = pPCManager->getCreatures();
	hash_map< ObjectID_t, Creature* >::const_iterator itr;
...
bool DynamicZoneTiffagues_2_2::sayMonster()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneTiffagues_2_2::openGateToOut()
{
	Assert( m_pZone != NULL );
...
void DynamicZoneTiffagues_2_2::timecheck()
{
	const PCManager* pPCManager = m_pZone->getPCManager();
...
bool DynamicZoneTiffagues_2_2::checkComplete()
{
	Timeval current;
	getCurrentTime( current );
...
bool DynamicZoneTiffagues_2_2::checkMsgComplete()
{
	Timeval current;
	getCurrentTime( current );
...
bool DynamicZoneTiffagues_2_2::checkMonsterMsgComplete()
{
	Timeval current;
	getCurrentTime( current );
...
void DynamicZoneTiffagues_2_2::processEntering()
{
	Assert( m_pZone != NULL );
...
bool DynamicZoneTiffagues_2_2::checkMono()
{
	Assert( m_pZone != NULL );
...
void DynamicZoneTiffagues_2_2::killPC()
{
	Assert( m_pZone != NULL );
...
DEFINE_DYNAMIC_ZONE_FACTORY( DynamicZoneTiffagues_2_2, DYNAMIC_ZONE_TIFFAGUES_2_2 )
