...
list<OptionType_t> OptionInfoManager::getMixOptionInfo(int OptionType)
	throw (NoSuchElementException, Error)
{
	__BEGIN_TRY
	...
	for (itr = m_OptionInfos.begin(); itr != m_OptionInfos.end() ; itr++)
	{
		if(OptionType == itr->second->getClass())
		m_OptionTypeClass.push_back(itr->second->getType());
	}
	return m_OptionTypeClass;
	__END_CATCH
}
